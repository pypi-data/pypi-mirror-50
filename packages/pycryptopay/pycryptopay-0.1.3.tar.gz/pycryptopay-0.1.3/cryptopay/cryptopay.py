...
class Application:
    MAIN_URL = 'https://api.cryptopay.click'
    ...
    def _process_request(self, *, method: str, endpoint: str, json=None):
        response = getattr(requests, method)(self.MAIN_URL + endpoint, headers={'app-token': self.token}, json=json)
        response.raise_for_status()
        return response.json()

    async def _process_request_async(self, *, method: str, endpoint: str, json=None):
        async with aiohttp.ClientSession() as session:
            resp = await getattr(session, method)(self.MAIN_URL + endpoint, headers={'app-token': self.token}, json=json)
