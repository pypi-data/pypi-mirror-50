...
def requestDialog(title, message):
    #TODO Nice feature would be to have a timeout so if there is not user input in 10 seconds then use default value. (press ok with empty box to skip 10seconds and go to default instantly)
    if user_interface.instance != None and user_interface.instance.selectedInterface == "testcenter":

        return TestCenter.testPoint("Quarch_Host.ShowRequestDialog","Title= " + __formatForTestcenter(title), "Message=" + __formatForTestcenter(message));
