    ...
    class HDCalibration (Calibration):
        ...
        def init_cal(self,voltage):
            ...
            if (int(unitTemp[0:2]) < minTemp):
                ...
                tempDifference = minTemp - int(unitTemp[0:2])
                startTemp = int(unitTemp[0:2])
                printText("\nRequired internal temperature: " + str(minTemp) + "C, current internal temperature: " + unitTemp)
                while(int(unitTemp[0:2]) < minTemp):
                    unitTemp = self.powerModule.dut.sendCommand('meas:temp unit?')
                    tempTimeout = 0
                    sleep(1)
                    tempTimeout += 1
                    progressBar((int(unitTemp[0:2])-startTemp),tempDifference)
