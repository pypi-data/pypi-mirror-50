    ...
    try:
        ...
        while inputOK is False:
            if (calPath is None):
                ...
                calPath = requestDialog("Enter Report Path", "Default path : "+calPath+"\nEnter the path where you would like the report to be saved (leave blank to default)")
                if calPath == "":
                    calPath = os.path.expanduser("~")
        ...
        inputOK = False
        while inputOK is False:
            calTemp = requestDialog("Enter Enclosure Temperature","Enter the enclosure temperature (if unknown leave blank)")
            if calTemp == "":
                calTemp=None

            if (calTemp is None or (float(calTemp)>0 and float(calTemp)<99)):
                inputOK = True
            else:
                printText("Supplied temperature path is invalid: " + calTemp)
                inputOK = False
                calTemp = None
