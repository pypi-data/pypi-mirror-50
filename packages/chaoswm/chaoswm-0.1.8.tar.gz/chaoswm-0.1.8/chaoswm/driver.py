    ...
    def up(self, filter: list=[]):
        ...
        for f in filter:
            ...
            if (not mapping_found):
                ...
                self.delete_mapping(mapping_found["id"])
                priority = mapping_found.get('priority', 1)
                up_mapping = {
                    "priority": priority,
                    "request": mapping_found["request"],
                    "response": mapping_found["response"]
                }
                ids.append(self.add_mapping(up_mapping))
        return ids
