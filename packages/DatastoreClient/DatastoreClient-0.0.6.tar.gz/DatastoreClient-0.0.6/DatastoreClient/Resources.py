...
class Resource(object):
    ...
    def create(
        ...
        success_files, failed_files, already_in_bucket_files = self.uploader.upload(files)
        create_record = self._create(success_files) + self._create(already_in_bucket_files)
        ...
        for file in files:
            ...
            if datas:
                ...
                if lastest['metadata'] != file['metadata'] or lastest['md5'] != file['md5']:
                    ...
                    need_removes.append(file)
            else:
                file['version'] = 0
            bar.update(1)
        for need_remove in need_removes:
            files.remove(need_remove)
