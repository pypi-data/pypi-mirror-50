...
class MEP:
    def __init__(self, bo, space, min_points,
                 precision=25, rrtsteps=10000, nebsteps=10, maxE=None):
        """
        Parameters:
        ...
        self.eps = 1/precision * np.min( self.space.bounds[1,:]
                                        -self.space.bounds[0,:])
        ...
        if maxE == None: maxE = np.max(self.bo.get_y())
    ...
    def run_mep(self, mainOutput):
        ...
        mainOutput.progress_msg('Running NEB for ' + 
            str(len(self.connections)) + ' paths.', 1)
        self.get_paths()
    ...
    def get_fullcrds(self, i, j, notchecked=None):
        ...
        if checkind.shape[0] == 0: return None

        found = [path.mi == j or path.mj == j for path in self.paths[checkind]]
        if np.any(found):
            path = self.paths[checkind[found]][0]
            if path.mj == j: return path.crds
            else: return np.flipud(path.crds)
        else:
            for ind in checkind:
                if self.paths[ind].mi == i: newi = self.paths[ind].mj
                else: newi = self.paths[ind].mi
                ...
                if np.all(fullcrds != None):
                    ...
                    else: return np.vstack((np.flipud(path.crds), fullcrds))
