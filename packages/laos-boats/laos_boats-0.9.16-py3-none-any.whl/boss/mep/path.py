...
class Path:
    ...
    def initFromConnection(self, connection):
        ...
        for node in parents_A: crds = np.vstack((crds, node.coords))
        crds = np.vstack((crds, end_node_A.coords, end_node_B.coords))
        for node in parents_B: crds = np.vstack((crds, node.coords))
        self.crds = crds
        ...
        while True:
            ...
            if nextdist >= (eps - prevdist):
                ...
                if next_i == self.crds.shape[0] -1: # if next point is the last
                    ...
                    self.crds = newpath
                    return
                d = np.linalg.norm(self.space.vec(
                           newpath[newpath.shape[0]-1,:], self.crds[next_i,:]))
                prevdist = d
                prev_pt = self.crds[next_i,:]
                next_i += 1
                direction = self.space.vec(self.crds[next_i,:],
                                           self.crds[next_i-1, :])
