...
def authenticate(
    ...
    def authenticate(state):
        ...
        if getattr(state.manage_user, mfa_enabled_attr, None):
            if not getattr(state.manage_user, mfa_authorized_attr, None):
                ...
                redirect_url = flask.url_for(
                    ...
                    caller_url=flask.request.full_path
                )
        ...
        if _mfa_required is None:
            _mfa_required = flask.current_app.config.get('MFA_REQUIRED')
...
def mfa_authenticate_scoped_session(
    ...
    if get_cache is None:
        get_cache = lambda s: \
                flask.current_app.config['MFA_SCOPED_SESSION_CACHE']
    ...
    def mfa_authenticate_scoped_session(state):
        ...
        if getattr(state.manage_user, enabled_attr, None):
            ...
            # Parse URL
            caller_url_parts = list(urlparse.urlparse(flask.request.full_path))

            # Remove 'mfa_scoped_session_key' from the query parameters
            query = dict(urlparse.parse_qsl(caller_url_parts[4]))
            query.pop('mfa_scoped_session_key', None)
            ...
            session_key = flask.request.values.get('mfa_scoped_session_key')
            if session_key:
...
def mfa_end_scoped_session(
    ...
    def mfa_end_scoped_session(state):
        secure.mfa.delete_scoped_session(
            get_cache,
            flask.request.values.get('mfa_scoped_session_key')
        )
