...
class LibTransformer(ast.NodeTransformer):
    ...
    def visit_FunctionDef(self, node):
        ...
        if '__' != node.name[0:2] or node.name == '__init__':
            ...
            if not self.active:
                # This is a trick so we don't transform functions containing yields/awaits/etc.
                write_debug_msg("Voluntarily did not transform function in file: {}".format(self.filepath))
                return node
    ...
    def visit_Yield(self, node):
        # TODO: Patch, we do not track generators, only functions
        self.active = False
        return node
