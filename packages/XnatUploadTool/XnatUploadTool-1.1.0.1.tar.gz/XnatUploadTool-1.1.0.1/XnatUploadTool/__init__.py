...
class XnatUploadTool:
    ...
    def upload_ingestedfile(self, subject=None, session=None, scan=None, scandata=None, resource=None):
        ...
        myurl = ("%sdata/projects/%s/subjects/%s/experiments/%s/scans/%s" %
                 (self.host, self.project, subject, session, scan))

        mydata = {'file': ((open(scandata['path'], 'rb')), 'multipart/form-data')}

        try:
            myurl = ("%s/resources/%s/files/%s?event_reason=batch_upload&event_action=UploadedFile&"
                     "update_stats=false&format=json" % (myurl, resource, os.path.basename(scandata['path'])))
