...
import warnings
import numpy as np
from .settings import MetricTypePrefixes, MetricPhasePrefixes
...
class Phase(object):
    ...
    _bad_prefix_custom = MetricPhasePrefixes.TRAIN + MetricTypePrefixes.CUSTOM + MetricPhasePrefixes.VAL
    _bad_prefix = MetricPhasePrefixes.TRAIN + MetricPhasePrefixes.VAL
    ...
    def __log_metric_raw(self, name, value, total=1, apply_prefix=True):
        ...
        if prefixed_name.startswith(self._bad_prefix_custom) or prefixed_name.startswith(self._bad_prefix):
            prefixed_name = prefixed_name.replace(MetricPhasePrefixes.VAL, '')
            prefixed_name = prefixed_name.replace(MetricPhasePrefixes.TRAIN, MetricPhasePrefixes.VAL)
...
class PhaseValidation(Phase):
    def log_metric(self, name, value, is_custom=True):
        name = MetricPhasePrefixes.VAL + name
        super(PhaseValidation, self).log_metric(name, value, is_custom)
