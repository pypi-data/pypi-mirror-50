...
def get_imagenet_transformers(size=224, resize=256):
  train_transform = Transformer(
    load=GetImage('RGB'),
    transform=transforms.Compose([
      transforms.Resize(resize, interpolation=Image.ANTIALIAS),
      transforms.RandomResizedCrop(size, scale=(.4, 1),
                                   interpolation=Image.ANTIALIAS),
      transforms.ColorJitter(brightness=.4, contrast=.2, saturation=.1,
                             hue=.05),
