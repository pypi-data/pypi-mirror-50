...
class HyperDriveConfig(object):
    ...
    def __init__(self,
        ...
        :type policy: azureml.train.hyperdrive.EarlyTerminationPolicy
        :param estimator: An estimator that will be called with sampled hyper parameters.
                          One of estimator, run_config and pipeline is mandatory.
        :type estimator: azureml.train.estimator.MMLBaseEstimator
        ...
        :param pipeline: A pipeline object for setting up configuration for pipeline runs.
                         The pipeline object will be called with the sample hyper parameters to submit pipeline runs.
                         One of estimator, run_config and pipeline is mandatory.
    ...
    def estimator(self):
        ...
        The estimator may be None if this object was built from server side data or using a run_config/pipeline.
    ...
    @property
    def pipeline(self):
        """Return the pipeline in this config.

        The pipeline may be None if this object was built from server side data or using run_config/estimator.
    ...
    def _get_platform_config(self, workspace, run_name, **kwargs):
        ...
        if self._pipeline is not None:
            ...
            platform_config.update(self._get_platform_config_data_from_run_config())

        self._platform_config = platform_config

        return self._platform_config

    def _get_platform_config_data_from_run_config(self):
