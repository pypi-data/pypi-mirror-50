...
w : image width
h : image height
br, bg, bb : background color's rgb value
x, y : texts position in image
tr, tg, tb : text color's rgb value
sx, sy : text position in screen(Pygame)
'''
def Hprint(a, w, h, br, bg, bb, x, y, tr, tg, tb, sx, sy):

    img = Image.new('RGB', (w, h), color=(br,bg,bb))
    fnt = ImageFont.truetype('NanumSquareRoundB.ttf', 25)
    d = ImageDraw.Draw(img)
    d.text((x, y), a, font=fnt, fill=(tr, tg, tb))
    img.save('hangul3.png')
    Hangul = p.image.load("hangul3.png")
    screen.blit(Hangul,(sx ,sy))
