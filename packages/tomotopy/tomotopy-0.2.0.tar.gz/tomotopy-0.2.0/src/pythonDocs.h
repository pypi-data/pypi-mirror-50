...
* `rm_top`: the number of top words to be removed. If you want to remove too common words from model, you can set this value to 1 or more.
...
* `rm_top`: 제거될 최상위 빈도 단어의 개수. 만약 너무 흔한 단어가 토픽 모델 상위 결과에 등장해 이를 제거하고 싶은 경우, 이 값을 1 이상의 수로 설정하십시오.
...
* `doc` : an instance of `tomotopy.Document` or a `list` of instances of `tomotopy.Document` to be inferred by the model.
It can be acquired from `tomotopy.LDAModel.make_doc` method.
* `iter` : an integer indicating the number of iteration to estimate the distribution of topics of `doc`.
The higher value will generate a more accuracy result.
* `tolerance` isn't currently used.
* `workers` : an integer indicating the number of workers to perform samplings. 
If `workers` is 0, the number of cores in the system will be used.
* `together` : all `doc`s are infered together in one process if True, otherwise each `doc` is infered independently. Its default value is `False`.
)"",
u8R""(새로운 문헌인 `doc`에 대해 각각의 주제 분포를 추론하여 반환합니다.
반환 타입은 (`doc`의 주제 분포, 로그가능도) 또는 (`doc`의 주제 분포로 구성된 `list`, 로그가능도)입니다.

* `doc` : 추론에 사용할 `tomotopy.Document`의 인스턴스이거나 이 인스턴스들의 `list`.
이 인스턴스들은 `tomotopy.LDAModel.make_doc` 메소드를 통해 얻을 수 있습니다.
* `iter` : `doc`의 주제 분포를 추론하기 위해 학습을 반복할 횟수입니다.
이 값이 클 수록 더 정확한 결과를 낼 수 있습니다.
* `tolerance` : 현재는 사용되지 않음
* `workers` : 깁스 샘플링을 수행하는 데에 사용할 스레드의 개수입니다. 
만약 이 값을 0으로 설정할 경우 시스템 내의 가용한 모든 코어가 사용됩니다.
* `together` : 이 값이 True인 경우 입력한 `doc` 문헌들을 한 번에 모델에 넣고 추론을 진행합니다.
...
DOC_SIGNATURE_EN_KO(CT___init____doc__,
	...
	u8R""(This type provides Correlated Topic Model (CTM) and its implementation is based on following papers:
...
u8R""(이 타입은 Correlated Topic Model (CTM)의 구현체를 제공합니다. 주요 알고리즘은 다음 논문에 기초하고 있습니다:
...
.. versionadded:: 0.2.0
* `k` : the number of topics between 1 ~ 32767.
* `vars` : an iterable of `str`, indicating types of response variables.
...
* `alpha` : hyperparameter of Dirichlet distribution for document-topic
* `eta` : hyperparameter of Dirichlet distribution for topic-word
* `mu` : mean of regression coefficients, in `float` or an `list` of `float`
* `nu_sq` : variance of regression coefficients, in `float` or an `list` of `float`
* `glm_param` : the parameter for Generalized Linear Model, in `float` or an `list` of `float`
* `seed` : random seed. The default value is a random number from `std::random_device{}` in C++
)"",
...
.. versionadded:: 0.2.0
* `k` : 토픽의 개수, 1 ~ 32767 범위의 정수.
* `vars` : 응답변수의 종류를 지정하는 `str` 타입의 iterable.
...
* `alpha` : 문헌-토픽 디리클레 분포의 하이퍼 파라미터
* `eta` : 토픽-단어 디리클레 분포의 하이퍼 파라미터
* `mu` : 회귀 계수의 평균값, `float`나 `float`의 `list` 타입이어야 합니다.
* `nu_sq` : 회귀 계수의 분산값, `float`나 `float`의 `list` 타입이어야 합니다.
* `glm_param` : 일반화 선형 모형에서 사용될 파라미터, `float`나 `float`의 `list` 타입이어야 합니다.
* `seed` : 난수의 시드값. 기본값은 C++의 `std::random_device{}`이 생성하는 임의의 정수입니다.
이 값을 고정하더라도 `train`시 `workers`를 2 이상으로 두면, 멀티 스레딩 과정에서 발생하는 우연성 때문에 실행시마다 결과가 달라질 수 있습니다.
)"");
...
* `words` : an iterable of `str`
* `y` : a `list` of `float`, which is used as response variables of this document. 
...
* `words` : 문헌의 각 단어를 나열하는 `str` 타입의 iterable
* `y` : 문헌의 응답 변수로 쓰일 `float`의 `list`. `y`의 길이는 모델의 응답 변수의 개수인 `tomotopy.SLDAModel.f`와 일치해야 합니다.
)"");
...
* `var_id`: an integer in range [0, `f`), indicating the reponse variable)"",
	u8R""(응답 변수 `var_id`의 회귀 계수를 반환합니다.

* `var_id`: 응답 변수를 지정하는 [0, `f`) 범위의 정수)"");
...
* `doc`: an instance of `tomotopy.Document` to be used for estimating response variables)"",
...
* `doc`: 응답 변수를 추정하려하는 `tomotopy.Document`의 인스턴스)"");
