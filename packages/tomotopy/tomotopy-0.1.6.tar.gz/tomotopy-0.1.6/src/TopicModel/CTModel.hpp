...
namespace tomoto
{
	...
	struct DocumentCTM : public DocumentLDA<_TW, _Shared>
	{
		...
		Eigen::Matrix<FLOAT, -1, -1> beta; // Dim: (K - 1, betaSample)
	...
	public:
		...
		static ICTModel* create(TermWeight _weight, size_t _K = 2,
	...
	protected:
		...
		{
			BaseClass::sampleDocument(doc, ld, rgs);
			if (this->iterated >= this->burnIn && this->optimInterval && (this->iterated + 1) % this->optimInterval == 0)
			{
				updateBeta(doc, rgs);
			}
		}
		...
		void optimizeParameters(ThreadPool& pool, _ModelState* localData, RANDGEN* rgs)
		{
			...
			for (size_t ch = 0; ch < chStride; ++ch)
			{
				res.emplace_back(pool.enqueue([&, this](size_t threadId, size_t ch)
				{
					for (size_t i = ch; i < this->docs.size(); i += chStride)
					{
						//this->docs[i].smBeta.head(this->K) = this->docs[i].beta.rowwise().mean();
						this->docs[i].smBeta.head(this->K) = this->docs[i].beta.col(0);
						this->docs[i].smBeta = this->docs[i].smBeta.array().exp();
						this->docs[i].smBeta /= this->docs[i].smBeta.array().sum();
					}
				}, ch));
			}
			for (auto&& r : res) r.get();
		...
		DEFINE_SERIALIZER_AFTER_BASE(BaseClass, topicPrior);

	public:
		CTModel(size_t _K = 2, FLOAT smoothingAlpha = 0.1, FLOAT _eta = 0.01, const RANDGEN& _rg = RANDGEN{ std::random_device{}() })
