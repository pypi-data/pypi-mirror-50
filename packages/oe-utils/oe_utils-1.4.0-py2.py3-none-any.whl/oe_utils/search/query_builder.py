...
AND_FILTER_REGEXES = [
    re.compile('([^&]+(&)(?:[^&]+&?)*)'),  # Matches Gent & groen
]
OR_FILTER_REGEXES = [
    re.compile(r'\[((?:[^,]+(,)?)+)\]'),  # Matches [Gent,groen]
]
...
class QueryBuilder:
    ...
    def parse_full_text_search_param(self, value):
        """Converts the user queryparam input to a valid query_string for ES
    ...
    def set_full_text_search_query(
            self, field_boosts, query, default_operator='AND',
            flags='AND|OR|NOT|PREFIX|PHRASE|PRECEDENCE|ESCAPE|WHITESPACE',
            analyze_wildcard=True):
        """
        Builds a simple_query_string query.
