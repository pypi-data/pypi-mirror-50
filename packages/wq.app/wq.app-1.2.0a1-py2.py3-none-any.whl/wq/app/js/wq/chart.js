/*
 * wq.app v1.2.0a1 - @wq/chart 1.2.0-alpha.1
...
chart.base = function () {
    var width = 700,
        ...
        innerFill = '#eee',
        legend = null,
        leftYAxis = true; // Accessors for entire data object

    var datasets = function datasets(d) {
        if (d.data) {
            return d.data;
        }
    ...
    var legendItems = function legendItems(d) {
        return datasets(d);
    }; // Accessors for individual datasets


    var id = function id(dataset) {
        return dataset.id;
    };

    var label = function label(dataset) {
        return dataset.label;
    };

    var items = function items(dataset) {
        return dataset.data || dataset.list;
    };

    var legendItemId = function legendItemId(d) {
        return id(d);
    };

    var legendItemLabel = function legendItemLabel(d) {
        return label(d);
    };
    ...
    var xunits = function xunits(dataset) {
        /* eslint no-unused-vars: off */
        return null;
    };

    var xmax = function xmax(dataset) {
        return d3.max(items(dataset), xvalue);
    };

    var xmin = function xmin(dataset) {
        return d3.min(items(dataset), xvalue);
    };

    var xset = function xset(d) {
        var xvals = d3.set();
    ...
    var yunits = function yunits(dataset) {
        return dataset.units;
    };

    var ymax = function ymax(dataset) {
        return d3.max(items(dataset), yvalue);
    };

    var ymin = function ymin(dataset) {
        return d3.min(items(dataset), yvalue);
    }; // Accessors for individual items


    var xscaled = function xscaled(d) {
        return xscale.scale(xvalue(d));
    };

    var yscaled = function yscaled(scaleid) {
    ...
    var itemid = function itemid(d) {
        return xvalue(d) + '=' + yvalue(d);
    }; // Rendering functions (should be overridden)


    var init = function init(datasets, opts) {
        /* eslint no-unused-vars: off */
    };

    var render = function render(dataset) {
        /* eslint no-unused-vars: off */
    };

    var wrapup = function wrapup(datasets, opts) {
        /* eslint no-unused-vars: off */
    }; // Legend item rendering


    var legendItemShape = function legendItemShape(sid) {
        /* eslint no-unused-vars: off */
        return 'rect';
    };

    var rectStyle = function rectStyle(sid) {
        var color = cscale(sid);
        return function (sel) {
            sel.attr('x', -3).attr('y', -3).attr('width', 6).attr('height', 6).attr('fill', color);
        };
    };

    var circleStyle = function circleStyle(sid) {
        var color = cscale(sid);
        return function (sel) {
            sel.attr('r', 3).attr('fill', color).attr('stroke', 'black').attr('stroke-width', 0.2).attr('cursor', 'pointer');
        };
    };

    var legendItemStyle = function legendItemStyle(sid) {
        return rectStyle(sid);
    }; // Generate translation function xscale + given yscale


    var translate = function translate(scaleid) {
    ...
    function _plot(data) {
        ...
        init.call(this, datasets(data), opts); // Clip for inner graphing area

        var clipId = 'clip' + uid;

        var defs = _selectOrAppend(svg, 'defs'); // Webkit can't select clipPath #83438
        ...
        clip.select('rect').attr('width', gwidth).attr('height', gheight); // Outer chart area (includes legends, axes & actual graph)

        var outer = _selectOrAppend(svg, 'g', 'outer');

        outer.attr('transform', _trans(padding, padding, true));

        _selectOrAppend(outer, 'rect').attr('width', cwidth).attr('height', cheight).attr('fill', outerFill); // Inner graphing area (clipped)


        var inner = _selectOrAppend(outer, 'g', 'inner').attr('clip-path', 'url(#' + clipId + ')').attr('transform', _trans(margins.left, margins.top));

        _selectOrAppend(inner, 'rect').attr('width', gwidth).attr('height', gheight).attr('fill', innerFill);
        ...
        if (ordinal) {
            xscale.scale.domain(xset(data).sort(d3.ascending)).range([0, gwidth]).padding(0.5);
        } else {
        ...
        if (renderBackground) {
            var background = _selectOrAppend(inner, 'g', 'background').selectAll('g.dataset-background').data(datasets(data), id);

            background.enter().append('g').attr('class', 'dataset-background').merge(background).each(renderBackground);
            background.exit().remove();
        }

        var series = _selectOrAppend(inner, 'g', 'datasets').selectAll('g.dataset').data(datasets(data), id);

        series.enter().append('g').attr('class', 'dataset').merge(series).each(render);
        series.exit().remove(); // Render axes

        var xaxis = _selectOrAppend(outer, 'g', 'xaxis').attr('transform', _trans(margins.left, cbottom)).call(xscale.axis),
            xlabel = xunits(data);

        if (xlabel) {
            _selectOrAppend(xaxis, 'text', 'axislabel').text(xlabel).attr('text-anchor', 'middle').attr('font-weight', 'bold').attr('fill', '#000').attr('transform', function () {
                return 'translate(' + gwidth / 2 + ', 30)';
            });
        }

        var yaxes = outer.selectAll('g.axis').data(d3.values(yscales), function (s) {
            return s.id;
        });
        ...
        newaxes.merge(yaxes).attr('transform', function (d) {
            ...
            axis.call(d.axis);
            axis.select('text').text(d.id || '').attr('text-anchor', 'middle').attr('font-weight', 'bold').attr('fill', '#000').attr('transform', function () {
                if (d.orient == 'left') {
                    return 'rotate(-90),' + 'translate(-' + gheight / 2 + ',-30)';
                } else {
                    return 'rotate(90),' + 'translate(' + gheight / 2 + ',-30)';
                }
            });
    ...
    function _renderLegend(items, opts) {
        ...
        var leg = _selectOrAppend(outer, 'g', 'legend').attr('transform', _trans(legendX, legendY));

        _selectOrAppend(leg, 'rect').attr('width', legendW).attr('height', legendH).attr('fill', 'white').attr('stroke', '#999');
        ...
        newitems.append('g').attr('class', 'data').each(function (d) {
            ...
            g.append('text').attr('font-family', 'sans-serif').attr('font-size', '13px').attr('cursor', 'pointer');
        });
        newitems.merge(legitems).each(function (d, i) {
            ...
            g.select('text').text(legendItemLabel(d)).attr('transform', _trans(10, 5));
        });
...
chart.scatter = function () {
    ...
    pointShape = function pointShape(sid) {
        /* eslint no-unused-vars: off */
        return 'circle';
    ...
    var lineStyle = function lineStyle(sid) {
    ...
    var pointover = function pointover(sid) {
        /* eslint no-unused-vars: off */
        return function (d) {
    ...
    var pointout = function pointout(sid) {
        /* eslint no-unused-vars: off */
        return function (d) {
            d3.select(this).selectAll(pointShape(sid)).attr('fill', plot.cscale()(sid));
        };
    };

    var pointLabel = function pointLabel(sid) {
        var x = plot.xvalue(),
            y = plot.yvalue();
        return function (d) {
    ...
    var drawPointsIf = function drawPointsIf(dataset) {
        var items = plot.items()(dataset);
        return items && items.length <= pointCutoff;
    };

    var drawLinesIf = function drawLinesIf(dataset) {
        var items = plot.items()(dataset);
        return items && items.length > pointCutoff;
    };
        ...
        return function () {
            ...
            plot.datasets()(data).forEach(function (dataset) {
                ...
                var sid = plot.id()(dataset),
                    ...
                    pty1 = yscaled(d1),
                    dist1 = Math.sqrt(Math.pow(ptx1 - mouse[0], 2) + Math.pow(pty1 - mouse[1], 2)),
                    ptx2 = xscale.scale(xvalue(d2)),
                    pty2 = yscaled(d2),
                    dist2 = Math.sqrt(Math.pow(ptx2 - mouse[0], 2) + Math.pow(pty2 - mouse[1], 2)),
                    threshold = 20;
            ...
            hover.enter().append('g').attr('class', 'line-hover').merge(hover).each(function (d) {
                var g = d3.select(this).datum(d.data);

                _selectOrAppend(g, pointShape(d.id)).call(pointStyle(d.id)).attr('transform', translate(d.units));
    ...
    plot.renderBackground(function (dataset) {
        ...
        d3.select(g.node().parentNode.parentNode).selectAll('g.line-hover').remove();
        ...
        if (path.empty()) {
            path = g.append('path').attr('class', 'data').attr('fill', 'transparent');
    ...
    plot.render(function (dataset) {
        ...
        points = g.selectAll('g.data').data(items, plot.itemid()); // Generate elements for new data

        newpoints = points.enter().append('g').attr('class', 'data');
        newpoints.append('title');
        newpoints.append(pointShape(sid)); // Update elements for new or existing data

        newpoints.merge(points).on('mouseover', pointover(sid)).on('mouseout', pointout(sid)).attr('transform', translate(yunits)).select(pointShape(sid)).call(pointStyle(sid));
...
chart.timeSeries = function () {
    ...
    plot.xField('date').xvalue(function (d) {
        var xField = plot.xField();
        return parse(d[xField]);
    }).xscalefn(d3.scaleTime).xnice(d3.timeYear).xunits(function (data) {
        /* eslint no-unused-vars: off */
        return null;
    }).yField('value').yunits(function (d) {
        return d.units;
...
chart.boxplot = function () {
    var plot = chart.base(),
        ...
        wr,
        offsets = {},
        prefix = 'value-'; // Accessors for individual items

    var q1 = function q1(d) {
        return d[prefix + 'q1'];
    };

    var q3 = function q3(d) {
        return d[prefix + 'q3'];
    };

    var med = function med(d) {
        return d[prefix + 'med'];
    };

    var whishi = function whishi(d) {
        return d[prefix + 'whishi'];
    };

    var whislo = function whislo(d) {
        return d[prefix + 'whislo'];
    };

    plot.xscalefn(d3.scalePoint).itemid(function (d) {
        ...
        var boxes = d3.select(this).selectAll('g.data').data(items, plot.itemid());
        boxes.enter().append('g').attr('class', 'data').merge(boxes).attr('transform', translate(yunits)).each(box(sid, yunits));
        boxes.exit().remove();
    });
    ...
    function box(sid, yunits) {
        ...
        return function (d) {
            ...
            var box = _selectOrAppend(d3.select(this), 'g', 'box').attr('transform', _trans(offsets[sid], 0));

            _selectOrAppend(box, 'line', 'q1').attr('x1', -r).attr('x2', r).attr('y1', y(dq1)).attr('y2', y(dq1)).attr('stroke-width', 2).attr('stroke', color);

            _selectOrAppend(box, 'line', 'q3').attr('x1', -r).attr('x2', r).attr('y1', y(dq3)).attr('y2', y(dq3)).attr('stroke-width', 2).attr('stroke', color);

            _selectOrAppend(box, 'line', 'med').attr('x1', -r).attr('x2', r).attr('y1', y(dmed)).attr('y2', y(dmed)).attr('stroke-width', 2).attr('stroke', color);

            _selectOrAppend(box, 'line', 'iqr-left').attr('x1', -r).attr('x2', -r).attr('y1', y(dq1)).attr('y2', y(dq3)).attr('stroke-width', 2).attr('stroke', color);

            _selectOrAppend(box, 'line', 'iqr-right').attr('x1', r).attr('x2', r).attr('y1', y(dq1)).attr('y2', y(dq3)).attr('stroke-width', 2).attr('stroke', color);

            _selectOrAppend(box, 'line', 'w-top').attr('x1', -wr).attr('x2', wr).attr('y1', y(dwhishi)).attr('y2', y(dwhishi)).attr('stroke', color);

            _selectOrAppend(box, 'line', 'w-bottom').attr('x1', -wr).attr('x2', wr).attr('y1', y(dwhislo)).attr('y2', y(dwhislo)).attr('stroke', color);

            _selectOrAppend(box, 'line', 'w-q3').attr('x1', 0).attr('x2', 0).attr('y1', y(dwhishi)).attr('y2', y(dq3)).attr('stroke', color);

            _selectOrAppend(box, 'line', 'w-q1').attr('x1', 0).attr('x2', 0).attr('y1', y(dwhislo)).attr('y2', y(dq1)).attr('stroke', color);
        };
    } // Getters/setters for accessors
