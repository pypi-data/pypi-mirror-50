...
class Git:
    ...
    def checkout(self, branch):
        command = """cd {directory} && 
                     if [[ $(git branch --list {branch}) && $(git rev-parse --abbrev-ref HEAD) -ne {branch} ]]; then git checkout {branch}; fi"""
        subprocess.Popen(command.format(directory=self._unit.codedir, branch=branch), shell=True,
                         executable='/bin/bash')
