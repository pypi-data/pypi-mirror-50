...
class Text2Dataset:
    ...
    def text2List(self, dataset, text_type="clean_text"):
        # Create lists of words and labels with input training dataset
        words_list = dataset[text_type].values.tolist()
        label_list = dataset["Label"].values.tolist()

        return words_list, label_list

    def load_dataset(self, train_data, val_data, text_type="clean_text"):
        """
            Crate n-grams dictionary of the dataset, return
            input and output vectors.
        """
        words_list, label_list = self.text2List(train_data, text_type=text_type)
        words_list_val, label_list_val = self.text2List(val_data, text_type=text_type)
        label_words_dict = {}
...
class FastText:
    ...
    def train(
        ...
        train_X, train_y, val_X, val_y = self.text2Dataset.load_dataset(train_data, val_data, text_type=text_type)
    ...
    def get_accuracy_metrics(self, dataset, text_type="clean_text"):
        preds, real = [], []
        ...
        for row in dataset.itertuples(index=True, name="Pandas"):
            ...
            if self.predict(getattr(row, text_type))[0][0] == "__label__0":
                preds = numpy.append(preds, 0)
            elif self.predict(getattr(row, text_type))[0][0] == "__label__1":
                preds = numpy.append(preds, 1)

        metrics = self.calculate_accuracy_metrics(preds, real)
        return metrics

    def evaluate_performance(self, train_set, dev_set, test_set, acceptance_f1_score, text_type="clean_text"):
        """
        ...
        train_results = self.get_accuracy_metrics(train_set, text_type)
        dev_results = self.get_accuracy_metrics(dev_set, text_type)
        test_results = self.get_accuracy_metrics(test_set, text_type)
