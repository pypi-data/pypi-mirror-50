...
def main():
    ...
    parser.add_argument('--exp-dir', '-d', help='top experiment directory', type=str, default='exps')
    parser.add_argument('--exp-name', '-n', help='experiment name', type=str, default='experiment_mmm')
    parser.add_argument('--create-ignore', '-t', dest='create_ignore', action='store_true',
                        help='if create .dfignore file in the current dir')
    parser.add_argument('--command', '-c', type=str, default=None, nargs='+',
                        help='the python command line, e.g. srun -p ... -- python ...')

    args = parser.parse_args()

    if args.create_ignore:
        with open('.dfignore', 'a+') as f:
