...
def main(BuildModel,csv,entries,modelname,clnum=None,progress_bar=None):
	...
	if BuildModel:
		...
		progress_bar.update()
		IDX,IDX_dist,bdsubtype,C,VamModel,height = cluster_main(csv,modelname,score,pc,bdpc,clnum,pcnum,VamModel,BuildModel,ch,None,entries,datetimenow)
		...
		progress_bar.update()
		pcnum=None
		IDX,IDX_dist,bdsubtype,C,VamModel_ch2,height = cluster_main(csv,modelname,score,pc,bdpc,clnum,pcnum,VamModel_ch2,BuildModel,ch,None,entries,datetimenow)
		...
		for setidx, setpath in enumerate(setpaths):
			...
			IDX_ch1,IDX_dist_ch1,bdsubtype_new,C_new,VamModel,height1 = cluster_main(csv,modelname,score_new,pc_new,bdpc_new,clnum,pcnum,VamModel,BuildModel,ch,condition[setidx],entries,datetimenow)	
			recordIDX(IDX_ch1,IDX_dist_ch1,UI,ch,setpath)
			try:
			...
			pcnum=VamModel['pcnum']
			#pc_new goes in for sake of placing, but pc from the model is used in cluster_main
			IDX_ch2,IDX_dist_ch2,bdsubtype_new,C_new,VamModel,height2 = cluster_main(csv,modelname,score_new,pc_new,bdpc_new,clnum,pcnum,VamModel,BuildModel,ch,condition[setidx],entries,datetimenow)
			recordIDX(IDX_ch2,IDX_dist_ch2,UI,ch,setpath)
