...
def recordIDX(IDX,IDX_dist,UI,cellornuc,setpath):
	print('## recordIDX.py')
	if cellornuc == 'ch1':
	...
	if os.path.exists(os.path.join(setpath,ledgername)): 
		...
		obj_ledger['IDX']=pd.Series(IDX) #write
		obj_ledger['Fitness']=pd.Series(IDX_dist)
	else: 
		d = {'IDX':pd.Series(IDX),'Fitness':pd.Series(IDX_dist)}
