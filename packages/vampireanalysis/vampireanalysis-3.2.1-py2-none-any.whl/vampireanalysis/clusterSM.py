...
def clusterSM(csv,modelname,score,pc,bdpc,clnum=None,pcnum=None,VamModel=None,BuildModel=None,ch=None,condition=None,entries=None,datetimenow='None'):
    ...
    if BuildModel:
        ...
        # print 'look at thissss cmsnnnnn'
        # print cmsn.shape
        # print cmsn
        # print 'this is CCCCCC'
        # print C.shape
        # print C
        # print 'this is D'
        # print D.shape
        # print D
        # print 'this is C plot'
        # figx,axx = plt.subplots()
        # axx.fill(C[0][0:10],C[0][10:],fill=False,edgecolor='r',linewidth=5)
        # cmsn_Norm = preprocessing.normalize(cmsn)
        # axx.fill(cmsn_Norm[0][0:10],cmsn_Norm[0][10:],fill=False,edgecolor='b',linewidth=5)
        # axx.set_title('cmsn norm plot')

        # figx,axx = plt.subplots()
        # axx.fill(C[0][0:10],C[0][10:],fill=False,edgecolor='r',linewidth=5)
        # cmsn_Norm = preprocessing.normalize(cmsn)
        # axx.fill(cmsn_Norm[0][0:10],cmsn_Norm[0][10:],fill=False,edgecolor='b',linewidth=5)
        # axx.set_title('cmsn plot')

        # figxx,axxx=plt.subplots()
        # axxx.fill(C[0][0:10],C[0][10:],fill=False,edgecolor='r',linewidth=5)
        # axxx.set_title('C plot')
        # plt.show()
        # time.sleep(100)
        ##################
    # clnum=max(IDX)+1
    ...
    # dump = True
    # if dump:
    #     IDX[IDX_dist>5]=clnum+1

    ## 5% of high IDX_dist gets IDX assinged to zero
    #dumpamount = 0.05
    dumpamount = entries['Fraction of discarded outliers [%]'].get()
    dumpamount = int(dumpamount)
    ...
    IDX[ind]=clnum+1
    # print('trash bin is'+str(np.max(IDX)))
    ...
    plt.close('all')
    return IDX,IDX_dist,bdsubtype,C,VamModel,height
...
def cluster_main(csv,modelname,score,pc,bdpc,clnum=None,pcnum=None,VamModel=None,BuildModel=None,cellornuc=None,condition=None,entries=None,datetimenow='None'):
    print('## clusterSM.py')
    start = time.time()
    IDX,IDX_dist,bdsubtype,C,VamModel,height=clusterSM(csv,modelname,score,pc,bdpc,clnum,pcnum,VamModel,BuildModel,cellornuc,condition,entries,datetimenow)
    end = time.time()
