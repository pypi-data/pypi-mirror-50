	...
	for field in fields:
		...
		if  field =='Merge CellProfiler Segmented Objects' or field == 'Build Model' or field =='' or field == 'Apply Model': 
			...
			if field == 'Number of shape modes':
				...
				ent.insert(0,'50')
			elif field == 'Fraction of discarded outliers [%]':
				ent.insert(0,'0')
...
def vampire(): 
	...
	fields = ('Merge CellProfiler Segmented Objects','Locate CellProfiler objects','', #merge button
		...
		'Apply Model','Image sets for applying','Model to apply','Fraction of discarded outliers [%]','',
		'Status','')
	ents,rows = makeform(root, fields)
	root.bind('<Return>', (lambda event, e=ents: fetch(e))) 
	#####################################################################################################################
	progress_bar = Progressbar(rows[14], style="red.Horizontal.TProgressbar", orient="horizontal", mode="determinate",maximum=100)
	...
	#function 9
	b9 = Button(rows[13],text='apply model',width=12,command=(lambda e=ents: Model(e,False,progress_bar)))
