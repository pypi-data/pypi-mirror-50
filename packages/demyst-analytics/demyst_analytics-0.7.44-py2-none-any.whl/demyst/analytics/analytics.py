...
class Analytics(object):
    ...
    def enrich(self, providers, inputs=None, hosted_input=None, validate=True, all_updates=False, fixed_list_size=None):
        ...
        return track_function(lambda: self.do_enrich(providers,
                                                     inputs,
                                                     validate=validate,
                                                     all_updates=all_updates,
                                                     fixed_list_size=fixed_list_size),
    ...
    def enrich_download_to_disk(self, id, file_path, overwrite=False, block_until_complete=True):
        ...
        try:
            ...
            pool.join()
            # Merge files
            self.merge_csv_files(exports, file_path)
        finally:
    ...
    def merge_csv_files(self, exports, out_file):
        for export in exports:
    ...
    def do_search(self, inputs=None, tags=None, view="html", strict=False):
        ...
        if (view == "html"):
            return self.render_connectors(connectors, inputs)
        elif (view == "json"):
            return connectors
        else:
            raise RuntimeError("View must be html or json.")

    def render_connectors(self, connectors, inputs):
        doc, tag, text, line = Doc().ttl()
        table = {}
        if connectors == []:
    ...
    def product_inputs(self, provider_names=[], all_products=False, view="dataframe"):
        ...
        if view == "dataframe":
            ...
            connectors = []
            for provider_name in provider_names:
                connector = self.C.lookup_provider(provider_name)
                if connector:
                    connectors.append(self.transform_provider_to_connector(connector))
                else:
                    print("Unknown connector: " + provider_name, file=sys.stderr)
            return self.render_connectors(connectors, [])
        else:
    ...
    def transform_provider_to_connector(self, provider):
    ...
    def sample_data(self, provider_name):
        provider = self.C.lookup_provider(provider_name)
        if not provider:
        ...
        id = str(provider["id"])
        resp = self.C.auth_post(self.C.get("MANTA_URL") + "table_providers/"+id+"/download_sample_data")
        return pd.read_csv(resp.json()["url"], na_filter=False)
