...
def embright_categorization(inFile, outFile, mNs_mass=3.0, max_mass=2.834648092299807):
	'''
	...
	[Compactness_inj, _] = computeDiskMass.computeCompactness(m2_inj, max_mass,
								  fromFile=True)
	[Compactness_rec, _] = computeDiskMass.computeCompactness(m2_rec, max_mass,
								  fromFile=True)
	...
	disk_mass_inj = computeDiskMass.computeDiskMass(m1_inj, m2_inj,
							chi1_inj, chi2_inj)
	EMB_classified = disk_mass_inj > 0.0
	EMB_classified = EMB_classified.astype(int)

	output = np.vstack((df.id.values, m1_inj, m2_inj, chi1_inj, chi2_inj,
						mc_inj, q_inj, R_isco_hat_inj, Compactness_inj,
						m1_rec, m2_rec, chi1_rec, chi2_rec,
						mc_rec, q_rec, R_isco_hat_rec, Compactness_rec,
						df.cfar.values, df.snr.values, NS_classified,
						EMB_classified)).T

	df_complete = pd.DataFrame(output, columns=['ID', 'm1_inj', 'm2_inj',
						    'chi1_inj', 'chi2_inj',
                                            	    'mc_inj', 'q_inj',
						    'R_isco_inj', 'Compactness_inj',
						    'm1_rec', 'm2_rec',
						    'chi1_rec', 'chi2_rec',
						    'mc_rec', 'q_rec', 'R_isco_rec',
						    'Compactness_rec', 'cfar',
						    'snr', 'NS', 'EMB'])

	try:
		File = open(outFile, 'wb')
		pickle.dump(df_complete, File)
	except:
		print('Could not dump data in pickle format')
		sys.exit(1)
