...
minQualRMS = args["minQuality"]
minQualGenotype = args["minQuality"]
...
print("Step 1:\tCreate tables from VCF files if stills necessary")
...
print("Step 2:\tParsing all tables files and extracting the unique variants.")
...
for path in tablesPaths:
    with open(path, "r") as table:
        ...
        pbar1.set_description(desc=path.split("/")[-1])
        ...
        for line in table:
            ...
            varDict.update({(chrom, pos, ref, alt): (cov, qual, sor, count, gq, path)})
        pbar1.update()

print(str(len(varSet)) + " unique variants found.")
print("Step 2:\tFiltering those variants and creating BAM files.")
...
def func(varList):
    ...
    if minCov <= float(cov) and float(cov) <= maxCov and float(qual) >= minQualRMS and float(sor) <= sorLimit and float(count) >= minCount and float(gq) >= minQualGenotype:
        ...
        subprocess.call(["samtools", "view", "-b", "-o", newName, bamfile, region])
