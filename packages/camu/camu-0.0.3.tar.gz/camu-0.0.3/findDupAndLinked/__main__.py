...
#control = "/media/nicolas/Intenso/R1F1-RefPool.sort.rmd.realn.recal.bam"
control = args["control"]

# ToDo:
#  look at preprocessing
#  add if file could not be read in log file - think about how to add...

#base = "/home/nicolas/Documents/bamFiles/R1F-Bam/"
base=args["directory"]
fileList = glob.glob(base+"*.bam")

if not args["output"]:
	#output = base + fileList[0].split("/")[-1].split(".")[0][:-1]+".ScoreTable.tsv"
        ...
        for entry in cigarList[:-1]:
            ...
            # Insertions are called for current AND next position
                ...
                for i in range(int(entry[:-1])):
                    ...
                    self.insertions.add(refCounter)
                    seqCounter +=1
...
def buildTable(bamfileName, currFile):
    ...
    refRange = bamfileName.split("_")[-1]
    refRange = refRange.split(".")[0]
    refRange = refRange.split(":")[1]
    currFile.refStart = int(refRange.split("-")[0])
    currFile.refEnd = int(refRange.split("-")[1])
    ...
    scaff = name.split(":")[0]
    scaff = scaff.split(".", 1)[1]

    proc = subprocess.run(
        ["samtools", "mpileup", "-A", control, "-r", scaff + ":" + str(leftmostPos-1) + "-" + str(rightmostPos+1)],
    ...
    if len(newMatrixSNP) <= 1:
        currFile.indMaxCorrDictSNP = {149: 0}
    else:
    ...
    if len(newMatrixIns) <= 1:
        currFile.indMaxCorrDictIns = {149: 0}
    else:
    ...
    if len(newMatrixDel) <= 1:
        currFile.indMaxCorrDictDel = {149: 0}
    else:
...
def appendResultTable(bamfileName, resultTable,
    ...
    # get base ratios
    if label == "Mismatch":
        nm_len = len(currFile.NMs)
        baseFreqTemp = [["G", currFile.NMs.count("G")/nm_len],
                    ["A", currFile.NMs.count("A")/nm_len],
                    ["T", currFile.NMs.count("T")/nm_len],
                    ["C", currFile.NMs.count("C")/nm_len]]
        baseFreq = list()
    ...
    score = np.log10(len(uniqueStartPos)) * pow(1 - indMaxCorrDict[refLength//2-1], 0.5)
...
def func(fileList):#, results, index):
    ...
    for bamfileName in fileList:
        ...
        try:
            posRange = bamfileName.split(".")
            posRange = posRange[2].split("_")
            posRange = posRange[1].split("-")
            endPos = int(posRange[1])
            posRange = posRange[0].split(":")
            startPos = int(posRange[1])
        except IndexError:
            print(bamfileName, ":\tFilename not valid")
            continue
        ...
        try:
            ...
            print(bamfileName, ":\tError when reading file")
            continue
        ...
        try:
            ...
            print(bamfileName, ":\tInvalid CIGAR string detected")
            continue
        ...
        if not validMutTypes:
            with open(base + "log.txt", "a") as log:
...
# clear log
with open(base + "log.txt", "w") as log:
