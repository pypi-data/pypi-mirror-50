...
class LabelerWindow(QMainWindow, Ui_MainWindow):
    ...
    def exportToCSV(self):
        if self.project is None: # inconsistency - button should not be enabled
            self.exportCSV.setEnabled(False)
            self.errorMessage(u'Error exporting',
            u"<p>Sorry, could not export to CSV. Did you open a project already?" )
        else:
            mes = self.project.exportCSV()
            self.status(mes)
