...
class GeoFinder:
    ...
    def __init__(self):
        ...
        # Set up configuration settings class
        if self.cfg is None:
            self.cfg = Config.Config()

        # Ensure GeoFinder directory structure is in place
        valid_directory = self.cfg.valid_directories()
        self.w.config_button.config(state="normal")

        if valid_directory:
            # See if  required Geonames files are present
            err = self.check_configuration()
            if err:
                # Missing files
                self.logger.warning(f'Missing files')
                self.w.status.set_text("Click Config to set up Geo Finder")
                self.set_preferred_init_button(Btn.CONFIG)
                ...
                if os.path.exists(self.cfg.get("gedcom_path")):
                    ...
                    self.set_preferred_init_button(Btn.LOAD)
                else:
                    # prompt user to select a GEDCOM file - GEDCOM file name isn't valid
                    self.w.load_button.config(state="disabled")
                    self.w.status.set_text("Choose a GEDCOM file")
                    self.set_preferred_init_button(Btn.CHOOSE)
        else:
    ...
    def display_result(self, place):
        ...
        if place.result_type in GeoKeys.successful_match:
            if place.place_type == Loc.PlaceType.ADMIN1 or place.place_type == Loc.PlaceType.ADMIN2:
    ...
    def set_preferred_review_button(self, btn):
        if btn == Btn.VERIFY:
    ...
    def set_preferred_init_button(self, btn):
        # Set all to normal
        self.w.change_button.configure(style="TButton")
        self.w.load_button.configure(style="TButton")
        self.w.config_button.configure(style="TButton")

        # Highlight preferred button and ensure it is enabled
        if btn == Btn.CHOOSE:
            self.w.change_button.configure(style="Preferred.TButton")  # Make the Save button the preferred selection
            self.w.change_button.config(state="normal")
        elif btn == Btn.LOAD:
            self.w.load_button.configure(style="Preferred.TButton")  # Make the Save button the preferred selection
            self.w.load_button.config(state="normal")
        elif btn == Btn.CONFIG:
            self.w.config_button.configure(style="Preferred.TButton")  # Make the Save button the preferred selection
            self.w.config_button.config(state="normal")
