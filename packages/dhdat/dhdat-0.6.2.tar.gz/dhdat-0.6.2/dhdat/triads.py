...
class Triads():
  ...
  def count(self, matrix, interaction):       
    ...
    self.d_triadCount.loc[interaction, :] = 0 #set counts to zero

    for index, triadNodes in self.d_combinations.iterrows():   
      triadEdges = [0, 0, 0]
                              #codes relation of each edge in a triad:  
      ...
      j = [1,2,2]
      for idx in range(3):
        up = matrix.loc[triadNodes[i[idx]], triadNodes[j[idx]]]
        down = matrix.loc[triadNodes[j[idx]], triadNodes[i[idx]]]
        triadEdges[idx] = (
              1 * (up > down) +
              2 * (up == down) +
              1 * (up == 0) * (down == 0)
