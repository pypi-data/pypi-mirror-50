...
class FunctionTemplate:
    ...
    def generate_function(self):
        ...
        if self.configuration.run.output or self.configuration.run.input:
        ...
        handler_path = self.temp_dir.copy_file(
            self.configuration.run.script, dest="handler.py", directory="function"
        )
        ...
        if self.configuration.build.copy_files is not None:
            for _copy in self.configuration.build.copy_files:
                if not is_url_resource(_copy.src):
                    self.temp_dir.copy_file(_copy.src, directory="function")
    ...
    def dump_environment(self, indent=4) -> str:
