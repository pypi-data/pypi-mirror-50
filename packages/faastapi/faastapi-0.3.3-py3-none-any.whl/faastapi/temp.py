...
class TempDirectory:
    ...
    def copy_file(
        self, src: str, dest: Optional[str] = None, directory: Optional[str] = None
    ) -> str:
        """
        Copy a given file from directory to temp directory
        """
        if dest is None:
            dest = os.path.basename(src)
        if directory is not None:
            _dest = os.path.join(directory, dest)
            dest = self.get_abs_path(_dest)
