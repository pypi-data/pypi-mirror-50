...
class HDFFileStorage:

    def __init__(self, filename='speedtest.hdf', key='speedtest'):
        self.filename = filename
        self.key = key

    def read_speeds(self):
        df = pd.read_hdf(self.filename, key=self.key)
        for _, speed in df.iterrows():
            yield speed

    def store_speed(self, speed=None):
        if speed is None:
            return None
        new = pd.DataFrame([speed]).astype({'timestamp': 'datetime64'})
        if os.path.exists(self.filename):
            old = pd.read_hdf(self.filename)
            combined = pd.concat(
                (old, new),
                axis=0,
                ignore_index=True,
            ...
            combined = new
        combined = combined.drop_duplicates().drop(
            columns=['share'],
            errors='ignore'
        )
        combined.to_hdf(self.filename, key=self.key)
    ...
    def encode_speed(self, speed=None):
        ...
        return pd.Series(speed).to_json()
...
class HDFSpeedMonitor(SpeedMonitor, HDFFileStorage):
    pass
...
# scripting layer
##################################################
def run_local_monitor(location, interval_seconds=60,
...
def run_http_monitor(location, interval_seconds=60,
                     filename='speedtest.json'):
