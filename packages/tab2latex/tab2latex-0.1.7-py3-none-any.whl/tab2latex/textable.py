...
def latex_tab(data=[[1,2,3],[42,42,42]],names=["col1","col2"],filename="test.tex",caption="Caption",label="test", dec_points=[0,2], format_string=None, linewidth=1.00, tabcolsep=1.00,):
    try: #test, if names and data and dec_points array have different lenghts
        __test_error(data,dec_points,names)
    except TypeError: raise
    else:
        ...
        texfile.write("\\sisetup{detect-all, parse-numbers=false}\n")#setup columnwidth to fit best
        texfile.write(r"\resizebox{"+r"{}".format(linewidth)+r"\linewidth}{!}{"+"\n")
        texfile.write(r"\setlength{\tabcolsep}{"+r'{}'.format(tabcolsep)+r"\tabcolsep}"+"\n")
