...
SCRAPE_URL = 'https://www.instadp.com/profile/{}'

class Profile:

	def __init__(self, username, reps=2):
	...
	def get_avatar(self):
		data = requests.get(SCRAPE_URL.format(self.username))
	...
	def get_stories_scraping(self):
		if not self.is_private:
			...
			data = soup(data.text, 'html.parser')
			data = data.find('ul', {'class':'stories-list'}).find_all('li', {'class':'story'})
			return data

		return False

	def get_stories(self, start=0, stop=101):
		data = self.get_stories_scraping()[::-1]
		if data:
			result = []
