...
class Profile:
	...
	def __str__(self):
		...
		if self.is_private:
			...
			data += "{}\n".format(len(self.get_stories_scraping()))
		if self.biography:
	...
	def get_stories_scraping(self):
		if not self.is_private:
			...
			for i in range(2):
				page = requests.get('https://www.instadp.com/stories/'+self.username)

			page = soup(page.text, 'html.parser')
		...
		if data:
			...
			for item in data:
				...
				if not content:
					...
					result += [[content1, content]]
					continue
				result += [[content.get('src')]]
