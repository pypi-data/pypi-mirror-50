...
class KaggleApi(KaggleApi):
    ...
    def dataset_list(self,
        ...
        if size:
            raise ValueError('The --size parameter has been deprecated. ' +
    ...
    def dataset_list_cli(self,
        ...
        datasets = self.dataset_list(sort_by, size, file_type, license_name,
                                     tag_ids, search, user, mine, page, max_size, min_size)
        fields = ['ref', 'title', 'size', 'lastUpdated', 'downloadCount', 'voteCount', 'usabilityRating']
        if datasets:
            if csv_display:
    ...
    def dataset_metadata_update(self, dataset, path):
        ...
        with open(meta_file, 'r') as f:
            ...
            updateSettingsRequest = DatasetUpdateSettingsRequest(
                ...
                collaborators = [Collaborator(username=c['username'], role=c['role']) for c in metadata['collaborators']],
                data = metadata['data']
            )
            result = self.process_response(self.metadata_post_with_http_info(owner_slug, dataset_slug, updateSettingsRequest))
    ...
    def dataset_metadata(self, dataset, path):
        ...
        result = self.process_response(self.metadata_get_with_http_info(owner_slug, dataset_slug))
