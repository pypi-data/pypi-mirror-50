...
def est_t_gg_cdc(quyu,conp_pg,conp_hawq,pxf_ip):
    ...
    LOCATION ('pxf://192.168.4.183:51200/hunan_huaihua_gcjs.t_gg_hawq?PROFILE=JDBC&JDBC_DRIVER=org.postgresql.Driver&DB_URL=jdbc:postgresql://192.168.4.199:5432/hunan&USER=postgres&PASS=since2015&PARTITION_BY=rowid:int&RANGE=1:1000000&INTERVAL=1000')
...
def out_t_gg_all_zlsys(quyu,conp):
    ...
    sql="""
    select row_number() over() as rowid
    ,guid    ,gg_name ,coalesce(href ,'暂无') as href   ,fabu_time   ,ggtype  ,quyu    ,diqu    ,jytype  ,create_time ,info   , page
