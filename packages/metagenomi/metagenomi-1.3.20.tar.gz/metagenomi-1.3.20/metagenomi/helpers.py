...
def cat_tsvs(files, outfile, headers=True):
    if headers is False:
        return cat(files, outfile)
    else:
        with open(outfile, 'w') as f:
            cmd = f'head -1 {files[0]}'
            subprocess.check_call(shlex.split(cmd), stdout=f)
        with open(outfile, 'a') as f:
            cmd = f'tail -n +2 -q  {" ".join(files)}'
            subprocess.check_call(shlex.split(cmd), stdout=f)
        return outfile
...
def copy_s3file(oldfile, newfile):
    ...
    S3resource.meta.client.copy(copy_source, newbucket, newkey)
    if check_s3file(f's3://{newbucket}/{newkey}'):
        return newfile
    else:
        raise ValueError('Copy failed')

    # OLD COPY FUNCTION (for record)
    # S3resource.Object(newbucket, newkey).copy_from(CopySource=f'{oldbucket}/{oldkey}')
    # if check_s3file(f's3://{newbucket}/{newkey}'):
    #     return newfile
    # else:
...
def download_file(s3file, dir_to_dl, dry_run=False, overwrite=True, unzip=False, verbose=False):
    '''
    Downloads a folder from s3
    :param s3file: s3 file path
    :param dir_to_dl: local path of dir to download to
    :return: local file path of the object downloaded

    If file on s3 is gzipped, will download that version of the file
    '''
    if not check_s3file(s3file):
        if check_s3file(s3file+'.gz'):
            s3file = s3file+'.gz'
            unzip = True
        else:
            raise ValueError(f'Invalid s3_path: {s3file}')
