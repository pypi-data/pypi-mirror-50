...
class Node2Vec():
    ...
    def __init__(self, network, workers=3, output_path='./tmp/'):
        self.network = network
        self.workers = workers
        self.output_path = output_path


    def random_walking(self, number_of_walks, length_of_walks):
        ...
        p = self.output_path +'random_walk/convert.py'
        ...
        p = self.output_path + 'random_walk/randomwalk'
    ...
    def generate_embeddings(self, embedding_dimension, cbow=False, **kwargs):
        ...
        dict_node = invert_dict(node_dict)
        nx.set_node_attributes(self.network, {dict_node[i]: {'embedding': self.embeddings.wv[str(i)]} for i in self.network_int.nodes()})

        # Return the embeddings in a pandas DataFrame as well
        dt.from_dict({dict_node[i]: self.embeddings.wv[str(i)] for i in self.network_int.nodes()}, orient='index')
