...
class Google:
    ...
    def SearchLink(self, q):
        ...
        anchors = self.anchors(soup)

        for a in anchors:
            href = a.get("href")
            link = self.filter_url(href)
            if link:
                return link

    def anchors(self, soup):
        try:
...
class StackOverFlow:
    ...
    def code(self, url):
        ...
        tags = self._getTags(soup)
        code = self._getCode(soup)
        code = formatter(tags, code).splitlines()
        if len(code[0]) < 20:
