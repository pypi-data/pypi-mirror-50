...
def get_kdata(target_list: 'list',
    ...
    for idx, data in enumerate(data_list):
        results[idx] = history_cvt.convert_k_data_to_df(data, target_list[idx], cnt.env.get_target_type(todotmc(new_targets[idx])))
    if df:
...
def get_kdata_n(target_list: 'list',
    ...
    for idx, target in enumerate(new_targets):
        ...
        results[idx] = history_cvt.convert_k_data_n_to_df(data[0], target_list[idx], cnt.env.get_target_type(todotmc(target)))
    if df:
        data_df = pd.concat(results, ignore_index=True)  # type: pd.DataFrame
        if sort_by_date:
            data_df = data_df.sort_values('time', ascending=True, na_position='first')  # type: pd.DataFrame
