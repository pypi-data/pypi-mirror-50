...
class URshInitDMA(ctypes.Structure):
    _pack_ = 1
    _fields_ = [
        ("type", ctypes.c_uint32),# код типа данных ( rshInitDMA )
        ("startType", ctypes.c_uint32),# настройки типа старта платы
        ("bufferSize", ctypes.c_uint32),# размер буфера в отсчётах (значение пересчитывается при инициализации в зависимости от сопутствующих настроек)
        ("frequency", ctypes.c_double),# частота дискретизации
        ("dmaMode", ctypes.c_uint32),# режим работы DMA
        ("control", ctypes.c_uint32),# специфические настройки для данного типа плат (например, диф режим)
        ("frequencyFrame", ctypes.c_double),# частота дискретизации    внутри пачки
        ("channels", URshChannel * RSH_MAX_LIST_SIZE),# параметры каналов
        ("threshold", ctypes.c_double),# уровень синхронизации в Вольтах
        ("controlSynchro", ctypes.c_uint32),# специфические настройки синхронизации
        ]
...
class RudshelLeonardo2ADC(VisaInstrument):
    ...
    def get_error(self, code):
        errorDesc = ctypes.create_string_buffer(1024)
        self.driver_get_error(ctypes.c_uint32(code), errorDesc, ctypes.c_uint32(1024), RSH_LANGUAGE_ENGLISH)
        return repr(errorDesc.value)
    ...
    def __init__(self, rm, resource):
        ...
        st = self.get_device_handle(b'LEONARDO2PCI', ctypes.byref(self.deviceHandle))
        if st != RSH_API_SUCCESS:
            raise Exception('UniDriverGetDeviceHandle returned' + self.get_error(st))

        # Подключаемся к устройству.
        st = self.driver_connect(self.deviceHandle, ctypes.c_uint32(1), RSH_CONNECT_MODE_BASE)
        if st != RSH_API_SUCCESS:
            raise Exception('Error connecting to device ' + self.get_error(st))

        self.init_sweeping()
        self._allocate_buffer()
        self.options = [ValueDeviceOption('Buffer size', 'samples',
                self.get_buffer_size, self.set_buffer_size)]
