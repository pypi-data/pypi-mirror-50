...
class S3Manager(object):
  ...
  def upload_img_file(self, filepath, bucket_name, file_key):
    ...
    upload_args = {}
    upload_args["ContentType"] = tools.get_image_media_type(img_ext)
    self.client.upload_file(
        filepath, bucket_name, file_key, ExtraArgs=upload_args)
    return self.generate_object_url(bucket_name, file_key, False, 3600)
  ...
  def upload_img_bin(self,
    ...
    upload_args = {}
    upload_args["ContentType"] = tools.get_image_media_type(img_format)
    if make_public:
    ...
    if make_public:
      ...
      self.generate_object_url(bucket_name, img_key_name)
