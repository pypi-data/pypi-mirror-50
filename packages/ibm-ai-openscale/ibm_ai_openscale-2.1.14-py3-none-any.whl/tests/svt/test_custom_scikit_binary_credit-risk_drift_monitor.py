...
class TestAIOpenScaleClient(unittest.TestCase):
    ...
    def generate_drift_model(self):
        from ibm_ai_openscale.utils.drift_detection import DriftDetection
        ...
        drift_detection_model = DriftDetection(self.data_df, drift_detection_input)
        drift_detection_model.generate_drift_detection_model(self.score_4_drift, batch_size=self.data_df.shape[0])
        print('filepath', self.drift_model_path, self.drift_model_name)

        DriftDetection.create_model_tar(drift_detection_model, path_prefix=self.drift_model_path+'/', file_name=self.drift_model_name)
    ...
    def test_40_unsubscribe(self):
        self.ai_client.data_mart.subscriptions.delete(self.subscription.uid)

    def test_41_unbind(self):
