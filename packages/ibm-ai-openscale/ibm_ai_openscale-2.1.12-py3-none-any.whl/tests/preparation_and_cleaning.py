...
def get_performance_timeout():
    if "E2E_SANITY" in get_env():
        json.loads(config.get(suite_config, 'performance_timeout')) * 3
...
def get_metrics_types(ai_client):
    requests_session = requests.Session()
    url = ai_client._href_definitions.get_monitor_definitions_href()
    ...
    response = handle_response(200, u'getting monitor definitions', response, json_response=False)
    metrics_types = {}
    json_data = json.loads(response)
    for row in json_data['monitor_definitions']:
        for metric in row['entity']['metrics']:
            metrics_types[metric['id']] = metric['thresholds'][0]['type']
