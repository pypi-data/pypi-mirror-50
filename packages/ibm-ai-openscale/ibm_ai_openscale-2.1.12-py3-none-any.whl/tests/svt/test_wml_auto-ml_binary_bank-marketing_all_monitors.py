...
class TestAIOpenScaleClient(unittest.TestCase):
    ...
    def test_11_score(self):
        ...
        iam_token = get_iam_token(env="YP", apikey=self.wml_credentials['apikey'])
        ml_instance_id = self.wml_credentials['instance_id']

        scoring_endpoint = 'https://us-south.ml.cloud.ibm.com/v4/deployments/02d32d2d-970b-4a43-a1b1-48e4d90dbed4/predictions'
        ...
        for i in range(0, int(self.scoring_records)):
            response_scoring = requests.post(
                ...
                headers=header)
            self.assertIsNotNone(response_scoring)
            records_list.append(
                PayloadRecord(request=payload_scoring['input_data'][0], response=response_scoring.json()['predictions'][0],
                              response_time=response_time))

        print("response json", response_scoring.json())
        # Log payload with Payload API (necessary for test on DEV_ME):
