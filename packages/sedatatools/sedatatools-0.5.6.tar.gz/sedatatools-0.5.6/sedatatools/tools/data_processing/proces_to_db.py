...
def create_db(config):
    ...
    try:
        query = r'CREATE DATABASE {} ON ( NAME = Sales_dat, ' \
                r'FILENAME="X:\MSSQL_Data\{}.mdf") ' \
                r'LOG ON ( NAME = Sales_log, ' \
                r'FILENAME="X:\MSSQL_Data\{}_log.mdf");' \
            .format(config['dbName'], config['dbName'], config['dbName'])
...
def parse_args():
    return 'config.yml'  # TODO change this to be parametrized from CLI
...
def verify_config_file(config_file_path):
    ...
    if not (os.path.isfile(config['variableDescriptionLocation']) or os.path.isdir(config['variableDescriptionLocation'])):
...
def get_file_names_list(config):
    ...
    if not os.path.isfile(config['fileNamesList']):
        logging.warning(
        ...
        files_list = [os.path.join(config['sourceDirectory'], file_name)
        ...
        with open(config['fileNamesList'], 'r') as fn:
            file_names_list = fn.readlines()
            file_names_list = [line.rstrip('\n') for line in file_names_list]

        files_list = [os.path.join(config['sourceDirectory'], file_name)
                      for file_name in file_names_list if file_name.endswith('csv')]
...
def create_system_columns_names(table, all_geotypes_and_sumlev, sl_levels):
    ...
    for level in sl_levels:
        try:
            ...
            table.rename(
                index=str, columns={
                    'NAME_y': level + '_NAME', 'NAME_x': 'NAME', 'Geo_x': 'Geo',
                },
                inplace=True,
            )
        except KeyError:
...
def check_last_table_name_if_exists(engine, config):
    ...
    for table in db_table_names:
        ...
        if table_suffix != "geo" and table != "table_names":
            unique_tables.append(table_suffix)

    if len(unique_tables) > 0:

        unique_tables = list(set(unique_tables)).sort()
        last_table_suffix = str(unique_tables[-1])
...
def merge_geo_data_tables(file_path, config_files):
    ...
    all_geo_types_and_sumlev = all_geo_types_and_sumlev.rename(columns={
                                                               "FIPS": "Geo",
    })

    if "SUMLEV" in original_table.columns:
        geotypes_org_tables_merged = pd.merge(
            all_geo_types_and_sumlev, original_table, how='left', on=['SUMLEV', 'Geo'],
        )
    else:
        geotypes_org_tables_merged = pd.merge(
            all_geo_types_and_sumlev, original_table, how='left', on='Geo',
        )
...
def create_table(file_path, config_files, file_counter, engine):
    ...
    # ToDo: For table size larger than defined, split columns proprerly - at this time this function doesn't split tables properly...
...
def to_sql_using_threads(df, table_name, config):
    ...
    if len(df) > chunksize:
        ...
        if len(df) > chunksize:
            ...
            conn1.close()
        for i in range(int(np.ceil((len(df) - initial_chunk) / chunksize))):
            if i == int(np.ceil((len(df) - initial_chunk) / chunksize)):
                df2 = df.iloc[initial_chunk + (i + 1) * chunksize:, :]
            else:
                df2 = df.iloc[
                    initial_chunk + i *
                    chunksize:initial_chunk + (i + 1) * chunksize, :
                ]
            conn2 = engine.connect()
            t = threading.Thread(df2.to_sql(
                table_name,
                conn2,
                if_exists='append',
                index=False,
...
def get_primary_keys_dict(config):
    ...
    for geo_info in reversed(geo_level_info):
        ...
        if len(lower_levels) == 0:
            primary_keys[geo_info[0]] = geo_info[0] + '_FIPS'
        else:
            primary_keys[geo_info[0]] = geo_info[0] + '_FIPS' + \
...
def set_primary_keys(config):
    engine = create_engine_for_db(config['dbName'])
    connection = engine.connect()
    primary_keys = get_primary_keys_dict(config)

    db_table_names = engine.table_names()
    for table_name in db_table_names:
        if "geo" not in table_name and "table_names" not in table_name:
            split_values = primary_keys[table_name[-9:-4]].split(',')
            for x in split_values:
                try:
                    connection.execute(
                        "ALTER TABLE " + table_name + " ALTER COLUMN " +
                        x + " varchar(max) NOT NULL;",
                    )
                    connection.execute(
                    ...
                    connection.execute(
                        ...
                        " ADD PRIMARY KEY (" + ",".join(split_values) + ");",
                    )
