...
def create_db(config):
    ...
    try:
        ...
        query = query.format(config['dbname'], config['dbname'], config['dbname'])
...
def create_engine_for_db(db_name):
   ...
   engine = create_engine(f'mssql+pyodbc://{user_name}:{password}@PRIME/{db_name}?driver=ODBC+Driver+11+for+SQL+Server',connect_args={'autocommit': True})
...
def verify_config_file(config_file_path):
    ...
    if not os.path.isfile(config_file_path):
        logging.warning("Error: Config file doesn't exist on selected location: " + config_file_path + " !")
        return False
...
def get_file_names_list(config):
    ...
    if os.path.basename(config['fileNamesList']) == '':
        logging.warning('There is no files list defined, reading files from Source directory')

        if not os.path.isdir(config['sourceDirectory']):
            logging.warning("Error: Source file doesn't exist on selected location: " + config['sourceDirectory'] + " !")
            return False
        ...
        for item in items:
            if item.endswith('csv'):
                files_list.append(os.path.join(config['sourceDirectory'], item))
...
def capitalize_level_names(geo_level):
    ...
    for gl in geo_level:
        ...
        if ' ' in gl:
            ...
            geo_names_cap.append(gl[0].upper() + ''.join([gl[i + 1] for i in blank_pos]).upper())
        else:
            ...
            if sl_level == geo_level[0]:
                ...
                for level_index in reversed(range(temp_ind+1)):
                    if level_index+1 <= int(geo_level_info[level_index][4]):
                        ...
                        table[geo_level_info[level_index][0] + '_NAME'] = 'United States'
                        table[geo_level_info[level_index][0] + '_FULL_FIPS'] = '00'
                        table[names[level_index]] = '00'
                    else:
                        if cut_cum == 0:
                            table[geo_level_info[level_index][0] + '_FIPS'] = table['Geo'].str[-int(geo_level_info[level_index][3]):]
                            table[names[level_index]] = table['Geo'].str[-int(geo_level_info[level_index][3]):]
                            cut_cum = int(geo_level_info[level_index][3])
                            table[geo_level_info[level_index][0] + '_FULL_FIPS'] = table['Geo'].str[-int(geo_level_info[level_index][2]):]
                        else:
                            table[geo_level_info[level_index][0] + '_FIPS'] = table['Geo'].str[-int(geo_level_info[level_index][3]) - cut_cum:-cut_cum]
                            table[names[level_index]] = table['Geo'].str[-int(geo_level_info[level_index][3]) - cut_cum:-cut_cum]
                            table[geo_level_info[level_index][0] + '_FULL_FIPS'] = table['Geo'].str[:-cut_cum]
                            cut_cum += int(geo_level_info[level_index][3])
    return table
...
def create_system_columns_names(table, all_geotypes_and_sumlev, sl_levels):
    ...
    for level in sl_levels:
        try:
            table = pd.merge(table, all_geotypes_and_sumlev[all_geotypes_and_sumlev['SUMLEV'] == level]
                             .drop(['TYPE', 'SUMLEV'], axis=1),
                             how='left',
                             left_on=[level + '_FULL_FIPS'],
                             right_on=['Geo'])
            table.drop(level + '_NAME', axis=1, inplace=True)
            table.rename(index=str, columns={'NAME_y': level+'_NAME', 'NAME_x': 'NAME', 'Geo_x': 'Geo'},
                         inplace=True)
        except KeyError:
            continue

    cols_to_join = [tn for tn in table.columns if '_NAME' in tn and tn != 'SL010_NAME']
...
def write_table_names(table_info, config, engine):
    ...
    for name in table_info:
        ...
        for cn in unique_names:
            table_names_content.append([os.path.basename(name[0]), cn, config['projectYear']])
        table_names = pd.DataFrame(table_names_content, columns=['descName', 'code_Name', 'projectYear'])
    try:
...
def create_table_id(sumlev, suffix_width, sub_counter, table_counter):
    ...
    if sub_counter == 0:
        additional_nulls = ''.join(['0' for _ in range(suffix_width - len(str(table_counter)))])
        return sumlev + '_' + additional_nulls + str(table_counter)
    else:
        additional_nulls_sub = ''.join(['0' for _ in range(suffix_width - len(str(sub_counter)))])
        additional_nulls = ''.join(['0' for _ in range(suffix_width - len(str(table_counter)))])
...
def merge_geo_data_tables(file_path, config_files):
    ...
    all_geotypes_and_sumlev = pd.read_csv(os.path.join(config_files['configDirectory'], 'all_geotypes_and_sumlev.csv'), dtype={'FIPS': 'str'})
    all_geotypes_and_sumlev = all_geotypes_and_sumlev.rename(columns={"FIPS": "Geo"})

    if "SUMLEV" in original_table.columns:
        geotypes_org_tables_merged = pd.merge(all_geotypes_and_sumlev, original_table, how='left', on=['SUMLEV', 'Geo'])
    else:
        geotypes_org_tables_merged = pd.merge(all_geotypes_and_sumlev, original_table, how='left', on='Geo')
...
def create_table(file_path, config_files, file_counter, engine):
    ...
    all_geotypes_and_sumlev, geotypes_org_tables_merged = merge_geo_data_tables(file_path, config_files)
    ...
    for sumlev in sumlev_ids:
        ...
        system_columns = ['Geo', 'SUMLEV', 'TYPE', 'NAME', 'QName'] + [sl + '_FIPS' for sl in sumlev_ids] + [sl + '_NAME' for sl in sumlev_ids] + geo_names_cap
        column_prefix = config_files['projectId'] + '_' + str(table_counter).zfill(3) + '_' if config_files['projectId'] != '' else ''
        data_columns = [column_prefix+ c for c in loaded_table.columns if c not in system_columns]
        ...
        if len(data_columns) > config_files['maxTableWidth']:
            ...
            for ind, column in enumerate(data_columns, 1):
                if (len(data_columns)-ind) < config_files['maxTableWidth']:
                    if (len(splitted_columns) < config_files['maxTableWidth']) & (len(data_columns)-ind == 0):
                        ...
                        table_holder[create_table_id(sumlev, 3, sub_table_counter, table_counter)] = loaded_table[system_columns + splitted_columns]
                        break
            ...
            table_holder[create_table_id(sumlev, 3, 0, table_counter)] = loaded_table
    ...
    for table_id, table in table_holder.items():
        try:
            table.columns = [column_prefix+c if c not in system_columns else c for c in table.columns]
            table_holder[table_id] = create_system_columns_fips(table_id, table, config_files, geo_level_names)
            table_holder[table_id] = create_system_columns_names(table, all_geotypes_and_sumlev, table_holder.keys())
            table_holder[table_id].drop(list(table_holder[table_id].filter(regex='_FULL_FIPS')), axis=1, inplace=True)
            ...
            prefix_to_table = config_files['projectId'] + '_' if config_files['projectId'] != '' else ''
            table_holder[table_id].to_sql(prefix_to_table + table_id, connection, index=False)
...
def process_data(configuration):
    ...
    for file_path in files_list:
        table_names_info.append(create_table(file_path, configuration, file_counter, engine))
        file_counter += 1
...
def get_primary_keys_dict(config):
    ...
    for geo_info in reversed(geo_level_info):
        ...
        if len(lower_levels) == 0:
            ...
            primarykeys[geo_info[0]] = geo_info[0] + '_FIPS'+','+",".join(str(x)+ '_FIPS' for x in lower_levels)
    ...
    for table_name in db_table_names:
        if "geo" not in table_name and "table_names" not in table_name:
            ...
            for x in split_values:
                try:
                    connection.execute("ALTER TABLE "+table_name+" ALTER COLUMN "+x+" varchar(max) NOT NULL;")
                    connection.execute("ALTER TABLE "+table_name+" ALTER COLUMN "+x+" int NOT NULL;")
                    connection.execute(
                        "ALTER TABLE "+table_name+" ADD PRIMARY KEY ("+",".join(split_values)+");")
                except:
                    logging.warning("Could not execute queries for changing column type and primary key: "+table_name)

def main():
    # parse commandline arguments
    config_file_path = parse_args()
    config = load_config_file(config_file_path)
    logging.basicConfig(filename=config['configDirectory']+'process_to_db_log.log', filemode='w', level=logging.DEBUG)
    create_db(config)
    process_data(config)
...
if __name__ == '__main__':
    ...
    logging.info("Finished in {:0>2}:{:0>2}:{:05.2f}".format(int(hours),int(minutes),seconds))
