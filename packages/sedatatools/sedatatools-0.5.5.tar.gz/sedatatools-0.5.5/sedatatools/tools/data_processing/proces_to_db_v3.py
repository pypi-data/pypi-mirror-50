...
def create_db(config):
    ...
    try:
        query = r'CREATE DATABASE {} ON ( NAME = Sales_dat, FILENAME="X:\MSSQL_Data\{}.mdf") LOG ON ( NAME = Sales_log, FILENAME="X:\MSSQL_Data\{}_log.mdf");'\
    ...
    for table_id, table in table_holder.items():
        try:
            ...
            s = mp.Process(
                ...
                args=(table_holder[table_id], prefix_to_table + table_id, config_files),
            )
            s.start()
            ...
            s = mp.Process(
                ...
                table_holder[table_id], prefix_to_table + table_id, config_files,
                ),
            )
            s.start()
            # table_holder[table_id].to_sql(prefix_to_table+table_id, connection, index=False)
