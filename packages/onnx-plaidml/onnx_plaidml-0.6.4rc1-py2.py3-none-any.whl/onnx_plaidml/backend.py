...
class PlaidMLBackendRep(onnx.backend.base.BackendRep):
    ...
    def save(self, filename):
        ...
        if not self._invoker:
            ...
            # When no input run with blank input
            for valinfo in self._input_valinfos:
                zeros = np.zeros(tuple(i.dim_value for i in valinfo.type.tensor_type.shape.dim))
                val = tile.Value.from_python_value(zeros, ctx=self._ctx, dev=self._dev).var
                self._invoker.set_input(_as_input_id(valinfo.name), val)
            self._invoker.save(filename)
        else:
            self._invoker.save(filename)
