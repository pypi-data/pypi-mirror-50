"""Steps: processes that have to happen to assemble a design."""
...
class Setup(Step):
    ...
    All transfers for a Setup step from the magical and bottomless 'Fridge'
...
class Pipette(Step):
    """Pipette the input containers to match a 'target' plate or remove a fixed 'volume'
...
class Move(Step):
    ...
    def execute(self, protocol: Protocol):
        ...
        protocol.instructions.append(
            Instruction(name=self.name, transfers=self.transfers)
        )
...
class Incubate(Step):
    ...
    def execute(self, protocol: Protocol):
        ...
        protocol.instructions.append(Instruction(name=self.name, temps=self.temps))
