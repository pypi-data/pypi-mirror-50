...
def moclo(
    ...
    of SeqRecord combinations that may circularize into new vectors

    Arguments:
        combinations {Iterable[List[SeqRecord]]} -- possible cobinations of fragments
        enzymes {List[RestrictionType]} -- enzymes to digest and ligate with
    ...
    return simulate(combinations, [BsaI, BpiI], resistance)


def simulate(
    combinations: Iterable[List[SeqRecord]],
    ...
    for combination in combinations:
        valid_assemblies.extend(_valid_assemblies(combination, enzymes, resistance))

    return valid_assemblies


def _valid_assemblies(
    record_set: List[SeqRecord], enzymes: List[RestrictionType], resistance: str
) -> List[List[SeqRecord]]:
