...
class EcoNetApiInterface(object):
    ...
    def set_state(_id, body):
        ...
        _LOGGER.debug("set_state response:")
        _LOGGER.debug(arequest.json())
    ...
    def get_modes(_id):
        ...
        _LOGGER.debug("get_modes response:")
        _LOGGER.debug(arequest.json())
        status_code = str(arequest.status_code)
        if status_code == '401':
    ...
    def get_usage(_id):
        ...
        url = USAGE_URL % _id
        arequest = requests.get(url, headers=HEADERS)
        _LOGGER.debug("get_usage response:")
        status_code = str(arequest.status_code)
        if status_code == '401':
            _LOGGER.error("Token expired.")
            return False
    ...
    def get_device(_id):
        ...
        url = DEVICE_URL % _id
        arequest = requests.get(url, headers=HEADERS)
        _LOGGER.debug("get_device response:")
        _LOGGER.debug(arequest.json())
        status_code = str(arequest.status_code)
        if status_code == '401':
            _LOGGER.error("Token expired.")
    ...
    def get_locations():
        ...
        _LOGGER.debug("get_locations response:")
        _LOGGER.debug(arequest.json())
        status_code = str(arequest.status_code)
        if status_code == '401':
            _LOGGER.error("Token expired.")
    ...
    def get_vacations():
        ...
        _LOGGER.debug("get_vacations response:")
        _LOGGER.debug(arequest.json())
        status_code = str(arequest.status_code)
        if status_code == '401':
            _LOGGER.error("Token expired.")
    ...
    def create_vacation(body):
        ...
        _LOGGER.debug("create_vacation response:")
        _LOGGER.debug(arequest.json())
        status_code = str(arequest.status_code)
        if status_code != '200':
    ...
    def delete_vacation(_id):
        ...
        _LOGGER.debug("delete_vacation response:")
        _LOGGER.debug(arequest.json())
        status_code = str(arequest.status_code)
        if status_code != '202':
