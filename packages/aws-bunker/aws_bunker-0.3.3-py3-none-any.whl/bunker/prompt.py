...
def prompt(instance_id=None, dostart=None, yes=None):
    #config here
    home = expanduser('~')
    ...
    if yes or query_yes_no("install custom bash prompt with neofetch on: "+instance_ip+"?", "yes"):
        print(Bcolors.GOLD+bytes.decode(b'\xE2\x8F\xB3', 'utf8')+" setting up bash prompt with neofetch"+Bcolors.ENDC)
        ...
        cmd = "aws ssm send-command --instance-ids \""+instance_id+"\" --document-name \"AWS-RunShellScript\" --parameters commands=\"echo source /home/"+username+"/.prompt.sh | sudo tee -a /home/ubuntu/.profile\""
