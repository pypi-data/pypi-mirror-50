...
for example files please visit: """+Bcolors.PALEBLUE+"""https://gitlab.com/shindagger/bunker"""+Bcolors.ENDC)
    ...
    parser_repo = subparsers.add_parser('repo', formatter_class=argparse.RawTextHelpFormatter, help="""add or remove repos to/from the repo file.
...
if the filename is not in the ignore file, it will be added.""")
    parser_ignore.add_argument('ignores', nargs='+', help='a list of filenames to add or remove to/from the ignore file. requires at least one file to ignore.')
    ...
    if args.subcommand == 'build':
        ...
        repos = args.repos
        backup = args.backup
        args.func(repos, yes, backup)
    elif args.subcommand == 'ignore':
        yes = args.yes
        ignores = args.ignores
        backup = args.backup
        args.func(ignores, yes, backup)
    else:
        args.func()
