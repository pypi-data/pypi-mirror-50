...
class sfs2:
    ...
    def kfold_model_perm(self, X, y, params, folds = 5, model = 'logit', cols, use_true_values = False, true_values = 'None'):
        ...
        for trn, test in kfold.split(X):
            ...
            df_dict = {
                       ...
                       'actual': y_test,
                       'test_ind': test,
                       'fold_num': pd.Series([fold_num for i in range(len(y_test))]),
                       'accuracy': pd.Series([accuracy for i in range(len(y_test))])
                      }
    ...
    def backwards_elimination(self, X, y, params, param_grid = 'None', model = 'logit', cols = [], verbose = 0):
        ...
        for i in range(len(X.columns)):
            ...
            print(begin)
            df_perm, perm_df = self.kfold_model_perm(X[:, indices], y, params = params, model = model, cols = cols, indices = indices)
