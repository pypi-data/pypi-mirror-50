...
def SelectKBest(i):
    remote_path = '/temp/'+'med_'+str(i)+'.csv'
    local_path = 'med_'+str(i)+'.csv'
    client = paramiko.Transport(("120.79.10.106",22))
    client.connect(username='nanshen',password='Aa123')
...
def Cross_Feature(k,p,num):
    for i in range(p,num):
            SelectKBest(i)
    data = pd.DataFrame()
    for j in range(p,num):
        data_1 = pd.read_csv('med_'+str(j)+'.csv')
        data = pd.concat([data,data_1])
    return data

def Data_Feature(data_1,k,p,num):
    data_columns = data_1.columns.tolist()
    data_2 = Cross_Feature(k,p,num)
    data_2 =data_2[data_columns]
