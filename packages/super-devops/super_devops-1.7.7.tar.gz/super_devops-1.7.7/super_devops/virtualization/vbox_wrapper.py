...
class BaseVbox(object):
    ...
    def setup_ip(self, vm, address, netmask, gateway):
        try:
            ...
            cmd = "su {} -c 'vboxmanage guestcontrol {} --username " \
                  "'Administrator' run " \
    ...
    def setup_dns(self, vm, dns):
        try:
            ...
            cmd = "su {} -c 'vboxmanage guestcontrol {} --username " \
                  ...
                  "name='Local Area Connection' static {}'".format(self.username, vm, dns)
            logger.debug(cmd)
            process = subprocess.Popen(
                cmd, shell=True,
                stdout=subprocess.PIPE, stderr=subprocess.PIPE
    ...
    def install_license(self, vm, lic):
        try:
            ...
            cmd = "su {} -c 'vboxmanage guestcontrol {} --username " \
                  "'Administrator' run " \
                  "--exe 'C:\\Windows\\system32\\cmd.exe' -- " \
                  "cmd.exe /c slmgr.vbs -ipk {}'".format(self.username, vm, lic)
            logger.debug(cmd)
            process = subprocess.Popen(
                cmd, shell=True,
                stdout=subprocess.PIPE, stderr=subprocess.PIPE
    ...
    def activate_license(self, vm):
        try:
            ...
            cmd = "su {} -c 'vboxmanage guestcontrol {} --username " \
                  "'Administrator' run " \
                  "--exe 'C:\\Windows\\system32\\cmd.exe' -- " \
                  "cmd.exe /c slmgr.vbs -ato'".format(self.username, vm)
            logger.debug(cmd)
            process = subprocess.Popen(
                cmd, shell=True,
                stdout=subprocess.PIPE, stderr=subprocess.PIPE
