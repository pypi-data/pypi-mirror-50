...
def send_command_to(config, aws, endpoint, response):
    ...
    aws.send_msg(endpoint, json.dumps(resp))
    logging.info('command %s sended', response.data)
    if isinstance(response, MessageResponse):
        # 如果是Message消息就发APNS推送
        msg = response.message.get('content').get('body').get('content')[:72]
        resp2 = aws.send_sns_to(resp.arn, msg)
        logging.info('push notification sended:%s with arn:%s', resp2, resp.arn)
