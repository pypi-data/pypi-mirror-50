...
__version__ = '1.8.0'
__author__ = 'Amr Hassan, hugovk, Philippe Normand'
__copyright__ = "Copyright (C) 2008-2010 Amr Hassan, 2013-2017 hugovk, 2017 Philippe Normand"
__license__ = "apache2"
...
class _Network(object):
    ...
    async def authenticate(self):
        ...
        if self.token and not self.session_key:
            sk_gen = SessionKeyGenerator(self)
            self.session_key = await sk_gen.get_web_auth_session_key(
                url=None, token=self.token)
        ...
        if ((self.api_key and self.api_secret) and not self.session_key and
           (self.username and self.password_hash)):
            sk_gen = SessionKeyGenerator(self)
            self.session_key = await sk_gen.get_session_key(
    ...
    async def _delay_call(self):
        ...
        if time_since_last < DELAY_TIME:
            i = 0
            while i < (DELAY_TIME - time_since_last):
                i += 0.1
                await asyncio.sleep(0.1)

        self.last_call_time = now

    def _sync_delay_call(self):
        """
            Makes sure that web service calls are at least 0.2 seconds apart.
        """
    ...
    async def create_new_playlist(self, title, description):
        """
        ...
        doc = await _Request(self, 'playlist.create', params).execute(False)
    ...
    async def get_top_artists(self, limit=None, cacheable=True):
        ...
        doc = await _Request(self, "chart.getTopArtists", params).execute(cacheable)

        return _extract_top_artists(doc, self)

    async def get_top_tracks(self, limit=None, cacheable=True):
        ...
        doc = await _Request(self, "chart.getTopTracks", params).execute(cacheable)
    ...
    async def get_top_tags(self, limit=None, cacheable=True):
        ...
        doc = await _Request(self, "tag.getTopTags").execute(cacheable)
    ...
    async def get_geo_events(
        ...
        doc = await _Request(self, "geo.getEvents", params).execute(cacheable)

        return _extract_events_from_doc(doc, self)

    async def get_metro_weekly_chart_dates(self, cacheable=True):
        """
        Returns a list of From and To tuples for the available metro charts.
        """

        doc = await _Request(self, "geo.getMetroWeeklyChartlist").execute(cacheable)
    ...
    async def get_metros(self, country=None, cacheable=True):
        """
        ...
        doc = await _Request(self, "geo.getMetros", params).execute(cacheable)
    ...
    async def get_geo_top_artists(self, country, limit=None, cacheable=True):
        ...
        doc = await _Request(self, "geo.getTopArtists", params).execute(cacheable)
    ...
    async def get_geo_top_tracks(
        ...
        doc = await _Request(self, "geo.getTopTracks", params).execute(cacheable)
    ...
    async def get_track_by_mbid(self, mbid):
        """Looks up a track by its MusicBrainz ID"""

        params = {"mbid": mbid}

        doc = await _Request(self, "track.getInfo", params).execute(True)

        return Track(_extract(doc, "name", 1), _extract(doc, "name"), self)

    async def get_artist_by_mbid(self, mbid):
        """Loooks up an artist by its MusicBrainz ID"""

        params = {"mbid": mbid}

        doc = await _Request(self, "artist.getInfo", params).execute(True)

        return Artist(_extract(doc, "name"), self)

    async def get_album_by_mbid(self, mbid):
        """Looks up an album by its MusicBrainz ID"""

        params = {"mbid": mbid}

        doc = await _Request(self, "album.getInfo", params).execute(True)
    ...
    async def update_now_playing(
        ...
        await _Request(self, "track.updateNowPlaying", params).execute()
    ...
    async def scrobble(
        ...
        return await self.scrobble_many(({
    ...
    async def scrobble_many(self, tracks):
        """
        ...
        await _Request(self, "track.scrobble", params).execute()

        if remaining_tracks:
            await self.scrobble_many(remaining_tracks)

    async def get_play_links(self, link_type, things, cacheable=True):
        ...
        doc = await _Request(self, method, params).execute(cacheable)
    ...
    async def get_artist_play_links(self, artists, cacheable=True):
        return self.get_play_links("artist", artists, cacheable)

    async def get_album_play_links(self, albums, cacheable=True):
        return self.get_play_links("album", albums, cacheable)

    async def get_track_play_links(self, tracks, cacheable=True):
...
class _ShelfCacheBackend(object):
    ...
    def __iter__(self):
        return iter(list(self.shelf.keys()))
...
class _Request(object):
    ...
    def sign_it(self):
        ...
        if "api_sig" not in list(self.params.keys()):
    ...
    async def _get_cached_response(self):
        ...
        if not self._is_cached():
            response = await self._download_response()
            self.cache.set_xml(self._get_cache_key(), response)

        return self.cache.get_xml(self._get_cache_key())

    def _sync_get_cached_response(self):
        """Returns a file object of the cached response."""

        if not self._is_cached():
            response = self._sync_download_response()
            self.cache.set_xml(self._get_cache_key(), response)
    ...
    async def _download_response(self):
        """Returns a response body string from the server."""

        if self.network.limit_rate:
            await self.network._delay_call()
        ...
        headers = {
            ...
            'User-Agent': "async-pylast" + '/' + __version__
        }

        (HOST_NAME, HOST_SUBDIR) = self.network.ws_server

        # FIXME: reimplement proxy support


        url = 'https://%s%s' % self.network.ws_server
        response_text = await HTTP_CLIENT.post(url, data)
        response_text = XML_ILLEGAL.sub("?", response_text)
        return self._check_response_for_errors(response_text)

    def _sync_download_response(self):
        """Returns a response body string from the server."""

        if self.network.limit_rate:
            self.network._sync_delay_call()
    ...
    async def execute(self, cacheable=False):
    ...
    def sync_execute(self, cacheable=False):
        """Returns the XML DOM response of the POST Request from the server"""

        if self.network.is_caching_enabled() and cacheable:
            response = self._sync_get_cached_response()
        else:
            response = self._sync_download_response()
...
class SessionKeyGenerator(object):
    ...
    async def _get_web_auth_token(self):
        """
    ...
    async def get_web_auth_session_key(self, url, token=""):
        """
        Retrieves the session key of a web authorization process by its url.
        """

        if url in list(self.web_auth_tokens.keys()):
    ...
    async def get_session_key(self, username, password_hash):
        """
...
class _BaseObject(object):
    ...
    def _sync_request(self, method_name, cacheable=False, params=None):
        if not params:
            params = self._get_params()

        request = _Request(self.network, method_name, params)
        return request.sync_execute(cacheable)

    async def _request(self, method_name, cacheable=False, params=None):
        if not params:
            result = self._get_params()
        ...
        request = _Request(self.network, method_name, params)
        return await request.execute(cacheable)
    ...
    def __hash__(self):
        ...
        values = list(map(six.text_type, list(self._get_params().values())))

        return hash(self.network) + hash(six.text_type(type(self)) + "".join(
            list(self._get_params().keys()) + list(values)
        ).lower())

    async def _extract_cdata_from_request(self, method_name, tag_name, params):
        doc = await self._request(method_name, True, params)

        return doc.getElementsByTagName(
            tag_name)[0].firstChild.wholeText.strip()

    async def _get_things(
    ...
    async def get_top_fans(self, limit=None, cacheable=True):
        ...
        doc = await self._request(self.ws_prefix + '.getTopFans', cacheable)
    ...
    async def share(self, users, message=None):
        """
        ...
        while(len(users) > 10):
            section = users[0:9]
            users = users[9:]
            await self.share(section, message)
        ...
        await self._request(self.ws_prefix + '.share', False, params)

    async def get_wiki_published_date(self):
        """
    ...
    async def get_wiki_summary(self):
        """
        Returns the summary of the wiki.
        Only for Album/Track.
        """
    ...
    async def get_wiki(self, section):
        """
        ...
        doc = await self._request(self.ws_prefix + ".getInfo", True)
    ...
    async def get_shouts(self, limit=50, cacheable=False):
        """
            Returns a sequence of Shout objects
        """

        shouts = []
        for node in await _collect_nodes(
                limit,
                self,
...
class _Chartable(object):
    ...
    async def get_weekly_chart_dates(self):
        """Returns a list of From and To tuples for the available charts."""

        doc = await self._request(self.ws_prefix + ".getWeeklyChartList", True)
    ...
    async def get_weekly_album_charts(self, from_date=None, to_date=None):
        """
    ...
    async def get_weekly_artist_charts(self, from_date=None, to_date=None):
        """
    ...
    async def get_weekly_track_charts(self, from_date=None, to_date=None):
        """
    ...
    async def get_weekly_charts(self, chart_kind, from_date=None, to_date=None):
        """
...
class _Taggable(object):
    ...
    async def add_tag(self, tag):
        ...
        * tag: a tag name or a Tag object.
        ...
        await self._request(self.ws_prefix + '.addTags', False, params)
    ...
    async def remove_tag(self, tag):
        ...
        await self._request(self.ws_prefix + '.removeTag', False, params)
    ...
    async def get_tags(self):
        ...
        doc = await self._request(self.ws_prefix + '.getTags', False, params)
    ...
    async def set_tags(self, tags):
        ...
        * tags: a sequence of tag names or Tag objects.
        ...
        await self.remove_tags(to_remove)
        await self.add_tags(to_add)

    async def get_top_tags(self, limit=None):
        """Returns a list of the most frequently used Tags on this object."""

        doc = await self._request(self.ws_prefix + '.getTopTags', True)
...
class _Opus(_BaseObject, _Taggable):
    ...
    def __eq__(self, other):
        if type(self) != type(other):
            return False
        a = self._sync_get_title().lower()
        b = other._sync_get_title().lower()
        c = self.get_artist().sync_get_name().lower()
        d = other.get_artist().sync_get_name().lower()
        return (a == b) and (c == d)
    ...
    async def _get_params(self):
        return {
            'artist': await self.get_artist().get_name(),
            self.ws_prefix: await self.get_title()}
    ...
    def _sync_get_title(self, properly_capitalized=False):
        """Returns the artist or track title."""
        if properly_capitalized:
            self.title = _extract(
                self._sync_request(self.ws_prefix + ".getInfo", True), "name")

        return self.title

    async def get_title(self, properly_capitalized=False):
        """Returns the artist or track title."""
        if properly_capitalized:
            self.title = _extract(
                await self._request(self.ws_prefix + ".getInfo", True), "name")

        return self.title

    async def get_name(self, properly_capitalized=False):
        """Returns the album or track title (alias to get_title())."""

        return self.get_title(properly_capitalized)

    def sync_get_name(self, properly_capitalized=False):
        """Returns the album or track title (alias to get_title())."""

        return self._sync_get_title(properly_capitalized)

    async def get_id(self):
        """Returns the ID on the network."""

        return _extract(
            await self._request(self.ws_prefix + ".getInfo", cacheable=True), "id")

    async def get_playcount(self):
    ...
    async def get_userplaycount(self):
        ...
        doc = await self._request(self.ws_prefix + ".getInfo", True, params)
    ...
    async def get_mbid(self):
        """Returns the MusicBrainz ID of the album or track."""

        doc = await self._request(self.ws_prefix + ".getInfo", cacheable=True)
...
class Album(_Opus):
    ...
    async def get_release_date(self):
        ...
        return _extract(await self._request(
            self.ws_prefix + ".getInfo", cacheable=True), "releasedate")

    async def get_cover_image(self, size=COVER_EXTRA_LARGE):
        """
    ...
    async def get_url(self, domain_name=DOMAIN_ENGLISH):
        ...
        * domain_name str: The network's language domain. Possible values:
        ...
        artist = _url_safe(await self.get_artist().get_name())
        title = _url_safe(await self.get_title())
...
class Artist(_BaseObject, _Taggable):
    ...
    def __repr__(self):
        return "pylast.Artist(%s, %s)" % (
            repr(self.sync_get_name()), repr(self.network))

    def __unicode__(self):
        return six.text_type(self.sync_get_name())
    ...
    def __eq__(self, other):
        if type(self) is type(other):
            return self.sync_get_name().lower() == other.sync_get_name().lower()
        else:
            return False
    ...
    async def _get_params(self):
        return {self.ws_prefix: await self.get_name()}
    ...
    async def get_correction(self):
        """Returns the corrected artist name."""

        return _extract(
            await self._request(self.ws_prefix + ".getCorrection"), "name")

    async def get_cover_image(self, size=COVER_MEGA):
        """
        Returns a uri to the cover image
        size can be one of:
            COVER_MEGA
        ...
        return _extract_all(
            await self._request(self.ws_prefix + ".getInfo", True), "image")[size]

    async def get_playcount(self):
        """Returns the number of plays on the network."""

        return _number(_extract(
            await self._request(self.ws_prefix + ".getInfo", True), "playcount"))
    ...
    async def get_listener_count(self):
        ...
        if hasattr(self, "listener_count"):
            ...
            self.listener_count = _number(_extract(
                await self._request(self.ws_prefix + ".getInfo", True), "listeners"))
            return self.listener_count

    async def is_streamable(self):
        """Returns True if the artist is streamable."""

        return bool(_number(_extract(
            await self._request(self.ws_prefix + ".getInfo", True), "streamable")))

    async def get_bio(self, section, language=None):
        """
    ...
    async def get_upcoming_events(self):
        ...
        doc = await self._request(self.ws_prefix + '.getEvents', True)

        return _extract_events_from_doc(doc, self.network)

    async def get_similar(self, limit=None):
        ...
        doc = await self._request(self.ws_prefix + '.getSimilar', True, params)
    ...
    async def get_top_albums(self, limit=None, cacheable=True):
    ...
    async def get_url(self, domain_name=DOMAIN_ENGLISH):
        ...
        * domain_name: The network's language domain. Possible values:
        ...
        artist = _url_safe(await self.get_name())
    ...
    async def shout(self, message):
        ...
        await self._request("artist.Shout", False, params)

    async def get_band_members(self):
...
class Event(_BaseObject):
    ...
    async def attend(self, attending_status):
        ...
        * attending_status: The attending status. Possible values:
        ...
        await self._request('event.attend', False, params)
    ...
    async def get_attendees(self):
        ...
        doc = await self._request("event.getAttendees", False)
    ...
    async def get_title(self):
        ...
        doc = await self._request("event.getInfo", True)

        return _extract(doc, "title")

    async def get_headliner(self):
    ...
    async def get_attendance_count(self):
    ...
    async def shout(self, message):
        ...
        await self._request("event.Shout", False, params)
...
class Country(_BaseObject):
    ...
    async def get_top_artists(self, limit=None, cacheable=True):
        ...
        doc = await self._request('geo.getTopArtists', cacheable, params)
...
class Metro(_BaseObject):
    ...
    async def _get_chart(
        ...
        doc = await self._request(method, cacheable, params)
    ...
    async def get_hype_artist_chart(
            self, tag="artist", limit=None, from_date=None, to_date=None,
            cacheable=True):
    ...
    async def get_unique_artist_chart(
            self, tag="artist", limit=None, from_date=None, to_date=None,
            cacheable=True):
    ...
    async def get_hype_track_chart(
            self, tag="track", limit=None, from_date=None, to_date=None,
            cacheable=True):
        """Get a chart of tracks for a metro.
        Parameters:
    ...
    async def get_unique_track_chart(
            self, tag="track", limit=None, from_date=None, to_date=None,
            cacheable=True):
        """Get a chart of tracks for a metro.
        Parameters:
...
class Library(_BaseObject):
    ...
    async def _get_params(self):
        return {'user': await self.user.get_name()}
    ...
    async def add_album(self, album):
        """Add an album to this library."""

        params = await self._get_params()
        params["artist"] = await album.get_artist().get_name()
        params["album"] = await album.get_name()

        await self._request("library.addAlbum", False, params)

    async def remove_album(self, album):
        ...
        await self._request(self.ws_prefix + ".removeAlbum", False, params)

    async def add_artist(self, artist):
        ...
        if type(artist) == str:
            ...
            params["artist"] = await artist.get_name()

        await self._request(self.ws_prefix + ".addArtist", False, params)

    async def remove_artist(self, artist):
        ...
        await self._request(self.ws_prefix + ".removeArtist", False, params)

    async def add_track(self, track):
        """Add a track to this library."""

        params = await self._get_params()
        params["track"] = await track.get_title()

        await self._request(self.ws_prefix + ".addTrack", False, params)

    async def get_albums(self, artist=None, limit=50, cacheable=True):
        """
    ...
    async def get_artists(self, limit=50, cacheable=True):
        """
        Returns a sequence of Album objects
        if limit==None it will return all (may take a while)
        """
    ...
    async def get_tracks(self, artist=None, album=None, limit=50, cacheable=True):
        """
        Returns a sequence of Album objects
        If limit==None it will return all (may take a while)
        """
    ...
    async def remove_scrobble(self, artist, title, timestamp):
        ...
        await self._request(self.ws_prefix + ".removeScrobble", False, params)
...
class Playlist(_BaseObject):
    ...
    async def _get_info_node(self):
        """
        Returns the node from user.getPlaylists where this playlist's info is.
        """

        doc = await self._request("user.getPlaylists", True)
    ...
    async def _get_params(self):
        return {'user': await self.user.get_name(), 'playlistID': self.get_id()}
    ...
    async def add_track(self, track):
        ...
        params = await self._get_params()
        params['artist'] = await track.get_artist().get_name()
        params['track'] = await track.get_title()

        await self._request('playlist.addTrack', False, params)

    async def get_title(self):
        """Returns the title of this playlist."""

        return _extract(await self._get_info_node(), "title")

    async def get_creation_date(self):
        """Returns the creation date of this playlist."""

        return _extract(await self._get_info_node(), "date")

    async def get_size(self):
        """Returns the number of tracks in this playlist."""

        return _number(_extract(await self._get_info_node(), "size"))

    async def get_description(self):
        """Returns the description of this playlist."""

        return _extract(await self._get_info_node(), "description")

    async def get_duration(self):
        """Returns the duration of this playlist in milliseconds."""

        return _number(_extract(await self._get_info_node(), "duration"))
    ...
    async def is_streamable(self):
        ...
        if _extract(await self._get_info_node(), "streamable") == '1':
            return True
        else:
            return False

    async def has_track(self, track):
        """Checks to see if track is already in the playlist.
        * track: Any Track object.
        """

        return track in await self.get_tracks()
    ...
    async def get_cover_image(self, size=COVER_EXTRA_LARGE):
        ...
        return _extract(await self._get_info_node(), "image")[size]
    ...
    async def get_url(self, domain_name=DOMAIN_ENGLISH):
        ...
        * domain_name: The network's language domain. Possible values:
        ...
        english_url = _extract(await self._get_info_node(), "url")
        appendix = english_url[english_url.rfind("/") + 1:]

        return self.network._get_url(domain_name, "playlist") % {
            'appendix': appendix, "user": await self.get_user().get_name()}
...
class Tag(_BaseObject, _Chartable):
    ...
    async def get_similar(self):
        ...
        doc = await self._request(self.ws_prefix + '.getSimilar', True)
    ...
    async def get_url(self, domain_name=DOMAIN_ENGLISH):
        ...
        * domain_name: The network's language domain. Possible values:
        ...
        name = _url_safe(await self.get_name())
...
class Track(_Opus):
    ...
    async def is_fulltrack_available(self):
    ...
    async def love(self):
        ...
        await self._request(self.ws_prefix + '.love')

    async def unlove(self):
        """Remove the track to the user's loved tracks. """

        await self._request(self.ws_prefix + '.unlove')

    async def ban(self):
        """Ban this track from ever playing on the radio. """

        await self._request(self.ws_prefix + '.ban')
...
class Group(_BaseObject, _Chartable):
    ...
    async def get_members(self, limit=50, cacheable=False):
        """
...
class XSPF(_BaseObject):
    ...
    async def get_tracks(self):
        ...
        doc = await self._request('playlist.fetch', True)
...
class User(_BaseObject, _Chartable):
    ...
    def __eq__(self, another):
        if isinstance(another, User):
            return self.sync_get_name() == another.sync_get_name()
        else:
            return False
    ...
    async def get_artist_tracks(self, artist, cacheable=False):
        """
        ...
        seq = []
        for track in await _collect_nodes(
                None,
                self,
    ...
    async def get_friends(self, limit=50, cacheable=False):
    ...
    async def get_loved_tracks(self, limit=50, cacheable=True):
        """
    ...
    async def get_neighbours(self, limit=50, cacheable=True):
        """Returns a list of the user's friends."""
    ...
    async def get_past_events(self, limit=50, cacheable=False):
        """
    ...
    async def get_playlists(self):
        ...
        doc = await self._request(self.ws_prefix + ".getPlaylists", True)

        playlists = []
        for playlist_id in _extract_all(doc, "id"):
            playlists.append(
                Playlist(await self.get_name(), playlist_id, self.network))
    ...
    async def get_now_playing(self):
        ...
        doc = await self._request(self.ws_prefix + '.getRecentTracks', False, params)
    ...
    async def get_recent_tracks(self, limit=10, cacheable=True,
    ...
    async def get_unixtime_registered(self):
    ...
    async def get_tagged_albums(self, tag, limit=None, cacheable=True):
        ...
        if limit:
            params['limit'] = limit
        doc = await self._request(self.ws_prefix + '.getpersonaltags', cacheable,
                                  params)
        return _extract_albums(doc, self.network)

    async def get_tagged_artists(self, tag, limit=None):
        ...
        if limit:
            params["limit"] = limit
        doc = await self._request(self.ws_prefix + '.getpersonaltags', True, params)
        return _extract_artists(doc, self.network)

    async def get_tagged_tracks(self, tag, limit=None, cacheable=True):
    ...
    async def get_top_artists(self, period=PERIOD_OVERALL, limit=None):
        ...
        * period: The period of time. Possible values:
        ...
        doc = await self._request(self.ws_prefix + '.getTopArtists', True, params)
    ...
    async def get_top_tags(self, limit=None, cacheable=True):
        ...
        * cacheable: Whether to cache results.
        ...
        doc = await self._request(self.ws_prefix + ".getTopTags", cacheable, params)
    ...
    async def compare_with_user(self, user, shared_artists_limit=None):
        """
        ...
        params['type1'] = 'user'
        params['type2'] = 'user'
        params['value1'] = await self.get_name()
        params['value2'] = user

        doc = await self._request('tasteometer.compare', False, params)
    ...
    async def shout(self, message):
        ...
        await self._request(self.ws_prefix + ".Shout", False, params)
...
class AuthenticatedUser(User):
    ...
    async def _get_params(self):
        return {"user": await self.get_name()}

    async def get_name(self):
        """Returns the name of the authenticated user."""

        doc = await self._request("user.getInfo", True, {"user": ""})    # hack

        self.name = _extract(doc, "name")
        return self.name

    async def get_recommended_events(self, limit=50, cacheable=False):
        """
        Returns a sequence of Event objects
        if limit==None it will return all
        """
    ...
    async def get_recommended_artists(self, limit=50, cacheable=False):
        """
...
class _Search(_BaseObject):
    ...
    def _get_params(self):
        ...
        for key in list(self.search_terms.keys()):
            params[key] = self.search_terms[key]

        return params

    async def get_total_result_count(self):
        """Returns the total count of all the results."""

        doc = await self._request(self._ws_prefix + ".search", True)

        return _extract(doc, "opensearch:totalResults")

    async def _retrieve_page(self, page_index):
        """Returns the node of matches to be processed"""

        params = self._get_params()
        params["page"] = str(page_index)
        doc = await self._request(self._ws_prefix + ".search", True, params)

        return doc.getElementsByTagName(self._ws_prefix + "matches")[0]

    async def _retrieve_next_page(self):
        self._last_page_index += 1
        return await self._retrieve_page(self._last_page_index)
...
class AlbumSearch(_Search):
    ...
    async def get_next_page(self):
        ...
        master_node = await self._retrieve_next_page()
...
async def _collect_nodes(limit, sender, method_name, cacheable, params=None):
    """
    ...
    while not end_of_pages and (not limit or (limit and len(nodes) < limit)):
        ...
        doc = await sender._request(method_name, cacheable, params)
