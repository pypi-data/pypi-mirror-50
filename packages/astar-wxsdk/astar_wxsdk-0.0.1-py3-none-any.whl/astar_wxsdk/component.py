# -*- coding: utf-8 -*-
"""
    astar_wxsdk.component
    ~~~~~~~~~~~~~~~

    This module provides client library for Wx Open Platform
...
from astar_wxsdk.client import WxComponentClient
from astar_wxsdk.constants import WxErrorCode
from astar_wxsdk.crypto import WxCrypto
from astar_wxsdk.exceptions import APILimitedException, WxClientException, WxOAuthException, \
    WxComponentOAuthException
from astar_wxsdk.fields import DateTimeField, StringField
from astar_wxsdk.messages import MessageMetaClass
from astar_wxsdk.session.memorystorage import MemoryStorage
from astar_wxsdk.utils import get_querystring, json, to_binary, to_text, ObjectDict
...
class BaseWxComponent(object):
    ...
    def __init__(self,
        ...
        if isinstance(session, six.string_types):
            ...
            from astar_wxsdk.session.shovestorage import ShoveStorage

            querystring = get_querystring(session)
            prefix = querystring.get('prefix', ['astar_wxsdk'])[0]
    ...
    def _handle_result(self, res, method=None, url=None, **kwargs):
        ...
        if 'errcode' in result and result['errcode'] != 0:
            ...
            if self.auto_retry and errcode in (
                    WxErrorCode.INVALID_CREDENTIAL.value,
                    WxErrorCode.INVALID_ACCESS_TOKEN.value,
                    WxErrorCode.EXPIRED_ACCESS_TOKEN.value):
            ...
            elif errcode == WxErrorCode.OUT_OF_API_FREQ_LIMIT.value:
...
class WxComponent(BaseWxComponent):
    ...
    def get_client_by_authorization_code(self, authorization_code):
        ...
        :params authorization_code: 授权code,会在授权成功时返回给第三方平台，详见第三方平台授权流程说明
        """
        warnings.warn('`get_client_by_authorization_code` method of `WxComponent` is deprecated,'
    ...
    def cache_component_verify_ticket(self, msg, signature, timestamp, nonce):
        ...
        :params nonce: 随机数
        """
        warnings.warn('`cache_component_verify_ticket` method of `WxComponent` is deprecated,'
    ...
    def get_unauthorized(self, msg, signature, timestamp, nonce):
        ...
        :params nonce: 随机数
        """
        warnings.warn('`get_unauthorized` method of `WxComponent` is deprecated,'
                      'Use `parse_message` instead',
                      DeprecationWarning, stacklevel=2)
        content = self.crypto.decrypt_message(msg, signature, timestamp, nonce)
        message = xmltodict.parse(to_text(content))['xml']
    ...
    def __init__(self, app_id, component_appid=None, component_access_token=None,
        ...
        if self.component is None:
            warnings.warn('cannot found `component` param of `ComponentOAuth` `__init__` method,'
                          'Use `WxComponent.get_component_oauth` instead',
                          DeprecationWarning, stacklevel=2)
    ...
    def _handle_result(self, res, method=None, url=None, **kwargs):
        ...
        if 'errcode' in result and result['errcode'] != 0:
            ...
            if self.component.auto_retry and errcode in (
                ...
                raise WxComponentOAuthException(
                    errcode,
                    errmsg,
                    client=self,
                    request=res.request,
