...
from astar_wxsdk.exceptions import WxPayException, InvalidSignatureException
from astar_wxsdk.pay.utils import (
    calculate_signature, calculate_signature_hmac, _check_signature, dict_to_xml
)
from astar_wxsdk.pay.base import BaseWxPayAPI
from astar_wxsdk.pay import api
from astar_wxsdk.client import BaseWxClient
...
def _is_api_endpoint(obj):
    return isinstance(obj, BaseWxPayAPI)


class WxPay(BaseWxClient):
    """
    微信支付接口
    ...
    redpack = api.WxRedpack()
    """红包接口"""
    transfer = api.WxTransfer()
    """企业付款接口"""
    coupon = api.WxCoupon()
    """代金券接口"""
    order = api.WxOrder()
    ...
    refund = api.WxRefund()
    """退款接口"""
    micropay = api.WxMicroPay()
    """刷卡支付接口"""
    tools = api.WxTools()
    """工具类接口"""
    jsapi = api.WxJSAPI()
    """公众号网页 JS 支付接口"""
    withhold = api.WxWithhold()
    """代扣接口"""

    API_BASE_URL = 'https://api.mch.weixin.qq.com/'

    def __new__(cls, *args, **kwargs):
        self = super(WxPay, cls).__new__(cls)
    ...
    def _request(self, method, url_or_endpoint, **kwargs):
        ...
        logger.debug('Request to Wx API: %s %s\n%s', method, url, kwargs)
    ...
    def _handle_result(self, res):
        res.encoding = 'utf-8'
        xml = res.text
        logger.debug('Response from Wx API \n %s', xml)
        try:
            data = xmltodict.parse(xml)['xml']
        except (xmltodict.ParsingInterrupted, ExpatError):
            # 解析 XML 失败
            logger.debug('Wx payment result xml parsing error', exc_info=True)
            return xml
