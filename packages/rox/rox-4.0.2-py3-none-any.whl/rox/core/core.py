...
class Core:
    ...
    def setup(self, sdk_settings, device_properties, rox_options):
        ...
        self.internal_flags = InternalFlags(self.experiment_repository, self.parser)
        ...
        buid = BUID(sdk_settings, device_properties, self.flag_repository, self.custom_property_repository)

        request_config_builder = RequestConfigurationBuilder(sdk_settings, buid, device_properties, roxy_path)
        ...
        if roxy_path is not None:
            self.configuration_fetcher = ConfigurationFetcherRoxy(request_config_builder, client_request, self.configuration_fetched_invoker)
        else:
            self.configuration_fetcher = ConfigurationFetcher(request_config_builder, client_request, self.configuration_fetched_invoker)
    ...
    def fetch(self):
        ...
        configuration_parser = ConfigurationParser(SignatureVerifier(), self.error_reporter, self.configuration_fetched_invoker)
        ...
        has_changes = self.last_configurations is None or self.last_configurations.data != result.data
    ...
    def start_or_stop_push_updated_listener(self):
        if self.internal_flags.is_enabled('rox.internal.pushUpdates'):
            if not self.push_updates_listener and not self.cancel_event.is_set():
                ...
                self.push_updates_listener.on('changed', lambda event: self.fetch())
