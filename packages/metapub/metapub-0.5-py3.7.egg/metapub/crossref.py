...
class CrossRefWork(object):
    ...
    def pprint(self):
        return """<CrossRefWork DOI: {doi}>\n\t{title} ({journal}, {year})\n\t{authors}""".format(**self.to_citation())

    def __str__(self):
        return """<CrossRefWork DOI: {doi} Score: {score}> "{title}" ({journal}, {year})\n\t""".format(doi=self.doi,
                    score=self.score, title=self.title[0], journal=self.container_title[0], year=self.pubyear)
