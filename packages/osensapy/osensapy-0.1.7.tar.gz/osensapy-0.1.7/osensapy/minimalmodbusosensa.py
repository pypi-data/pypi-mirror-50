...
class Instrument():
    ...
    def __init__(self, port, slaveaddress, baudrate=BAUDRATE, parity=PARITY, mode=MODE_RTU, close_port_after_each_call=False, timeout=TIMEOUT, exclusive=None):
        if exclusive is None or not exclusive:
            if port not in _SERIALPORTS or not _SERIALPORTS[port]:
                self.serial = _SERIALPORTS[port] = serial.Serial(port=port, baudrate=baudrate, parity=parity, bytesize=BYTESIZE, stopbits=STOPBITS, timeout=timeout, exclusive=None)
            else:
        ...
        Attributes:
            ...
            - exclusive (float): whether or not to give exclusive port access
                - Defaults to None
        """
