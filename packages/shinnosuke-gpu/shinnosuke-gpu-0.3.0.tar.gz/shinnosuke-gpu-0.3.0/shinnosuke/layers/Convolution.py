...
class Conv2D(Layer):
    ...
    def forward(self,is_training=True):
        W,b=self.variables
        # pad
        inputs = cp.pad(self.input_tensor, ((0, 0), (0, 0), self.pad_size, self.pad_size), 'constant')
        # padded size
...
class ZeroPadding2D(Layer):
    ...
    def forward(self,is_training=True):
        ...
        self.output_tensor = cp.pad(inputs, ((0, 0), (0, 0), (self.pad_h,self.pad_h), (self.pad_w,self.pad_w)), 'constant')
        if is_training:
            if self.require_grads:
                self.grads=cp.zeros_like(inputs)
        del inputs
        super().forward(is_training)
...
class MaxPooling2D(Layer):
    ...
    def forward(self,is_training=True):
        ...
        pool_argmax = cp.argmax(col, axis=1)

        outputs = cp.max(col, axis=1)
    ...
    def backward(self):
        ...
        pool_h,pool_w=self.pool_size
        dmax = cp.zeros((grads.size, pool_h*pool_w))
        dmax[cp.arange(self.argmax_index.size), self.argmax_index.flatten()] = grads.flatten()
...
class MeanPooling2D(Layer):
    ...
    def forward(self,is_training=True):
        ...
        pool_argmean = cp.array([range(col.shape[1])])

        outputs = cp.mean(col, axis=1)
        self.output_tensor = outputs.reshape(batch_nums, n_H, n_W, n_C).transpose(0, 3, 1, 2)
        if is_training:
            self.inputs_shape = inputs.shape
            self.argmean_index = pool_argmean
            if self.require_grads:
                self.grads = cp.zeros_like(self.output_tensor.value)
            del inputs
        super().forward(is_training)
    ...
    def backward(self):
        ...
        dmean=cp.repeat(grads.flatten(),self.argmean_index.size)/(pool_w*pool_h)
