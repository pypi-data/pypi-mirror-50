...
class Uniform(Initializer):
    ...
    def __call__(self,size):
        return cp.random.uniform(-self.scale,self.scale,size=size)
...
class LecunUniform(Initializer):
    ...
    def __call__(self, size):
        ...
        return Uniform(cp.sqrt(3./fan_in))(size)
...
class GlorotUniform(Initializer):
    ...
    def __call__(self, size):
        fan_in,fan_out=self.decompose_size(size)
        return Uniform(cp.sqrt(6./(fan_in+fan_out)))(size)
...
class HeUniform(Initializer):
    ...
    def __call__(self, size):
        fan_in,fan_out=self.decompose_size(size)
        return Uniform(cp.sqrt(6./fan_in))(size)
...
class Normal(Initializer):
    ...
    def __call__(self,size):
        return cp.random.normal(loc=self.mean,scale=self.std,size=size)
...
class LecunNormal(Initializer):
    ...
    def __call__(self, size):
        fan_in,fan_out=self.decompose_size(size)
        return Normal(cp.sqrt(1./fan_in))(size)
...
class GlorotNormal(Initializer):
    ...
    def __call__(self, size):
        fan_in,fan_out=self.decompose_size(size)
        return Normal(cp.sqrt(2./(fan_in+fan_out)))(size)
...
class HeNormal(Initializer):
    ...
    def __call__(self, size):
        fan_in,fan_out=self.decompose_size(size)
        return Normal(cp.sqrt(2./fan_in))(size)
