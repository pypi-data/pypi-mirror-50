...
class SchemaValidator(AbstractValidator):
    ...
    def announce(self):
        ColorPrinter.printBlue("Checking for schema issues...")

    def validate(self):
        requiredColsNotInData = self.getRequiredColumnsThatArentInData()
        if len(requiredColsNotInData) > 0:
            # debug = False because we need to fail no matter what if the schema is wrong, otherwise
            # downstream checks will fail
