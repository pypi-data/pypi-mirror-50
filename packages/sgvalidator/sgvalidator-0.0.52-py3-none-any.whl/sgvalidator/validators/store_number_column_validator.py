...
class StoreNumberColumnValidator(AbstractValidator):
    ...
    def isColumnTotallyBlank(self):
        return self.countNumBlanks(self.data["store_number"]) == len(self.data["store_number"])

    def isColumnPartiallyFilled(self):
        return 0 < self.countNumBlanks(self.data["store_number"]) < len(self.data["store_number"])
