...
class Definition(AbstractVariable):
    ...
    def redefine_code(self,value,obscure,indentation):
        return "\n{}#ifdef {}\n".format("\t"*indentation,self.get_name(obscure=obscure))\
               +"{}#undef {}\n".format("\t"*indentation,self.get_name(obscure=obscure))\
               +"{}#define {} {}\n".format("\t"*indentation,self.get_name(obscure=obscure),value)\
               +"{}#endif\n".format("\t"*indentation)

    def redefine(self,value):
        return Redefinition(code=partial(self.redefine_code,value=value),obscurable=False)
...
class AbstractFunction(AbstractVariable):
    ...
    def __call__(self, *args):
        ...
        argvars = [(to_abstract_var(args[i]).cast(self.arguments[i].type)
                    if isinstance(self.arguments[i].type,ArduinoEnum) else
                    to_abstract_var(args[i])) for i in range(len(args))]

        return AbstractVariable(
            name=lambda obscure,indentation:"{}({})".format(self.get_name(obscure=obscure),','.join([
                arg.get_name(obscure=obscure)
                for arg in argvars
            ])),
            type = self.type
        )
...
class ArduinoEnum():
    def __init__(self,name,possibilities,type=uint8_t):
        ...
        self.possibilities_description = {key:self.possibilities[key][1] for key in self.possibilities}
...
class ArduinoClassInstance(Variable):
    def __init__(self, arduinoclass, name=None,value=None):
        ...
        for attr_name, attribute in {**arduinoclass.__class__.__dict__,**arduinoclass.__dict__}.items():
            if isinstance(attribute,ArduinoClassInstanceFunction):
                new_attribute=ArduinoClassInstanceFunction(self,attribute.name,arguments=attribute.arguments,return_type=attribute.type)
...
class ArduinoClass(AbstractStructureType):
    def __init__(self, *attributes, class_name=None, include=None):
        ...
        for attr_name, attribute in {**self.__class__.__dict__,**self.__dict__}.items():
            if isinstance(attribute,AbstractFunction):
                new_attribute=ArduinoClassInstanceFunction(self,attribute.name,obscurable=False,arguments=attribute.arguments,return_type=attribute.type)
                setattr(self,attr_name,new_attribute)
            if isinstance(attribute,Variable):
                new_attribute=ArduinoClassInstanceVariable(self,attribute.name,obscurable=False,type=attribute.type)
                setattr(self,attr_name,new_attribute)
    ...
    def __call__(self,name, *args):
        return ArduinoClassInstance(arduinoclass=self,name=name,value="{}({})".format(self.name,",".join([str(arg) for arg in args])))
