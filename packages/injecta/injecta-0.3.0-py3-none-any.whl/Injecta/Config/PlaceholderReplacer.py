...
class PlaceholderReplacer:
    ...
    def replace(self, appConfig: dict):
        self.__appConfigWithResolvers = self.__resolvePlaceholders(appConfig)

        return self.__resolveFinalValues(self.__appConfigWithResolvers)

    def __resolvePlaceholders(self, config):
        output = {}

        for key, value in config.items():
            if isinstance(value, dict):
                output[key] = self.__resolvePlaceholders(value)
            elif isinstance(value, list):
                output[key] = list(map(lambda listItem: self.__resolvePlaceholders(listItem), value))
    ...
    def __resolveFinalValues(self, config):
        output = {}

        for key, value in config.items():
            if isinstance(value, dict):
                output[key] = self.__resolveFinalValues(value)
            elif isinstance(value, list):
                output[key] = list(map(lambda listItem: self.__resolveFinalValues(listItem), value))
            elif callable(value):
                output[key] = value()
            else:
                output[key] = value
