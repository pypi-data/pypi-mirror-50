...
class Building:
        ...
        class ComplexEncoder(json.JSONEncoder):
            def default(self, obj):
                if isinstance(obj, trimesh.Trimesh):
                    self.print_status('converting trimesh')
    ...
    def import_data_from_excel(self, filename, import_config):
        ...
        self.print_status('importing data from file: {}'.format_map(filename))
        ...
        for index, row in material_df.iterrows():
            self.print_status('Creating Material and Layer for: ', row['Name'], row['ID'])
