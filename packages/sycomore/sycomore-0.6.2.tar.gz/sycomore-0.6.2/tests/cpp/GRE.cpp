...
BOOST_FIXTURE_TEST_CASE(Real, Fixture, *boost::unit_test::tolerance(1e-9))
{
    ...
    // Using the current JSON output, we can run a simple benchmark with
    // (for i in $(seq 1 10); do ./tests/GRE -t Real -l nothing -r no; done) | jq --slurp 'sort_by(.total)[0]'
    // CoMoTk: 6.13 s
    // Using -ffast-math, we gain a lot on pulses (0.18 vs. 0.32) and lose a bit
    // on time interval (0.41 vs. 0.36)
    // Total time: 0.653842 s
    // Pulse time: 0.176492 s
    // Time interval time: 0.467764 s
    // Isochromat time: 0.00518554 s
    // Unaccounted time: 0.0044 s
    // Sycomore speedup: 9.38
