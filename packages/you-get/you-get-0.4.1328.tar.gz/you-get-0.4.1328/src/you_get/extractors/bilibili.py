...
class Bilibili(VideoExtractor):
    ...
    def download_playlist_by_url(self, url, **kwargs):
        ...
        if sort == 'video':
            ...
            doc_id = re.match(r'https?://h\.bilibili\.com/(\d+)', self.url).group(1)
            api_url = self.bilibili_album_api(doc_id)
            api_content = get_content(api_url, headers=self.bilibili_headers())
            doc_info = json.loads(api_content)
            urls = [i['img_src'] for i in doc_info['data']['item']['pictures']]
            # TBD
            # example: https://h.bilibili.com/9265499
            log.w(' '.join(urls))
