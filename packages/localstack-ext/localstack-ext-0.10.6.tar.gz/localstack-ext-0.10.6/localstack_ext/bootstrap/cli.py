...
def import_auth():
 try:
  ...
  raise vHJBz('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=vHJBN)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=vHJBd)
def get_params(args,names=[],mandatory=vHJBN):
 result=()
 for name in names:
  value=vHJBD
  if name in args:
  ...
  if not value and mandatory:
   raise vHJBz('Please provide %s=... parameter'%name)
  result+=(value)
 if vHJBM(result)==1:
  return result[0]
 return result
