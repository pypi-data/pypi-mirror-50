...
def import_auth():
 try:
  ...
  raise xHMAP('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=xHMAi)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=xHMAg)
def get_params(args,names=[],mandatory=xHMAi):
 result=()
 for name in names:
  value=xHMAO
  if name in args:
  ...
  if not value and mandatory:
   raise xHMAP('Please provide %s=... parameter'%name)
  result+=(value)
 if xHMAj(result)==1:
  return result[0]
 return result
