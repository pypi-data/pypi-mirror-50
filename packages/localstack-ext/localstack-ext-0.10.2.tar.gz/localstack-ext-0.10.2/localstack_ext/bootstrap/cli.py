...
def import_auth():
 try:
  ...
  raise Fgmow('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=FgmoE)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=Fgmoh)
def get_params(args,names=[],mandatory=FgmoE):
 result=()
 for name in names:
  value=Fgmoc
  if name in args:
  ...
  if not value and mandatory:
   raise Fgmow('Please provide %s=... parameter'%name)
  result+=(value)
 if Fgmoa(result)==1:
  return result[0]
 return result
