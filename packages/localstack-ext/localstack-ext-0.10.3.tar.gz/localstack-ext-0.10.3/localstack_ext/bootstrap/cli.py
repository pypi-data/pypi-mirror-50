...
def import_auth():
 try:
  ...
  raise khVfv('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=khVfK)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=khVfr)
def get_params(args,names=[],mandatory=khVfK):
 result=()
 for name in names:
  value=khVfg
  if name in args:
  ...
  if not value and mandatory:
   raise khVfv('Please provide %s=... parameter'%name)
  result+=(value)
 if khVfi(result)==1:
  return result[0]
 return result
