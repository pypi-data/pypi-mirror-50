...
def import_auth():
 try:
  ...
  raise YQMNo('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=YQMNr)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=YQMNS)
def get_params(args,names=[],mandatory=YQMNr):
 result=()
 for name in names:
  value=YQMNK
  if name in args:
  ...
  if not value and mandatory:
   raise YQMNo('Please provide %s=... parameter'%name)
  result+=(value)
 if YQMNF(result)==1:
  return result[0]
 return result
