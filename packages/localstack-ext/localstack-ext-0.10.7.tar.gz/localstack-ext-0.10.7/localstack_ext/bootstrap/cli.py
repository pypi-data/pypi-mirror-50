...
def import_auth():
 try:
  ...
  raise gWqkI('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=gWqkp)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=gWqkO)
def get_params(args,names=[],mandatory=gWqkp):
 result=()
 for name in names:
  value=gWqkl
  if name in args:
  ...
  if not value and mandatory:
   raise gWqkI('Please provide %s=... parameter'%name)
  result+=(value)
 if gWqke(result)==1:
  return result[0]
 return result
