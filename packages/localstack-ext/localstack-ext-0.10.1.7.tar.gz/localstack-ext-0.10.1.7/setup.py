...
# define install command hooks
class InstallLibCommand(install_lib):
    def run(self, *args, **kwargs):
        install_lib.run(self)

        print('install cmd is_alpine()', is_alpine(), args, kwargs, self.install_dir)

        if is_alpine():
            # Fetch libmusl compatible version of _pytransform.so
            target_file = os.path.join(self.install_dir, 'localstack_ext', '_pytransform.so')
