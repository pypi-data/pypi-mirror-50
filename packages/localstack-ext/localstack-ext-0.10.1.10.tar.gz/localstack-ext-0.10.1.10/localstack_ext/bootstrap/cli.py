...
def import_auth():
 try:
  ...
  raise obsxw('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=obsxK)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=obsxS)
def get_params(args,names=[],mandatory=obsxK):
 result=()
 for name in names:
  value=obsxG
  if name in args:
  ...
  if not value and mandatory:
   raise obsxw('Please provide %s=... parameter'%name)
  result+=(value)
 if obsxJ(result)==1:
  return result[0]
 return result
