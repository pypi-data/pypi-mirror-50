...
def import_auth():
 try:
  ...
  raise CkgBU('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=CkgBR)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=CkgBe)
def get_params(args,names=[],mandatory=CkgBR):
 result=()
 for name in names:
  value=CkgBL
  if name in args:
  ...
  if not value and mandatory:
   raise CkgBU('Please provide %s=... parameter'%name)
  result+=(value)
 if CkgBJ(result)==1:
  return result[0]
 return result
