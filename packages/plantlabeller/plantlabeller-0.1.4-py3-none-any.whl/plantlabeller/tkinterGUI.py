...
def manualeraser(inputimage):
    global temptif,panelA,modified_tif,plantchoice,tiflayers,w
    canvas_width=450
    canvas_height=450
    panelC=Toplevel()
    canvasframe=LabelFrame(panelC,text='manual eraser')
    ...
    w.create_image(0,0,image=inputimage,anchor=NW)
    okeybutton=Button(panelC,text='OK',command=partial(canvasok,panelC))
    restartbutton=Button(panelC,text='Reset',command=partial(canvasreset,inputimage))

    canvasframe.grid(row=0,column=0,columnspan=2)
    w.grid(row=0,column=0)
    okeybutton.grid(row=1,column=1)
    restartbutton.grid(row=1,column=0)
    CreateToolTip(canvasframe,'Erase areas that are merged together')
    CreateToolTip(okeybutton,'Save modification and exit')
    CreateToolTip(restartbutton,'Reset current image')
...
def kmeansclassify(bandchoice):
    ...
    CreateToolTip(genchbutton,'Click to see K-Means reaults with selected classes and open manual eraser window')
