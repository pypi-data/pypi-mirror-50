...
def resetrotate():
    global modified_tif,panelA,rotation
    rotation=0
...
def QGIS_NDVI():
    global panelA,panelB,RGBlabel,Inflabel,Heightlabel,panelTree,treelist
    if panelA is not None:
        panelA.destroy()
    if panelB is not None:
        panelB.destroy()
...
def export_result():
    ...
    if type(rotation)!=type(None):
        center=(225,225)
        M=cv2.getRotationMatrix2D(center,rotation*-1,1.0)
        border=cv2.warpAffine(border,M,dsize=(450,450),flags=cv2.INTER_LINEAR)
        labels=cv2.warpAffine(labels,M,dsize=(450,450),flags=cv2.INTER_LINEAR)
    if len(path)>0:
        ...
        for uni in colortable:
            lastone=np.where(floatlabels==float(uni),int(colortable[uni]),lastone)
        ...
        with open(filename,mode='w') as f:
            ...
            csvwriter.writerow(['Plot','Area(#pixels)','avg-NDVI','sum-NDVI','Length(#pixel)','Width(#pixel)'])#,'#holes'])
            for uni in datatable:
                row=[uni,datatable[uni][0],datatable[uni][1],datatable[uni][2],datatable[uni][3],datatable[uni][4]]#,
                     #datatable[uni][5]]
                print(row)
...
def calculatecustom(entbox):
    ...
    if bandname not in bandarrays:
        ...
        bandarrays.update(tempdict)
        worktempdict={"".join(equation):cv2.resize(tempband,(450,450),interpolation=cv2.INTER_LINEAR)}
        workbandarrays.update(worktempdict)
...
root=Tk()
root.title('PlantExtraction')
root.geometry('1000x500')

root.option_add('*tearoff',False)

top_frame=Frame(root,width=1000,height=50)
center=Frame(root,width=1000,height=410)
...
ctr_tree=Frame(center,width=250,height=410,padx=5,pady=5)
ctr_left=Frame(center,width=450,height=410)
ctr_right=Frame(center,width=450,height=410)
...
panelWelcome=Label(top_frame,text="Welcome to use PlantExtraction!\nPlease use the menubar on top to begin your image processing.",justify=CENTER)
