...
def filter_references(references, reference_lengths, limit_size):
    filter_references = list()
    filter_reference_lengths = list()
    for i_ref, i_length in zip(references, reference_lengths):
        if len(i_ref) < limit_size:
            filter_references.append(i_ref)
            filter_reference_lengths.append(i_length)
    return filter_references, filter_reference_lengths
...
def get_regions(ccd_name, references, lengths, segmentSize):
    segmentSize = int(segmentSize)
    if ccd_name:
        origin_region = read_ccd(ccd_name)
        return split_ccd(origin_region, segmentSize)
    else:
        return split_reference(references, lengths, segmentSize)


def gvc_pipeline(root, reference_filename, segmentSize, bam, outpath, sampleName, version, volumes, variants, rm_bam=False, rm_gvcs=True, dbsnp=None, ccd=None):
    Fasta = pysam.Fastafile(reference_filename)
    references = Fasta.references
    if segmentSize:
        region = get_regions(ccd, Fasta.references, Fasta.lengths, segmentSize)
        if ccd:
