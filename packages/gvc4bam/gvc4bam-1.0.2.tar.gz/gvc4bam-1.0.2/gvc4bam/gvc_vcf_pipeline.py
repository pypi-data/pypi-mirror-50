...
from gvc4bam  import bam_load_check
from gvc4bam import bed_check_sort_reg
from gvc4bam import gvc_check
from gvc4bam import gvc_pipeline
from gvc4bam import qc_pipeline
from gvc4bam import predict
from gvc4bam import vcf_oncodecode
from gvc4bam import cnv
import pysam
import copy
import itertools
...
def get_link_files(v_path):
    ...
    vpath_inner = [map(lambda x: os.path.join(root,x),files) for root,_,files in os.walk(v_path)]
...
def get_dynamic_volumes(*intput_paths):
    ...
    dirpaths =  map(lambda x: os.path.dirname(x) if os.path.isfile(x) else x,subpath)
...
def bam_vcf_pipeline(root, gvc_lib, reference, dbsnp, strategy, bed, outpath, group_info, mutantType, segmentSize,
                     sample_name, model_path,check_bam,enable_vcf_oncodecode = False , CNV_enable= True , SV_enable = True ,rm_temp=True,volumes=None):
    ...
    if  not volumes :
        if bed :
            volumes = get_dynamic_volumes(reference,model_path,gvc_lib,dbsnp,bed,outpath,*bam_list)
        else:
            volumes = get_dynamic_volumes(reference,model_path,gvc_lib,dbsnp,outpath,*bam_list)
        #print(gvc_lib)
    ...
    qc_txts = qc_pipeline.qc_pipeline(root,temp,outpath,strategy,bam_list,group_info.keys(),reference,bed_sort_file,sample_name,version,volumes)
    ...
    root = root.encapsulate()
    if 'Somatic' in mutantType :
        predict.post_filter(root,predict_dict,['snv','indel'],reference,bam_list[-1],sample_name,outpath,version,volumes)
        root = root.encapsulate()
    # vcf_oncodecode
...
if __name__ == "__main__":
    ...
    start_job = bam_vcf_pipeline(start_job, options.gvc_lib, options.reference, options.dbsnp, options.strategy, options.bed,
                                ...
                                options.sample_name,model_path, check_bam=True,CNV_enable=options.CNV,SV_enable=options.SV,rm_temp=options.rmtmp)
