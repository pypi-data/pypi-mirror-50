    ...
    def ary4x4(self, arg=None):
        if arg is not None:
            if np.linalg.matrix_rank(arg) < 3:
                ...
                self._origin3 = np.transpose(arg)[3][:3]
...
if __name__ == '__main__':
    ...
    a = np.hstack((np.vstack((a, [1, 2, 3])), [[0], [0], [0], [1]]))
    p = [1, 1, 1, 1]

    print(list(p @ a) == list(np.array(tomat4x4(a).MultiplyPoint(p))))

    cs = CS4x4()
    cs.translate([1, 2, 3])
    print(cs)
    print(cs.mpt([0, 0, 0]))
    cs.rotate(270, [0, 0, 1], [1, 0, 0])
    # cs.reflect([1, 1, 0], [1, 1, 0])
    print(cs.mpt([0, 0, 0]))
    cs1 = CS4x4(cs)
    print(cs.ary4x4())
