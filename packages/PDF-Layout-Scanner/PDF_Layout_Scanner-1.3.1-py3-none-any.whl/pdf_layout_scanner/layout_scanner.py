...
# Processing Pages
def _parse_pages(doc, images_folder, return_df=False, progressbar=False, separate_lines=False):
    """With an open PDFDocument object, get the pages and parse each one
    [this is a higher-order function to be passed to with_pdf()]"""
    rsrcmgr = PDFResourceManager()

    if separate_lines:
        laparams = LAParams(detect_vertical=True, all_texts=True, line_margin=0)
    else:
...
def get_pages(pdf_doc, pdf_pwd="", images_folder="/tmp", return_df=False, progressbar=False, separate_lines=False):
    """Process each of the pages in this pdf file and return a list of strings representing the text found in each page"""
    return with_pdf(pdf_doc, _parse_pages, pdf_pwd, images_folder, return_df, progressbar, separate_lines)
