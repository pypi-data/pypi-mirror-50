...
class Datasets:
    ...
    def create_sales(self):
        ...
        # Fixing merging errors (could be done better).
        animale_df["bodygarment"].replace(
            r"^None(.*)None", r"\1", regex=True, inplace=True
        )
        animale_df["bodygarment"].replace(
            r"^(.*)NoneNone", r"\1", regex=True, inplace=True
        )
        animale_df.drop(
    ...
    def scale_input(self, scaler=StandardScaler()):
        """
            Scales the model's input using any scaler.

            The scaler must be an object with at least two methods, :func:fit and 
            :func:transform.
        """
