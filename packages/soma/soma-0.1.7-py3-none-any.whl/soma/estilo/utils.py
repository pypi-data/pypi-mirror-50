    ...
    for col in tags:
        for cat in tags[col].unique():
            if cat:
                ...
                new_col = re.findall("([a-z]*)[A-Z]", cat)
                # Category value starts at the first uppercase letter.
                category_value = re.findall("([A-Z].+)", cat)
                if new_col:
                    new_col = new_col[0]
