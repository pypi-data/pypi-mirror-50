...
class _ResultBase:
    ...
    def plot_function(self, range=None, dim_indeces=None,
                      title='Function', logo=True, save_to=None):
        ''' Plot objective function'''
        if self._dim <= 0:
            print(sys.stderr, "Problem dimenstion is unknown: can't plot function")
            return
        ...
        if self._dim == 1:
            if not isinstance(range, list):
                ...
                print("Plot 1d function for range: %s" % range)
                plot.fobj_1d(self._fobj, range, title, logo, save_to)
            ...
            if not isinstance(range, list):
                ...
                print("Plot 2d function for range: %s" % range)
                if not dim_indeces:
                    dim_indeces = [0, 1]
                plot.fobj_2d(self._fobj, range[0], range[1],
                             self._dim, dim_indeces, title, logo, save_to)

    def plot_optimum(self, title='Optimum', eps=0.01,
                     dim_indeces=None, logo=True, save_to=None):
        ''' Plot optimum of found solution'''
        if self._dim <= 0:
            print(sys.stderr, "Problem dimenstion is unknown: can't plot optimum")
            return
        if self._dim == 1:
            print("Plot 1d optimum")
            plot.optimum_1d(title, self._fobj, self._solution, eps, logo, save_to)
        else:
            print("Plot 2d optimum")
            plot.optimum_2d(title, self._fobj, self._solution, eps,
                            self._dim, dim_indeces, logo, save_to)
