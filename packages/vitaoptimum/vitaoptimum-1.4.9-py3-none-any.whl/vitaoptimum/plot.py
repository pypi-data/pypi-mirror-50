...
def fobj_2d(fobj, solution, range_x, range_y, dim, dim_indices,
    ...
    ax.set_xlim(x[0], x[-1])
    ax.set_ylim(y[0], y[-1])
    ax.set_xlabel('x%s' % d0)
    ax.set_ylabel('x%s' % d1)
    ax.set_zlabel('fobj')
    ax.set_title(title)
...
def fobj_1d(fobj, range, title, logo=True, save_to=None):
    ...
    x = np.arange(range[0], range[1], 0.0001)  # TODO as option for user to change steps=1000 (with default value)
    y = fobj(x)
...
def optimum_1d(title, fobj, solution, eps, logo, save_to=None):
    ...
    x = np.arange(solution - eps, solution + eps, eps / 100)  # TODO as steps=1000 optional param with default value
    y = fobj(x)
...
def optimum_2d(title, fobj, solution, eps, dim, dim_indices, logo, save_to=None):
    '''Plot optimum area for 1d objective function (result: 2d graph)
    '''
    d0, d1 = _get_dimenstion_indeces(dim, dim_indices)
    x = np.arange(solution[d0] - eps, solution[d0] + eps, eps / 300)
    y = np.arange(solution[d1] - eps, solution[d1] + eps, eps / 300)
    X, Y = np.meshgrid(x, y)
