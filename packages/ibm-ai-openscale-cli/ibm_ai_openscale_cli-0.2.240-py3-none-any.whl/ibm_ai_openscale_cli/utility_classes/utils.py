...
def update_port_in_url(url, new_port):
    elements = urlparse(url)
    if not elements.scheme or not elements.hostname:
        error_msg = 'Invalid input, url must start with a valid scheme'
        logger.log_error(error_msg)
        raise Exception(error_msg)
    url_with_new_port = '{}://{}:{}'.format(elements.scheme, elements.hostname, new_port)
    if elements.path and len(elements.path) > 1:
        url_with_new_port = '{}{}'.format(url_with_new_port, elements.path)
    if elements.params:
        url_with_new_port = '{};{}'.format(url_with_new_port, elements.params)
    if elements.query:
        url_with_new_port = '{}?{}'.format(url_with_new_port, elements.query)
    if elements.fragment:
        url_with_new_port = '{}#{}'.format(url_with_new_port, elements.fragment)
    return url_with_new_port
