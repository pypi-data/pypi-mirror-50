    ...
    def wait_for_payload_logging(self, initial_pause=8):
        ...
        for pause in [16, 32, 64, 64, -1]: # -1 signals no more retries
            ...
            if self._model.expected_payload_row_count == actual_payload_rows:
                ...
                logger.log_error('Expecting {} rows in the payload table for this model, but {} rows already in table'.format(self._model.expected_payload_row_count, actual_payload_rows))
                break
            else:
                delaymsg = ''
                if pause > 0:
