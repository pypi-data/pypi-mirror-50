...
from ibm_ai_openscale_cli.utility_classes.utils import get_iam_headers, remove_port_from_url, update_port_in_url, get_port_from_url
...
class OpenScaleClient(OpenScaleReset):
    ...
    def generate_sample_scoring(self, engine_client, numscores, values_per_score, to_init_payload_logging=False):
        ...
        if self._args.ml_engine_type is MLEngineType.WML:
            ...
            if self._args.is_icp:
                is_wml_nginx_dep_url = WML_NGINX_HOST in deployment_url or WML_NGINX_HOST_V3 in deployment_url
                is_aios_nginx_dep_url = AIOS_NGINX_HOST in deployment_url
                is_internal_url = is_wml_nginx_dep_url or is_aios_nginx_dep_url
                if is_internal_url:
                    deployment_url = remove_port_from_url(deployment_url)
                    if is_aios_nginx_dep_url:
                        deployment_url = deployment_url.replace('https://', 'http://')
                else:
                    if 'apikey' not in self._ml_engine_credentials:
                        wml_port = get_port_from_url(self._ml_engine_credentials['url'])
                        deployment_url = update_port_in_url(deployment_url, wml_port if wml_port else 31843)
            ...
            for score_num in range(numscores):
                ...
                if self._args.is_icp:
                    ...
                    remote_cloud_wml = False
                    remote_icp_wml = False
                    if self._args.wml or self._args.wml_json:
                        if 'apikey' in self._ml_engine_credentials:
                            remote_cloud_wml = True
                        else:
                            remote_icp_wml = True
                    if remote_cloud_wml or remote_icp_wml:
    ...
    def wait_for_payload_logging(self, initial_pause=8):
        ...
        for pause in [16, 32, 64, 64]:
            ...
            if self._model.expected_payload_row_count == actual_payload_rows:
                ...
                time.sleep(pause)
        logger.log_warning('WARNING: unable to confirm all payloads are stored into the datamart database')
    ...
    def _reliable_trigger_fairness_check(self, background_mode):
        if 'fairness_configuration' not in self._model.configuration_data:
            ...
            try:
                ...
                logger.log_warning('WARNING: Problems occurred while running fairness check: {}'.format(str(e)))
    ...
    def _reliable_trigger_quality_check(self, background_mode):
        if 'quality_configuration' not in self._model.configuration_data:
            ...
            try:
                ...
                logger.log_warning('WARNING: Problems occurred while running quality check: {}'.format(str(e)))
    ...
    def _reliable_trigger_drift_check(self, background_mode):
        if 'drift_configuration' not in self._model.configuration_data:
            ...
            try:
                ...
                start = time.time()
                self._subscription.drift_monitoring.run(background_mode=background_mode)
                elapsed = time.time() - start
                logger.log_debug('TIMER: subscription.drift_monitoring.run in {:.3f} seconds'.format(elapsed))
                logger.log_info('Drift check triggered successfully')
            except Exception as e:
                logger.log_warning('WARNING: Problems occurred while running drift check: {}'.format(str(e)))
    ...
    def reliable_post_drift_metrics(self, start_time, end_time, windows=8): # eight 3-hour drift windows in 24 hours
        ...
        start = time.time()
        response = requests.post(drift_tasks_url, json={}, headers=iam_headers, verify=self._verify)
        elapsed = time.time() - start
    ...
    def generate_explain_requests(self):
        ...
        for i in range(num_explains):
            ...
            try:
                ...
                logger.log_warning('WARNING: Problems occurred while running explanation: {}'.format(str(e)))
