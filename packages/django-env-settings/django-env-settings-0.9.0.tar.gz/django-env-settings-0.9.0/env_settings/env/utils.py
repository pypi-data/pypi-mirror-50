...
def get_bool(name: str, default: bool = None) -> bool:
...
def allowed_hosts() -> List[str]:
    if env('DJANGO_ALLOWED_HOSTS'):
        return str(env('DJANGO_ALLOWED_HOSTS')).strip('"').split()
    elif is_debug():
        return ['*']
    else:
...
def is_dev() -> bool:
    return env('DJANGO_ENV') == DjangoEnv.DEV.value
...
def is_debug() -> bool:
    if env('IS_DJANGO_DEBUG') is None:
        ...
        return get_bool('IS_DJANGO_DEBUG')
