...
class asfpyLDAPException(Exception):
...
class manager:
    ...
    def __init__(self, user, password, host = LDAP_URI):
        ...
        if not re.match(r"^[-_a-z0-9]+$", user):
            raise asfpyLDAPException("Invalid characters in User ID. Must be alphanumerical or dashes only.")
        ...
        try:
            ...
            raise asfpyLDAPException("Invalid username or password supplied!")
        except ldap.TIMEOUT:
            raise asfpyLDAPException("The backend authentication server timed out, please retry later.")
        except:
            raise asfpyLDAPException("An unknown error occurred, please retry later.")
        ...
        try:
            ...
            raise asfpyLDAPException("Common backend assertions failed, LDAP corruption?")
    ...
    def create_account(
        ...
        if self.load_account(uid):
            raise asfpyLDAPException("An account with this uid already exists")
        
        uidnumber = self.nextUid()
        if not self.isAdmin:
            raise asfpyLDAPException("You do not have sufficient access to create accounts")
