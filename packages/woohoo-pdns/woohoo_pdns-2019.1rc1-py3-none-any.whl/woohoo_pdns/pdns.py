...
class Database(object):
    ...
    def add_record(self, rrtype, rrname, rrdata, first_seen=None, last_seen=None,  num_hits=1):
        """
        Add a (new) record to the database.

        If a record with that rrtype, rrname, rdata already exists in the database, the hitcount is increased by one,
        first_seen or last_seen are updated if necessary and the existing object is returned. Otherwise a new object
        will be created and returned (with hitcount 1, fist_seen = last_seen = sighted_at (or "now" if sighted_at is not
        provided)).
        ...
        Args:
            ...
            rrdata (string): the "right hand side" of the record; a trailing dot will be removed
        ...
        """
        r_dict = self._prepare_record_dict(rrtype, rrname, rrdata, first_seen=first_seen, last_seen=last_seen,
                                           num_hits=num_hits)
        r_dict_value = list(r_dict.values())[0]
        self._l.debug("r_dict_value: {}".format(r_dict_value))

        new_record = Record(
            rrtype=r_dict_value["rrtype"],
            rrname=r_dict_value["_rrname"],
            rrdata=r_dict_value["_rdata"],
            first_seen=r_dict_value["first_seen"],
            last_seen=r_dict_value["last_seen"],
            hitcount=r_dict_value["hitcount"]
        )
        try:
            # self._l.debug("Starting find for {} {}".format(rrname, rdata))
            record = self.find_record(rrtype, rrname, rrdata=rrdata)
    ...
    def find_record(self, rrtype, rrname, rrdata=None):
        """
        ...
        Args:
            ...
            rrdata (string): The "right hand side" of the record.
        ...
        """
        self._l.debug("find_record called: rrype '{}', rrname '{}', rrdata '{}'".format(rrtype, rrname, rrdata))
        try:
            if not rrdata:
    ...
    def load(self, source_name, batch_size=10000, cfg=None, data_timezone="UTC", strict=False,
        ...
        while importer.has_more_data:
            entries_batch = importer.load_batch(batch_size)
