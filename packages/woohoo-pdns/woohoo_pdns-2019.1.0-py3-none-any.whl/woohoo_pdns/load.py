...
class SilkFileImporter(FileImporter):
    ...
    def _tokenise_record(self, rec):
        ...
        try:
            ...
            last_seen = tokens[1]
            rrtype = int(tokens[2])
            rrname = tokens[3].strip()
            hitcount = int(tokens[4])
            rdata = tokens[5].strip()
        except ValueError:
            return None
...
class DNSLogFileImporter(FileImporter):
    ...
    def _parse_tokenised_record(self, tokenised_rec):
        ...
        """
        rec = tokenised_rec[0]
        first_seen = datetime.utcfromtimestamp(int(rec.first_seen))
        last_seen = datetime.utcfromtimestamp(int(rec.last_seen))
    ...
    def _tokenise_record(self, rec):
        ...
        for ans in y["message"]["response_message_data"]["ANSWER_SECTION"]:
            try:
                ...
                entry = record_data(first_seen, last_seen, rrtype, rrname, 1, rdata, None)
