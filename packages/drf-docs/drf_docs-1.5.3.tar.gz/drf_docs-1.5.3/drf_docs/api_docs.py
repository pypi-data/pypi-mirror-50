...
from drf_docs.api_endpoint import ApiEndpoint
...
class ApiDocumentation(object):
    ...
    def get_all_view_names(self, urlpatterns, parent_pattern=None):
        for pattern in urlpatterns:
            if isinstance(pattern, URLResolver):
                if "admin" in pattern.pattern._regex:
                    continue
                parent_pattern = None if pattern.pattern._regex.split(pattern.namespace)[0] == "^" else pattern
    ...
    def _is_format_endpoint(self, pattern):
        ...
        return '?P<format>' in pattern.pattern._regex
