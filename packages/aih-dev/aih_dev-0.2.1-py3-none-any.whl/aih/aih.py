...
class AIH:
    ...
    def upload_images(self, fullname, images):
        try:
            path = f'{self.__url}/sagemaker/datas'
    ...
    def upload_cropped_faces(self, fullname, images):
        try:
            path = f'{self.__url}/sagemaker/datas?only_face=true'
            files = map(lambda image: ('file', image), images)
            res = requests.patch(path, data={"fullname": fullname}, files=files, auth=TokenAuth(self.__token))
            return res.json()
        except Exception as e:
    ...
    def train_model(self):
        try:
            ...
            res = requests.post(path, auth=TokenAuth(self.__token))
            return res.json()
        except Exception as e:
            raise e
    ...
    def stop_training(self, model_id):
        try:
            path = f'{self.__url}/sagemaker/models/{model_id}'
            res = requests.delete(path, auth=TokenAuth(self.__token))
            return res.json()
        except Exception as e:
            raise e
