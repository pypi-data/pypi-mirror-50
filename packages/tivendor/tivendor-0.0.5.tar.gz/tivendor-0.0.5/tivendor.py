...
class TiVendor:
    ...
    def request(self, api_num, service_name='', ti_request_headers=None, **kwargs):
        ...
        response = requests.request(method, url, timeout=60, verify=False, **kwargs)
...
def kafka_push(**kwargs):
    ...
    body = {
        ...
        'triple_url': url,
        'triple_host': '%s:%s' % (url_parse.hostname, url_parse.port) if url_parse.port else url_parse.port,
        'method': method,
        'request': {
            'headers': json.dumps(dict(response.request.headers)),
            'params': kwargs.get('params', ''),
            'body': json.dumps(kwargs.get('json', {})) or kwargs.get('data', ''),
        },
        'response': {
