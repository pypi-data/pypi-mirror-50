...
def save_model(model, name):
    ...
    model_meta_dict = OrderedDict([(mmc.MMT_COL_model_name, name),
                                   (mmc.MMT_COL_model_created_by, current_user),
                                   (mmc.MMT_COL_model_engine, __get_model_engine(model)),
                                   (mmc.MMT_COL_model_class, __get_model_class(model)),
                                   (mmc.MMT_COL_model_created_at, str(pd.to_datetime(time.time(), unit='s'))),
                                   (mmc.MMT_COL_model_creation_lang, "Python"),
                                   (mmc.MMT_COL_model_mode, "private")])
    ...
    #
    try:
        copy_to_sql(df=model_meta_pdf, table_name=mmc.MODELS_METADATA_TABLE_NAME, schema_name=_get_context_temp_databasename())
    except:
    ...
    try:
        copy_to_sql(df=model_object_pdf, table_name=mmc.MODELS_OBJECT_TABLE_NAME, schema_name=_get_context_temp_databasename())
    except:
...
def list_models(name = None, function_name = None):
    ...
    if name is not None:
        ...
        filter_expression = __get_like_filter_expression_on_col(models_meta_df._metaexpr, mmc.MMT_COL_model_name, name)
    ...
    if function_name is not None:
        ...
        # This is case-insensitive look-up.
        filter_expression = __get_like_filter_expression_on_col(models_meta_df._metaexpr, mmc.MMT_COL_model_class,
                                                                function_name)
        models_meta_df = models_meta_df.__getitem__(filter_expression)

    display_df = models_meta_df.select([mmc.MMT_COL_model_name, mmc.MMT_COL_model_engine,
                                        mmc.MMT_COL_model_class, mmc.MMT_COL_model_created_at])
...
def describe_model(name):
    ...
    print(models_meta_df.select([mmc.MMT_COL_model_name, mmc.MMT_COL_model_engine, mmc.MMT_COL_model_class,
                                 mmc.MMT_COL_model_created_at]))

    # Print Model Attributes.
    print("\n*** '{}' Model Attributes ***".format(name))
    print(display_df[display_df[mmc.MMOT_COL_object_index] == -1].select(
        [mmc.MMOT_COL_object_name, mmc.MMOT_COL_attribute_value]))

    # Print Model Output Tables.
    print("\n*** '{}' Model Output Tables ***".format(name))
    print(display_df[display_df[mmc.MMOT_COL_object_index] != -1].select(
        [mmc.MMOT_COL_object_name, mmc.MMOT_COL_output_table_name]))
...
def delete_model(name):
    ...
    models_output_df = models_output_df[models_output_df[mmc.MMOT_COL_object_index] != -1].select(
        [mmc.MMOT_COL_output_table_name]).get_values()
...
def retrieve_model(name):
    ...
    model_engine_class_info = models_meta_df.select([mmc.MMT_COL_model_engine, mmc.MMT_COL_model_class]).get_values()
...
def __check_if_model_exists(name=None, raise_error_if_exists=False, raise_error_if_model_not_found=False):
    ...
    models_meta_df = DataFrame(in_schema(_get_context_temp_databasename(), mmc.MODELS_METADATA_TABLE_NAME))
...
def __check_if_model_management_tables_exists_or_not(raise_error_if_does_not_exists=False):
    ...
    model_table_exists = conn.dialect.has_table(conn, table_name=mmc.MODELS_METADATA_TABLE_NAME,
                                                schema=_get_context_temp_databasename())
    model_output_table_exists = conn.dialect.has_table(conn, table_name=mmc.MODELS_OBJECT_TABLE_NAME,
                                                       schema=_get_context_temp_databasename())
...
def __create_model_metadata_table(conn):
    ...
    MODELS_TABLE_DDL = "CREATE MULTISET TABLE {0} (" \
                       ...
                       "    PRIMARY KEY ({1},{2}))".format(__get_models_metadata_table_name(), mmc.MMT_COL_model_name,
                                                           mmc.MMT_COL_model_created_by, mmc.MMT_COL_model_engine,
                                                           mmc.MMT_COL_model_class, mmc.MMT_COL_model_created_at,
                                                           mmc.MMT_COL_model_creation_lang, mmc.MMT_COL_model_mode)
...
def __create_model_output_table(conn):
    ...
    MODELS_OBJECT_TABLE_DDL = "CREATE TABLE {0} (" \
                              ...
                              "     REFERENCES {1} ({2}, {3}))".format(__get_models_output_table_name(),
                                                                       ...
                                                                       mmc.MMT_COL_model_name, mmc.MMT_COL_model_created_by,
                                                                       mmc.MMOT_COL_object_name, mmc.MMOT_COL_object_index,
                                                                       mmc.MMOT_COL_output_table_name,
                                                                       mmc.MMOT_COL_attribute_value)
...
def __delete_specific_row_by_model_name(tablename, name):
    ...
    delete_row_query = SQLBundle._build_delete_spcific_rows_from_table(tablename, "{0} = '{1}' and {2} = '{3}'".format(
        mmc.MMT_COL_model_name, name, mmc.MMT_COL_model_created_by, getpass.getuser()))
...
def __get_models_metadata_table_name():
    ...
    EXAMPLES:
        __get_models_metadata_table_name()
    """
    return "{}.{}".format(_get_context_temp_databasename(), mmc.MODELS_METADATA_TABLE_NAME)
...
def __get_models_output_table_name():
    ...
    """
    return "{}.{}".format(_get_context_temp_databasename(), mmc.MODELS_OBJECT_TABLE_NAME)
...
def __get_model_output_attributes(models_meta_df):
    ...
    try:
        ...
        models_output_df = DataFrame(in_schema(_get_context_temp_databasename(), mmc.MODELS_OBJECT_TABLE_NAME))
    except:
        raise

    # Join both Model Administration tables on model_name and created_by user and return the results.
    return models_meta_df.join(models_output_df, on=[mmc.MMT_COL_model_name, mmc.MMT_COL_model_created_by],
...
def __read_model_attributes_and_values(models_meta_df):
    ...
    model_attributes_df = model_attribute_output_df[model_attribute_output_df[mmc.MMOT_COL_object_index] == -1].select(
        [mmc.MMOT_COL_object_name, mmc.MMOT_COL_attribute_value]).get_values()
    ...
    model_attributes_df = model_attribute_output_df[model_attribute_output_df[mmc.MMOT_COL_object_index] != -1].select(
        [mmc.MMOT_COL_object_name, mmc.MMOT_COL_output_table_name]).get_values()
...
def __retrieve_model_attributes_info(model, name):
    ...
    _model_attribute_dict = OrderedDict([(mmc.MMT_COL_model_name, []),
                                         (mmc.MMT_COL_model_created_by, []),
                                         (mmc.MMOT_COL_object_name, []),
                                         (mmc.MMOT_COL_object_index, []),
                                         (mmc.MMOT_COL_output_table_name, []),
                                         (mmc.MMOT_COL_attribute_value, [])])
    ...
    for key in model.__dict__.keys():
        ...
        if not key.startswith('_'):
            ...
            _model_attribute_dict[mmc.MMT_COL_model_name].append(name)
            _model_attribute_dict[mmc.MMT_COL_model_created_by].append(getpass.getuser())
            _model_attribute_dict[mmc.MMOT_COL_object_name].append(key)
            _model_attribute_dict[mmc.MMOT_COL_object_index].append(object_index)
            _model_attribute_dict[mmc.MMOT_COL_output_table_name].append(output_table_name)
            _model_attribute_dict[mmc.MMOT_COL_attribute_value].append(attribute_value)
