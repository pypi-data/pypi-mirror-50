...
class DLembedder:
    ...
    def getNodeEnrichment(self, node_int, genelist_ints, num_permutations=100000, random_state=42, include_pval=True,
        ...
        if agg_score.lower() == 'prob':
            score = np.prod(probs[genelist_ints])**(1/len(genelist_ints))
        else:
            score = np.mean(probs[genelist_ints])/np.sum(probs)


        other_ints = np.setdiff1d(np.arange(self.voc_size), genelist_ints)

        if testtype == 'ks':
            D, p_val = ks_2samp(probs[other_ints], probs[genelist_ints])
        else:
            U, p_val = mannwhitneyu(probs[genelist_ints], probs[other_ints], alternative='both')
        '''
        if include_pval:
    ...
    def runDiscreteEnrichment(self, pathways_ints, genelist_ints, Nperm=10000,
        ...
        if testtype == 'mannwhitney':
            ...
            output_dict['Pval'] = [mannwhitneyu(probs_list[i, :], all_probs[i, :][other_genes], alternative='greater')[1]
                                   for i in range(len(pathways_ints))]
        elif testtype == 'ks':
            other_genes = np.setdiff1d(gene_ints, genelist_ints)
            output_dict['Pval'] = [ks_2samp(probs_list[i, :], all_probs[i, :][other_genes])[1]
                                   for i in range(len(pathways_ints))]
        elif testtype == 'permute':
            shuffles = score
