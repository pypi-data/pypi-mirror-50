...
def combine_simulation_files (iterations, output_path, chromosomes, samples):
	'''
	...
	'''
	for i in iterations:
		with open(output_path + str(i) + ".vcf", "wb") as f:
			for chrom in chromosomes:
				with open(output_path + str(i) + "_" + chrom + ".vcf",'rb') as fd:
					shutil.copyfileobj(fd, f)
				os.remove(output_path  + str(i) + "_" + chrom + ".vcf")
	...
	for chrom in chromosomes:
		...
		for sample in sample_names:
			...
			while(simulations > 0):
				...
				if bed:
					outputFile = ''.join([sample_path,str(simulations),"_",chrom,"_BED.vcf"])
				else:   
					outputFile = ''.join([sample_path,str(simulations),"_",chrom,".vcf"])
				# if bed:
