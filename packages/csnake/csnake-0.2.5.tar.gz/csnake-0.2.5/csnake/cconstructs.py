...
class FuncPtr:
    ...
    def __init__(
        ...
        self.arguments = []  # type: List[Variable]
...
class Variable:
    ...
    def __init__(
        ...
        if isinstance(primitive, FuncPtr):
            self.primitive = primitive  # type: Union[FuncPtr, str]
        else:
    ...
    def generate_initialization(
        ...
        if isinstance(value, (CArrayLiteral, CStructLiteral)):
            ...
            formatters = LiteralFormatters()
            assignment = generate_c_value_initializer(  # type: ignore
                value, formatters
            )
...
class Function:
    ...
    def add_code(self, code: Union[str, Iterable[str]]) -> None:
        ...
        If `code` is an iterable, it should be one string - one line. Since a
        :class:`CodeWriter` is an iterable of :obj:`str`\\ s , you can simply add
        its contents to the function.
...
class Enum:
    ...
    def __init__(
        ...
        self.typedef = bool(typedef)
        # enum values
        self.values = []  # type: List[Enum.EnumValue]
        self.name = assure_str(name)
