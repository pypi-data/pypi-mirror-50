...
class Client():
    ...
    def dataset(self, dataset_id, version=None):
        ds = Dataset(self.token, dataset_id, version=version, url=self.url, client=self)
        self.data_sources.append(ds)
        return ds

    def space(self, version=None, **kwargs):
        sp = Space(self.token, url=self.url, version=version, **kwargs)
...
class Dataset():
    def __init__(self, token, dataset_id, version=None, url='', client=None):
        self.token = token
