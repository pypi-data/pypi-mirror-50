...
def add_partitions(
    ...
    for partition in partition_paths:
        if file_format == "parquet":
            partition_def = parquet.get_partition_definition(partition)
        elif file_format == "csv":
            partition_def = csv.get_partition_definition(partition)
        else:
...
def create_table(
    ...
    client = get_session(session_primitives=session_primitives).client("glue")
    if file_format == "parquet":
        table_input = parquet.get_table_definition(table, partition_cols, schema, path)
    elif file_format == "csv":
        table_input = csv.get_table_definition(table, partition_cols, schema, path)
    else:
        raise UnsupportedFileFormat(file_format)
