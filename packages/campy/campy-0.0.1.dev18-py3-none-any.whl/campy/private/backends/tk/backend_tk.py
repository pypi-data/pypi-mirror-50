...
class TkBackend(GraphicsBackendBase):
    ...
    def event_add_mouse_handler(self, event, handler):
        ...
        if event == MouseEventType.MOUSE_CLICKED:
            win._master.bind('<ButtonPress-1>', lambda cb_event: handler(self._wrap_mouse_event(cb_event, win, event)))
        ...
        elif event == MouseEventType.MOUSE_ENTERED:
            win._master.bind('<Enter>', lambda cb_event: handler(self._wrap_mouse_event(cb_event, win, event)))
        elif event == MouseEventType.MOUSE_EXITED:
            win._master.bind('<Leave>', lambda cb_event: handler(self._wrap_mouse_event(cb_event, win, event)))
        elif event == MouseEventType.MOUSE_DOUBLE_CLICKED:
            win._master.bind('<Double-ButtonPress-1>', lambda cb_event: handler(self._wrap_mouse_event(cb_event, win, event)))
        elif event == MouseEventType.MOUSE_TRIPLE_CLICKED:
            win._master.bind('<Triple-ButtonPress-1>', lambda cb_event: handler(self._wrap_mouse_event(cb_event, win, event)))
        else:
            logger.warning('Unrecognized event type: {}. Quietly passing.'.format(event))

    def event_mouse_generate(self, style, x, y, window, warp):
        from campy.gui.events.mouse import MouseEventType
        if not window:
            try:
        ...
        # Since the registered callbacks have closed around the appropriate window, just setting x and y is sufficient.
        if style == MouseEventType.MOUSE_PRESSED:
            event = '<Button-1>'
        elif style == MouseEventType.MOUSE_RELEASED:
            event = '<ButtonRelease-1>'
        elif style == MouseEventType.MOUSE_MOVED:
            event = '<Motion>'
        elif style == MouseEventType.MOUSE_DRAGGED:
            event = '<B1-Motion>'
        else:
            logger.warning('Unrecognized event type: {}. Quietly passing.'.format(event))

        window._master.event_generate(event, x=x, y=y, warp=warp)
