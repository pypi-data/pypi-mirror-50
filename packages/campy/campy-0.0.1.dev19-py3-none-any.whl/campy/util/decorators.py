...
def curryable(decorator):
    """Wrap a decorator into one that curries its non-function arguments.

    Usually, a decorator either has to
        ...
        foo = dec(foo)
        foo = dec(foo, x=1, y=2, a=3, z=4, b=5)
    """
    @functools.wraps(decorator)
    def wrapper(function=None, *args, **kwargs):
        if not function:
