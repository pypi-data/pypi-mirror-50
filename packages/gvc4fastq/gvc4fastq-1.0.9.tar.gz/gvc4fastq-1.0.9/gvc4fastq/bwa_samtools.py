...
class Map(Job):
    ...
    @decorator_wrapt
    def __init__(self, R1file, R2file, bwa_threads, ref, samtoolst, sampleName, outpath, image, volumes, *args, **kwargs):
        self.R1file = R1file
        self.R2file = R2file
        ...
        self.commandLine = ['bash', '/bin/bwa_samtools.sh', self.R1file, self.R2file,
                            self.bwa_threads, self.ref, self.samtoolst, self.outpath, self.outfile]
        ...
        if os.path.exists(self.READY_FILE) is False:
            ...
            if os.path.exists(self.READY_FILE) is False:
                docker_runner_method(self, self.commandLine,
                                     "BwaShm", self.image, self.volumes, ipc_mode="host")
...
class Duplicate(Job):
    ...
    def run(self, fileStore):
        docker_runner_method(self, self.commandLine,
                             "Duplicate", self.image, self.volumes)
        #os.remove(self.bam)
...
def mapping_pipeline(root, R1file, R2file, ref, outpath, sampleName, version, volumes, max_threads):
    bwa_threads =  int(round(max_threads * 0.75))
    sort_threads = int(round(max_threads * 0.75))
    dup_threads =  4
    index_threads = int(round(max_threads* 0.5))
    if (os.path.exists(ref + '.bwt') and os.path.exists(ref + '.pac') and os.path.exists(ref + '.sa') and os.path.exists(ref + '.amb')) is False:
    ...
    map_job = Map(R1file, R2file, str(bwa_threads), ref, str(sort_threads), sampleName, outpath,
                  version['gmap'], volumes, cores=int(max_threads))
    BwaShm_job.addChild(map_job)

    i_sort = sortbamInterface.SortBamInterface()
    i_sort.append_bam(map_job.get_bam_name())
    sort_index_job = IndexBam(map_job.get_bam_name(), version['gmap'], volumes, threads=str(index_threads), cores=int(index_threads))
    map_job.addChild(sort_index_job)
    dup_job = Duplicate(map_job.get_bam_name(), outpath + '/' + sampleName + '.sort.dup.bam',
                        version['duplication'], volumes, '12', cores=int(dup_threads))
    sort_index_job.addChild(dup_job)
    index_job = IndexBam(dup_job.get_bam_name(
    ), version['gmap'], volumes, threads=str(index_threads), cores=int(index_threads))
...
if __name__ == "__main__":
    ...
    map_job = mapping_pipeline(start_job, options.R1, options.R2, options.ref,
                               options.outpath, options.sampleName, version, volumes, threads)
