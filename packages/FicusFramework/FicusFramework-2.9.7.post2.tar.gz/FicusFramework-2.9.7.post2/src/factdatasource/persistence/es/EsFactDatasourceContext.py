    ...
    def _single_thread_inserts(self, table: str, result_list: list):
        ...
        try:
            ...
            for index, result in enumerate(result_list):
                ...
                # 最后一个再刷新
                if index < last_index:
                    self.dao.create(index=table, doc_type=self.DEFAULT_INDEX_TYPE, id=primary_values_key, body=result, refresh='wait_for')
                else:
                    self.dao.create(index=table, doc_type=self.DEFAULT_INDEX_TYPE, id=primary_values_key, body=result, refresh=True)
        except Exception as e:
    ...
    def _single_thread_updates(self, table: str, result_list: list):
        ...
        try:
            ...
            for index, result in enumerate(result_list):
                ...
                self.dao.partial_update(index=table, doc_type=self.DEFAULT_INDEX_TYPE, id=primary_values_key, body=result, refresh=True)
        except Exception as e:
    ...
    def _single_thread_inserts_or_updates(self, table: str, result_list: list):
        ...
        try:
            ...
            for index, result in enumerate(result_list):
                ...
                # 最后一个再刷新
                if index < last_index:
                    self.dao.upsert(index=table, doc_type=self.DEFAULT_INDEX_TYPE, id=primary_values_key, body=result, refresh='wait_for')
                else:
                    self.dao.upsert(index=table, doc_type=self.DEFAULT_INDEX_TYPE, id=primary_values_key, body=result, refresh=True)
        except Exception as e:
