...
def update_docker_image(image_name, old_version=None, new_version=None, cmds=None):
    ...
    pure_image_name = image_name.split('/')[-1].strip()
    print_with_color('try to load %s files'%pure_image_name, 'greed')
    try:
        image_files_module = importlib.import_module('xserver.docker_image.files.%s'%pure_image_name)
        default_docker_file_content = load_files_data(image_files_module.files_data)
        print_with_color('%s files are loaded'%pure_image_name, 'greed')
    except:
        pass
