...
class StartModule():
	...
	def crawl(self, url=None, depth=100, proxyhost=None, proxyuser=None, proxypassword=None, proxyport=None, proxysecure="http"):
		...
		try:
			...
			webForms[url] = storeWebSiteForms(url)
			...
			try:
				for link in links:
					...
					#process.start()	
					#process.join()
					handleLink(link=link, depth=depth, webForms=webForms, proxies=proxies, linksVisited=linksVisited, totalProfundity=totalProfundity)				
					totalProfundity = 0
			finally:
				pass
		...
		if ('href' in dict(link.attrs) and "http" in link['href']):
			try:
				...
				if totalProfundity <= depth:
					...
					webForms[link['href']] = storeWebSiteForms(link['href'])
					for sublink in depthLinks:
						#processLink = multiprocessing.Process(target=self.handleLink, args=[sublink])
						#processLink.daemon = True
						#processLink.start()
						handleLink(link=sublink, depth=depth, webForms=webForms, proxies=proxies, linksVisited=linksVisited, totalProfundity=totalProfundity)
				else:
					totalProfundity -= 1
					return
			except:
	...
	def storeWebSiteForms(self, url, proxies=proxies, forms={}):
