...
config_utils = Config.getConfig(parentKey='modules', key='Utils', subkey='dictionaries')
...
def saveFileFromHtmlForm(myfile, module_name, category):
    try:
        location = os.path.join("core", "library", "hackingtools", "modules", category, module_name, "output")
...
def randomText(length=8, alphabet='lalpha-numeric'):
    try:
        return ''.join([random.choice(list(config_utils[alphabet])) for _ in range(len(int(length)))])
