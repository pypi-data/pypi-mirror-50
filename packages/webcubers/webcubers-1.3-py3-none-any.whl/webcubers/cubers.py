...
def arg_snippets(args):
    ...
    for snippet in snippets['data']:
        snippets_list.append(f'{snippet["id"]}. {snippet["filename"]} ( {snippet["title"]} )')
    
    questions = [
        inquirer.List('snippet',message="Select target snippet", choices=snippets_list),
    ]
    target_snippet_id = inquirer.prompt(questions)['snippet'].split('.')[0]
    download_snippet(target_snippet_id)
