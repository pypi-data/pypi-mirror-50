...
class LiabilitiesDetails(BoolDataBase):
    ...
    df_default_header = OrderedDict({'name': '姓名',
                                     'phone': '手机号',
                                     'ident_number': '身份证号',
                                     'cflenders': '消费金融类机构数',
...
def batch_test_apply_detail(input: str,
    ...
    :return: None
    """
    column = columns or ApplyDetail.df_default_header
    ...
    if input_is_str:
        ...
        excel_ins: pd.DataFrame = Excel(input).read(names=names).dataframe if isinstance(input, str) else input
    ...
    for _, series in excel_ins.dataframe.iterrows():
        ...
        guard_result.rename(columns=column, inplace=True)
...
def batch_test_overdue_detail(input: str,
    ...
    :return: None
    """
    columns = columns or OverdueDetail.df_default_header
    error_log = error_log or File.join_path(File(input).dirname, '.'.join((File(input).pure_name, 'error_log')))
    if input_is_str:
        print('batch_test_apply_detail excel_ins', '*' * 10, input, names, '*' * 10)
    ...
    for _, series in excel_ins.dataframe.iterrows():
        ...
        if not guard_ins.is_succeed():
            ...
            with open(error_log, mode='a') as f:
                ...
                f.write(f"{series['idcard']},{series['name']},{series['phone']}\n")
        guard_result = guard_ins.result_as_df_partial()
        guard_result.rename(columns=columns, inplace=True)
        result_df = result_df.append(guard_result)
        # break
...
def batch_test_repayment_detail(input: str,
    ...
    :return: None
    """
    columns = columns or RepaymentDetail.df_default_header
    error_log = error_log or File.join_path(File(input).dirname, '.'.join((File(input).pure_name, 'error_log')))
    if input_is_str:
        print('batch_test_apply_detail excel_ins', '*' * 10, input, names, '*' * 10)
        excel_ins: pd.DataFrame = Excel.prepare_file(input).read(names=names, skiprows=0)
...
def batch_test_liabilities_detail(input: str,
    ...
    :return: None
    """
    columns = columns or LiabilitiesDetails.df_default_header
    error_log = error_log or File.join_path(File(input).dirname, '.'.join((File(input).pure_name, 'error_log')))
