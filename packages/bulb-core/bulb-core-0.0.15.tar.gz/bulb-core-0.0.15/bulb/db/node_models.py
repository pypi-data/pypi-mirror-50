                ...
                for key, value in object_properties_dict.items():
                    if field_name == key:
                        ...
                        # Date handling.
                            ...
                            render.append(f"{key}: date('{value}')")

                        # Time handling.
                        elif isinstance(value, datetime.time):
                            render.append(f"{key}: time('{value}')")
    ...
    def update(self, property_name, new_property_value):
        ...
        if not settings.BULB_CREATE_PROPERTY_IF_NOT_FOUND and property_name not in self.__dict__.keys():
            ...
            # Datetime handling.
                ...
                print("DATETIME IS FOUND IN UPDATE")
                gdbh.w_transaction("""
                MATCH (n:%s {uuid:"%s"})
                WHERE exists(n.%s)
                SET n.%s = datetime("%s")
