                    ...
                    if value is not None:
                        ...
                        if key == field_name:
                            ...
                            if eval(f"node_or_rel_object.__class__.{key}.sftp"):
                                ...
                                setattr(node_or_rel_object, key, callable_result)
                                current_object_properties_dict[key] = callable_result
...
class Node(BaseNode):
    ...
    @classmethod
    def get(cls, uuid=None, order_by=None, limit=None, skip=None, desc=False, only=None):
        """
    ...
    def update(self, property_name, new_property_value):
        ...
        if not settings.BULB_CREATE_PROPERTY_IF_NOT_FOUND and property_name not in self.__dict__.keys():
            ...
            # Integer, boolean and list handling
            elif isinstance(new_property_value, int) or isinstance(new_property_value, bool) or isinstance(new_property_value, list):
                gdbh.w_transaction("""
                MATCH (n:%s {uuid:"%s"})
                WHERE exists(n.%s)
                SET n.%s = %s
...
class Relationship(BaseNode):
                ...
                # Support string syntax.
                    ...
                    if self.start == "self":
                        ...
                        for node_model in all_node_models:
                            if node_model.__name__ == self.start:
        ...
        if instance is not None:
            ...
            if self.direction == "from" or self.direction == "bi":
                ...
                if self.target is not None:
                    ...
                    if self.target in other_node_instance.__class__.__mro__:
                ...
                if self.start is not None:
                    if self.start in other_node_instance.__class__.__mro__:
    ...
    def get(self, direction="bi", returned="node", order_by=None, limit=None,  skip=None, desc=False, distinct=False,
        ...
        :param direction: Must be "from", "to", or "bi". If it is "from", the research will be focused on all the
                          relationships that have as start point the self node_model's instance. If it is "bi", the 
...
class RelationshipInstance:
    ...
    def update(self, property_name, new_property_value):
        ...
        if not settings.BULB_CREATE_PROPERTY_IF_NOT_FOUND and property_name not in properties_fields.keys():
            ...
            # Integer and boolean handling
            elif isinstance(new_property_value, int) or isinstance(new_property_value, bool):
                gdbh.w_transaction("""
                MATCH ()-[r%s %s]->()
                WHERE exists(r.%s)
                SET r.%s = %s
