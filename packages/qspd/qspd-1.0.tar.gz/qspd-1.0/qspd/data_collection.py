...
def collect_data(taus: Iterable = range(1, 6), epsilons=None, root_partitioner=None, big_ns=None):
    if epsilons is None:
        epsilons = [1e-4]
    if big_ns is None:
        big_ns = [None]
    ...
    for tau in taus:
        for epsilon in epsilons:
            for forced_big_n in big_ns:
                for record in create_records(tau, epsilon, root_partitioner, forced_big_n):
                    if not first:
                        print(",")
...
def create_records(tau, epsilon, root_partitioner, forced_n):
    logger = logging.getLogger("datacollection")
    logger.info(f"epsilon={nstr(epsilon)} tau={nstr(tau)} N={nstr(forced_n)}")
    early_times = {}

    start = timer()
    big_a, big_b = jacobi_anger(tau, epsilon, forced_n)
...
def main():
    ...
    parser.add_argument("--epsilon", type=float, nargs="+", help="The value(s) of epsilon, controlling precision, or a logarithmic (base 10) range of epsilon expressed by --epsilon minexp maxexp numpoints")
    ...
    if epsilon is None:
        ...
        epsilons = logspace(epsilon[0], epsilon[1], epsilon[2])
    else:
        raise AssertionError("--epsilon should have 1 (ex: .000001), or 3 (np.logspace, ex: -4 -9 4) arguments")

    n = args.n
    if n is None:
        ns = n
    elif n == ["auto"]:
        ns = [int(ceil(ldexp(e, -1) * max(taus) - ln(min(epsilons))))]
    elif len(n) == 1:
        ns = [int(n[0])]
    elif len(n) == 3:
    ...
    if n is None:
        logger.info(f"Running {len(taus)} iteration(s) from tau={min_str} to tau={max_str}, {len(epsilons)} iteration(s) from epsilon={min(epsilons)} to epsilon={max(epsilons)}, and N=n is not forced larger than needed ")
    else:
        logger.info(f"Running {len(taus)} iteration(s) from tau={min_str} to tau={max_str}, {len(epsilons)} iteration(s) from epsilon={min(epsilons)} to epsilon={max(epsilons)}, and {len(ns)} iteration(s) from N=n={min(ns)} to N=n={max(ns)} ")
    collect_data(taus=taus, epsilons=epsilons, root_partitioner=root_partitioner, big_ns=ns)
