...
def visualize(data, epsilon_scale=False, left_aligned=False, auto_scale=False, n_scale=False,
    ...
    epsilons = [record["epsilon"] for record in data]
    angle_lists.reverse()
    taus.reverse()
    epsilons.reverse()
    ns.reverse()
    ...
    if auto_scale and not epsilon_scale:
    ...
    if no_axes:
        ...
        if n_scale:
            ...
            super_title = "Changing N, constant Tau = " + str(data[0]["tau"]) + " epsilon = " + str(
                    data[0]["epsilon"])
            plt.suptitle(super_title, fontsize=14, fontweight='bold')

        elif epsilon_scale:
            plt.ylabel(r"$\epsilon$").set_rotation(0)
        else:
        ...
        if not auto_scale:
            if n_scale:
                ...
                ticks = [tick for tick in range(len(epsilons)) if tick % 20 == 0]
                labels = [epsilons[tick] for tick in ticks]
...
def main():
    ...
    parser.add_argument("--epsilon-scale", action="store_true",
                        help="Add if epsilon is changing rather than tau")
    ...
    visualize(data, epsilon_scale=args.epsilon_scale, left_aligned=args.left_aligned,
