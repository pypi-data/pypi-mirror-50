...
def harmonic_balance(cct, resp, num_b=15, max_it=10, stop_rerror=0.001, vj_initial=None, damp_coeff=1., mode="o", verbose=True, zj_guess=0.67,):
    ...
    # Bias points that still need to be solved
    mask = np.ones_like(cct.vb, dtype=bool)

    _cct = deepcopy(cct)

    ij_2d   = np.zeros((num_n, npts), dtype=complex)
    err_all = np.zeros((num_n, npts), dtype=complex)

    for iteration in range(max_it + 1):

        # Only solve for unfinished points
        _cct.vb = cct.vb[mask]
        _cct.vb_npts = np.sum(mask)

        # Current vector (junction current)
        time_current = timer()
        if num_f == 1:
            ij_2d[:, mask] = _qt_current_for_hb(vj_2d[:, mask], _cct, resp, num_b, resp_matrix=respfn_interp[:,mask])
        elif num_f == 2:
            ij_2d[:, mask] = _qt_current_for_hb(vj_2d[:, mask], _cct, resp, num_b, resp_matrix=respfn_interp[:,:,mask])
        elif num_f == 3:
            ij_2d[:, mask] = _qt_current_for_hb(vj_2d[:, mask], _cct, resp, num_b, resp_matrix=respfn_interp[:,:,:,mask])
        ...
        if iteration == 0 and verbose:
            ...
            it_time = (timer() - time_current) * (num_n * 2 + 1)
        ...
        # Error vector
        err_all[:, mask] = vt_2d[:, None] - zt_2d[:, None] * ij_2d[:, mask] - vj_2d[:, mask]
        ...
        for k in range(num_n):
            with np.errstate(divide='ignore'):
                ...
                complete = np.sum(good_points) / npts
        ...
        inv_j = _inv_jacobian(err_all[:, mask], vj_2d[:, mask], vt_2d, zt_2d, _cct, resp, num_b, resp_matrix=respfn_interp, verbose=verbose)
        # if num_f == 1:
        #     inv_j = _inv_jacobian(err_all[:, mask], vj_2d[:, mask], vt_2d, zt_2d, _cct, resp, num_b, resp_matrix=respfn_interp[:,mask], verbose=verbose)
        # elif num_f == 2:
        #     inv_j = _inv_jacobian(err_all[:, mask], vj_2d[:, mask], vt_2d, zt_2d, _cct, resp, num_b, resp_matrix=respfn_interp[:,:,mask], verbose=verbose)
        # elif num_f == 3:
        #     inv_j = _inv_jacobian(err_all[:, mask], vj_2d[:, mask], vt_2d, zt_2d, _cct, resp, num_b, resp_matrix=respfn_interp[:,:,:,mask], verbose=verbose)
        ...
        corr = np.zeros((num_n, np.sum(mask)), dtype=complex)
        for p in range(num_n):
            for q in range(num_n):
                corr[p, :] -=      (inv_j[p * 2,     q * 2    ] * np.real(err_all[q, mask]) +
                                    inv_j[p * 2,     q * 2 + 1] * np.imag(err_all[q, mask]))
                corr[p, :] -= 1j * (inv_j[p * 2 + 1, q * 2    ] * np.real(err_all[q, mask]) +
                                    inv_j[p * 2 + 1, q * 2 + 1] * np.imag(err_all[q, mask]))
        vj_2d[:, mask] += corr * damp_coeff

        mask = np.invert(np.invert(mask) | np.invert(finished_points))

    # Format results (from [k,vb] to [f,p,vb])
    vj_out = np.zeros((num_f + 1, num_p + 1, np.sum(mask)), dtype=complex)
    vj_out[1:, 1:, :] = vj_2d.reshape((num_f, num_p, np.sum(mask)))

    time = timer() - start_time
    if verbose:
        print((" - time: {0:.2f} s / {1:.2f} min.".format(time, time/60.)))
        if iteration >= 1:
            tpit = time / iteration  # time per iteration
            msg = " - {} iterations required"
            print((msg.format(iteration)))
            msg =  "\t- {:.2f} s / iteration"
            print((msg.format(tpit)))
            msg =  "\t- {:.2f} min / iteration"
            print((msg.format(tpit/60.)))
