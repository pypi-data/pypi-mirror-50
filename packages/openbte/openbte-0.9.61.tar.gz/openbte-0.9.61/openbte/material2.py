...
class Material(object):
 ...
 def compute_isotropic_3D(self,**argv): 
   ...
   #Import material-------------------------------------------------------------------
   tmp = np.loadtxt(argv['matfile'],skiprows=1)
   mfp_bulk = tmp[:,0]*1e9; 
   kappa_bulk = tmp[:,1]
 ...
 def compute_gray_2DSym(self,**argv):
 ...
 def compute_isotropic_2DSym(self,**argv):  
   ...
   for p in range(n_phi): 
    for t in range(n_theta): 
     for m in range(n_mfp_bulk):
      (m1,a1,m2,a2) = self.get_linear_indexes(mfp,mfp_bulk[m]*ftheta[t]*np.sin(theta[t]),scale='linear',extent='True')
      index_1 = p*n_mfp + m1
      index_2 = p*n_mfp + m2
