...
def musicxml_note_to_lily_main_event (n):
    ...
    if mxl_pitch:
        ...
        # edu{
        note_head = n.get_maybe_exist_named_child ('notehead')
        if note_head:
            event.notehead = note_head.get_text()
        else:
            event.notehead = None
        notetype = n.get_maybe_exist_named_child('type')
        if notetype:
            event.symbolsize = notetype._attribute_dict.get('symbol-size', None)
        else:
            event.symbolsize = None
        # }edu
    ...
    # edu {
    fontsize = n._attribute_dict.get('font-size')
