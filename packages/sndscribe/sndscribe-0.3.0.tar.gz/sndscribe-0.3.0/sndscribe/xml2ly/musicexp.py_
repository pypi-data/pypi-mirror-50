...
class Output_printer:
    ...
    def dump_version (self):
        self.newline ()
        self.print_verbatim ('\\version "2.19"')
        self.newline ()
...
class NoteEvent(RhythmicEvent):
    ...
    def print_ly (self, printer):
        ...
        if self.pitch:
            # edu{
            if self.symbolsize:
                printer(" \\override NoteHead #'font-size = #" + self.symbolsize + " ")
                # printer(" \\override Accidental #'font-size = #" + self.symbolsize + " ")
            elif hasattr(self, 'fontsize'):
                fontsize = self.fontsize
                notationsizes = ["teeny", "tiny", "small", "normalsize", "large", "huge"]
                if fontsize in notationsizes:
                    notationsize = fontsize
                else:
                    fontsize = int(self.fontsize)
                    minfontsize, maxfontsize = 8, 40
                    index = (fontsize - minfontsize) / float(maxfontsize-minfontsize)
                    index = int(min(max(index, 0), 1) * (len(notationsizes)-1) + 0.5)
                    notationsize = "\\" + notationsizes[index]
                printer(notationsize)
                print("fontsize: %d %s" % (fontsize, notationsize))
            # }edu    
            self.pitch.print_ly (printer)
