class Formula(object):
    ...
    def stripParentsFromMechanism(self):
        ...
        if isinstance(self, Atom):
            return self
        if self.f2 is None:
            if isinstance(self.f1, Atom):
                return [self.f1]
            return self.f1.stripParentsFromMechanism()
        if isinstance(self.f1, Atom) and isinstance(self.f2, Atom):
            return [self.f1, self.f2]
        if isinstance(self.f1, Atom) and not isinstance(self.f2, Atom):
            return [self.f1] + self.f2.stripParentsFromMechanism()
        if not isinstance(self.f1, Atom) and isinstance(self.f2, Atom):
