...
class AzureAppService(AppService):
    ...
    def update_network_status(self, app_name: str, status: str):
        self._status_maintainer.update_app_network_status(app_name=app_name, status=status)
...
class AWSAppService(AppService):
    ...
    def update_network_status(self, app_name: str, status: str):
        table, item = Utils.retry_on_exception(
            ...
            logger=self._logger,
            logger_msg=f'getting dynamo-db table for updating app "{app_name} with status "{status}"')
