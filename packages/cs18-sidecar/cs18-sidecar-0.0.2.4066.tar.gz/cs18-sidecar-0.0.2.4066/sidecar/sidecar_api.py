...
from sidecar.sandbox_public_address_fetcher import SandboxPublicAddressFetcher, AwsSandboxPublicAddressFetcher
...
def get_sandbox_public_address():
    if not _sandbox_public_address_fetcher:
        return '', 200

    public_address = _sandbox_public_address_fetcher.get_value()
    if public_address:
...
@flask.route('/application/<app_name>/<instance_id>/configuration-status/<status>', methods=['PUT'])
...
@flask.route('/application/<app_name>/<instance_id>/artifacts-status/<status>', methods=['PUT'])
...
@flask.route('/application/<app_name>/artifacts-status/<status>', methods=['PUT'])
