...
class InstanceV(DockerV):
    ...
    def restore_filestore(self, src_folder, database_name):
        ...
        original_fs_size = utils.get_size(src_folder)
        restored_fs_size = utils.get_size(path.join(dest, 'filestore'))
        logger.info('Restored filestore size: %s', restored_fs_size)
        logger.info('Original filestore size: %s', original_fs_size)
        if original_fs_size != restored_fs_size:
            error_msg = (
                'The size of the restored filestore is not the same as the original:'
                ' Expected size: {expected} Current size: {current}'.format(
                    expected=original_fs_size, current=restored_fs_size)
            )
            raise errors.CommandError('restore', error_msg)
    ...
    def execute_in_auxiliary_container(self, db_name, commands, volumes=False):
        ...
        if not volumes:
            ...
            working_folder = path.join(self.config.get("working_folder"),
                                       self.config.get('container_name'))
            volumes = container.generate_binds(volumes, working_folder)
