...
def create_gcp(ctx, name):
    ...
    spell_client.update_cluster_version(version)
...
@click.pass_context
def update_gs_cluster(ctx, cluster):
    """
    ...
    if len(need_to_add) > 0:
        answer = click.confirm("Your cluster needs to be updated to have the most recent set of role permissions.\n"
                               "Role {} is currently missing these permissions:\n{}\n"
                               "Is it ok to add these permissions?".format(role_name, "\n".join(
                                   ["- " + s for s in need_to_add])))
        if not answer:
            raise ExitException("You will not have the ability to use all of the most up to date Spell features until "
                                "you update your cluster")
    ...
    spell_client.update_cluster_version(cluster["name"], version)
