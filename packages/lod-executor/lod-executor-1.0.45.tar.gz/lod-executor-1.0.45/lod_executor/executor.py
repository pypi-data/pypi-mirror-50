...
def _make_path_default_to_local(val, default_name):
    """
    Takes a path, relative or absolute, or None. Returns a valid absolute path. Defaults to the same location as this file.
    """
    if val is None:
        val = os.path.join(os.path.dirname(os.path.abspath(__file__)), default_name)
    return os.path.abspath(val)
_CONFIG_FILE = _make_path_default_to_local(None, 'CONFIG.txt')
_config = None
def _get_config():
    """
...
def configure(args):
    ...
    with open(_CONFIG_FILE, 'w') as f:
        config = {
            ...
            'ssl_crt' : _make_path_default_to_local(args.ssl_crt, 'lod-executor-ssl-certificate.crt'),
            'ssl_key' : _make_path_default_to_local(args.ssl_key, 'lod-executor-ssl-certificate-key.key'),
            'exec_env_root_folder' : _make_path_default_to_local(args.exec_env_root_folder, 'executions'),
...
subparser.add_argument('--ssl-crt', required=False,
    help="""The location of a certificate file on your machine that is trusted by your browser.
    If you don't have one yet, you can use the 'create-certificate' command to create it later.""")
subparser.add_argument('--ssl-key', required=False,
...
def create_self_signed_cert(args):
    ...
    """
    # Load and save the locations of the certificate from the configuration
    with open(_CONFIG_FILE, 'r') as f:
        config = json.load(f)
    ssl_crt = config['ssl_crt'] if args.ssl_crt is None else args.ssl_crt
    ssl_key = config['ssl_key'] if args.ssl_key is None else args.ssl_key
    config['ssl_crt'] = _make_path_default_to_local(ssl_crt, 'lod-executor-ssl-certificate.crt')
    config['ssl_key'] = _make_path_default_to_local(ssl_key, 'lod-executor-ssl-certificate-key.key')
    with open(_CONFIG_FILE, 'w') as f:
        json.dump(config, f)
    ...
    print("Make sure that the certificate file is trusted by your browser for SSL connections (to learn how, check elody.com/tutorial/lod_executor/#the-certificate). \nThe file:\n%s" % (_get_config()['ssl_crt'],))
    print("Then, run 'lod-executor run' and verify that you can reach the following website without getting a security warning:")
...
def hello():
    return "Success!\nIf you are reading this, your browser accepts the SSL connection. You should make sure that this text is still visible if you reload the website, without having to click away a warning message from the browser."
