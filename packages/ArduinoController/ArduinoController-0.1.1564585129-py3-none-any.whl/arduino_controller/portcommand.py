...
class PortCommand:
    def __init__(
        ...
        self.sendlength = np.array([],dtype=python_sendtype).itemsize if python_sendtype is not None else 0
        self.receivelength = np.array([],dtype=python_receivetype).itemsize if python_receivetype is not None else 0
    ...
    def set_arduino_function(self, arduino_function):
        ...
        if arduino_function is not None:
            ...
            self.arduino_function.name="{}_{}".format(self.arduino_function.name,self.byteid)
        else:
    ...
    def receive(self, nparray):
        ...
        self.receivefunction(self.module,
