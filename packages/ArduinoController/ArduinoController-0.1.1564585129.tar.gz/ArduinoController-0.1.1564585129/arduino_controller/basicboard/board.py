...
class ArduinoBasicBoard:
    ...
    arduino_id = arduio_variable(arduino_data_type=uint64_t, name="id", eeprom=True, getter=False, setter=False,
                                 arduino_setter=False, arduino_getter=False)
    arduino_id_cs = arduio_variable(arduino_data_type=uint16_t, name="id_cs", eeprom=True, getter=False, setter=False,
                                    arduino_setter=False, arduino_getter=False)
    ...
    def create_ino(self, obscure=False):
        ...
        text = arduino_code_creator.create_code(obscure=obscure)
        print(text)
        return
        import inspect
        import os
    ...
    def add_arduino_code(self, ad):
        ...
        current_character = ad.add(
            at.Variable(type=uint8_t, name="current_character")
        )
        checksum = ad.add(at.Variable(type=uint16_t, name="checksum"))

        serialreadpos = ad.add(
            at.Variable(type=uint8_t, value=0, name="serialreadpos")
        )
        commandlength = ad.add(
            at.Variable(type=uint8_t, value=0, name="commandlength")
        )
        ...
        checkuuidvar = at.Variable("i", uint8_t, self.eeprom_position.get(self.arduino_id))
        check_uuid.add_call(
            generate_checksum(
            ...
            if_(
                ...
                code=(
                    ...
                    Eeprom.get(self.eeprom_position.get(self.arduino_id), self.arduino_id),
                    generate_checksum(
                        self.arduino_id.to_pointer(), Arduino.sizeof(self.arduino_id)
                    ),
    ...
    def add_ardvar_to_board(self, attr, ad):
        if attr.is_data_point:
            self.dataloop.add_call(
                WRITE_DATA_FUNCTION(
                    attr,
                    self.get_portcommand_by_name(
                        _GET_PREFIX + attr.name
                    ).byteid,
                )
            )
        if attr.eeprom:
            self.eeprom_position.add_possibility(attr, size=attr.type.byte_size)
            ad.setup.prepent_call(
                Eeprom.get(self.eeprom_position.get(attr), attr),
            )
