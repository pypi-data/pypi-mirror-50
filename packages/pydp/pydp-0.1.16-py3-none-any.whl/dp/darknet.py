...
class Darknet():
    ...
    def __init__(self,
                 ...
                 faceid_path='./data/facedb/faceid/'
                 ):
        """
        初始化
        ...
        # init face
        self.face = None
        if faceid_path and os.path.exists(faceid_path):
            self.face = Face(faceid_path)
    ...
    def detect(self, img_file, thresh=.25, hier_thresh=.25, nms=.45, tag_img=False, detect_face=False, thresh_person=0.35):
        """
        目标检测

        :param img_file: 图像文件地址
        :param thresh: 对象识别置信度（默认>0.25）
        :param hier_thresh: 
        :param nms: 去重重叠率（默认重叠45%就去重）
        :param tag_img: 是否在图片上标注识别结果（默认False）
        :param detect_face: 是否检测图片中的人脸（默认False）
        :param thresh_person: 针对人的置信度过滤严格一些（默认>0.35）
        :returns: 检测结果
        """
        if utils.get_python_version() == 3:  # python3 need str.encode()
        ...
        for o in res:
            ...
            # 特殊过滤
            if label == 'person' and o[1] < thresh_person:  # 针对人的置信度过滤严格一些
                continue
            # 目标分类合并
            x, y, w, h = o[2]
            left = int(x - w / 2)
        ...
        # 识别人脸，并绑定到persion上
        faceids = []
        if self.face and detect_face:
            faceids = self.face.get_faceids(img_file, zoom=0.5)
            # print(faceids)
            for face in faceids:
                del(face['distance'])
                left, top, right, bottom = face['rect']
                # 人脸位置如果完全嵌入在某个persion对象中，就绑定
                is_bind = False
                for i in range(len(result) - 1, -1, -1):
                    obj = result[i]
                    if obj['label'] != 'person':
                        continue
                    if obj['rect'][0] < left and obj['rect'][2] < top and obj['rect'][1] > right and obj['rect'][3] > bottom:
                        logging.info('bind! {}'.format(label))
                        is_bind = True
                        result[i]['ext'] = face
                        break
                # 人脸绑定不到现有对象时，新建一个对象
                if is_bind is False:
                    ...
                    if rl_pos >= 0.75 and tb_pos <= 0.25:
                        ...
                        pos_name = '左下方'
                    result.append({
                        'label': 'person',  # 目标英文label
                        'label_name': '人',    # 目标中文label
                        'weight': round(face['weight'], 4),   # 目标置信度
                        'rect': face['rect'],  # 目标位置
                        'pos': (rl_pos, tb_pos, pos_name),  # 目标相对位置(左右位置, 上下位置, 主视角度方位名)
                        'ext': face,  # 扩展信息
                    })
    ...
    def caption(self, result, thresh=0.25):
        ...
        for obj in result:
            ...
            if label not in pos_stat[pos_name]:
                pos_stat[pos_name][label] = {'cnt': 0, 'facename': set()}
            pos_stat[pos_name][label]['cnt'] += 1
            if 'facename' in obj['ext'] and obj['ext']['facename']:  # 人脸名称
                pos_stat[pos_name][label]['facename'].add(obj['ext']['facename'])
        ...
        for pos_name in pos_stat:
            ...
            for label in pos_stat[pos_name]:
                count = pos_stat[pos_name][label]['cnt']
                # person时增加人名
                facename = ''
                if len(pos_stat[pos_name][label]['facename']) > 0:
                    facename = '({})'.format(','.join(pos_stat[pos_name][label]['facename']))

                if count > 3:
                    caption_pos += '{}{}{},'.format(LABELS[label]['units'], LABELS[label]['name'], facename)
                else:
                    caption_pos += '{}{}{}{},'.format(utils.NUMERALS[count], LABELS[label]['unit'], LABELS[label]['name'], facename)
...
if __name__ == "__main__":
    ...
    #darnet_path = '/home/work/project/darknet'
    dn = Darknet(
        ...
        meta_file=darnet_path + '/cfg/coco.data',  # 80个分类 *如果命令不在darknet目录执行时，需要把meta_file里的“names = data/coco.names”改为绝对路径
        # meta_file=darnet_path + '/cfg/openimages.data',  # 9k个分类
        # meta_file=darnet_path + '/cfg/imagenet1k.data',  # 1k个分类
        faceid_path='./data/facedb/faceid/',  # 人脸ID目录
    )
    # detect检测
    #res = dn.detect(darnet_path + '/data/dog.jpg')
    args = utils.get_args('-f=test.jpeg')
    res = dn.detect('{}/data/{}'.format(darnet_path, args.f), detect_face=True)  # , tag_img=True, nms=.45)
    print(res)

    # caption理解
    res = dn.caption(res, thresh=.1)
    print(res)
    ...
    _tts = tts.get_engine('baidu-tts', profile, cache_path=constants.TEMP_PATH + '/tts/')
    #video_file = tts.get_speech(phrase)
    _tts.say(res['pos'], force_update=True)
