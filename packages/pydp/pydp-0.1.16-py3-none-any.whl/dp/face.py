...
class Face():
    """人脸测试"""
    FACEID_PATH = constants.APP_PATH + "/data/facedb/faceid"
    TEMP_PATH = constants.USER_PATH + "/.pydp/face/tmp"
    ...
    def __init__(self, faceid_path=None, temp_path=None):
        ...
        if faceid_path is not None:
            self.FACEID_PATH = faceid_path
        if temp_path is not None:
            self.TEMP_PATH = temp_path
        utils.mkdir(self.TEMP_PATH)
        utils.mkdir(self.FACEID_PATH)
    ...
    def create_facedb(self):
        ...
        """
        self.facedb, self.facenames = [], []
        if os.path.exists(self.FACEID_PATH) is False:
            return
        for img_file in os.listdir(self.FACEID_PATH):
            if img_file in utils.TMPNAMES or img_file[-4:] == '.bak':
                continue
            image = face_recognition.load_image_file('{}/{}'.format(self.FACEID_PATH, img_file))
            _emb = face_recognition.face_encodings(image)
            if len(_emb) == 0:
                continue
            emb = _emb[0]
            tmp = img_file.split('.')
    ...
    def append_facedb(self, faceid_file, faceid):
        """
        添加单个人脸到FaceDB字典
        :param faceid_file: face图片
        :param faceid: faceid信息
        :returns:
        """
    ...
    def register_faceid(self, face_img_file, name, rect=None):
        """
        注册新的faceid
        ...
        faceid_file = '{}/{}.{}.jpg'.format(self.FACEID_PATH, faceid, name.replace('.', '_'))
    ...
    def fact_extract(self, extract_path, output_path):
        """
        照片中的face提取
        
        :param extract_path: 要提取face的目录
        :param output_path: 提取的face文件保存位置
        :returns: count 提取出的face文件个数
        """
        utils.mkdir(output_path)
        i=0
        for img_file in os.listdir(extract_path):
            filename = '{}/{}'.format(extract_path, img_file)
            im = cv2.imread(filename)
            image = face_recognition.load_image_file(filename)
            for top, right, bottom, left in face_recognition.face_locations(image):
                i += 1
                face_image = im[top:bottom, left:right]
                cv2.imwrite('{}/face.{}.jpg'.format(output_path, i), face_image)
                print('{}\t{},{},{},{} -> {}'.format(filename, top, right, bottom, left, i))
        return i
                
    def photo_extract(self, extract_path, output_path):
        """
        按faceid自动分拣照片到不同目录（建议调用此函数前，先进行照片extract face提取）
            face_reco = Face(Face.FACEDB_PATH + 'faceid_test/') # 每个人物取一个face图片放到此目录用于分类
...
if __name__ == '__main__':
    ...
    # 摄像头人脸识别对象
    face = Face(faceid_path=Face.FACEID_PATH, temp_path=Face.TEMP_PATH)
    # 创建人脸ID
    #faceid = face.register_faceid(constants.APP_PATH + '/data/image/test-face.jpg', name='faceid', rect=None)
    #print(faceid)
    # 启动摄像头人脸识别
    face.get_camera_face(camera_id=0, camera_data=face.camera_data, callback=show_camera_face_window)

    #提取图片中的所有人脸
    #ret = face.fact_extract(constants.APP_PATH + '/data/tmp/photo', constants.APP_PATH + '/data/tmp/out')
    #print(ret)
