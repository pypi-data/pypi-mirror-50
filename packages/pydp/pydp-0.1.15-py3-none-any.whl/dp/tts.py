...
class AbstractTTS(object):
    ...
    def say(self, phrase, callback=None, force_update=False):
        ...
        # get cache
        voice_file = self._get_cache(phrase, force_update)
        ...
        if not voice_file:
            ...
            # set cache
            cache_file = self._set_cache(phrase, voice_file)
            if cache_file:
    ...
    def _get_cache(self, phrase, force_update=False):
        """
        获取语音cache文件位置
        ...
        :returns: cache文件地址
        """
        if self.cache_path != '' and force_update is False:
            utils.mkdir(self.cache_path)
            cache_file = os.path.join(self.cache_path, utils.md5(phrase)+'.mp3')
            if os.path.exists(cache_file):
                logging.info('get cache! {} {}'.format(phrase, cache_file))
                return cache_file
        return ''

    def _set_cache(self, phrase, tmpfile):
        """
        保存语音文件cache
        ...
        if self.cache_path != '' and os.path.exists(tmpfile):
            cache_file = os.path.join(self.cache_path, utils.md5(phrase)+'.mp3')
            utils.rm(cache_file)
