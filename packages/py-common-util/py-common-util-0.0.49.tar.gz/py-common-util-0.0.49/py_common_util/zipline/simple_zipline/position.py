...
class Position(object):
    """
    记录每只股票的持仓情况。
    init_cash 该股票分配的初始回测金额，e.g. 1000000
    amount 剩余持有股票份数, 可以为负数，is_buy: >0, is_sell: <0, 刷新浮动盈亏: 0
    ...
    def __init__(self, security_code, init_cash_per_stock, amount=0, start_price=0, market_position=1):
        ...
        self.income = 0  # init_cash + income = 剩下的现金
        self.last_price = start_price
        self.start_price = start_price  # 调仓周期中的起始价格
        self.balance_cash = self.init_cash_per_stock  # 分配给该只股票每次交易后还剩下的静态现金（不根据股价浮动，已经减去了交易时的price*amount和cost）
    ...
    def update(self, added_amount, new_last_price, min_move, commision, market_position=1):
        ...
        if self.market_position > 0:
            if added_amount > 0:
                ...
                self.balance_cash = self.balance_cash + self.income
        ...
        # 第一次交易时更新start_amount和start_price
