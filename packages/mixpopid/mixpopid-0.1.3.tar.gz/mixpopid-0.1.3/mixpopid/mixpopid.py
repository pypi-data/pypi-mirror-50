def rateexpo(p, x):
    ...
    parameters p exposed to a certain drug with a given concentration according
    ...
    import math
    return (p[0]+math.log(p[1]+ (1-p[1])/(1+(x/p[2])**p[3])))

def popexpo(params, x, T):
    """
    A function calculating a certain cell population count using a function
    'rate' at a given time point according to the exponential cell population 
    growth model.
    ...
    import math
    return (math.exp(T* rateexpo(params, x)))

def objective(PopN, Params, DATA, concvec, timevec, NR, ConcT, TimeT, model):
    """
    ...
    if model=="expo":
        if len(p)%4==0:
            PMat=[[p[4*j+i] for i in range(4)] for j in range(PopN)]
        else:
    ...
    for i in range(1,len(timevec)):
        for j in range(len(concvec)):
            for k in range(NR):
                s=0
                for l in range(PopN-1):
                    s+=DATA[k][j][0]*X1[l]*f(PMat[l],concvec[j],timevec[i])
                    
                s+=DATA[k][j][0]*(1-sum(X1))*f(PMat[PopN-1],concvec[j],timevec[i])
                resid = DATA[k][j][i] - s
                #Adding noize:
                if ((concvec[j]<=ConcT) and (timevec[i]>=TimeT)):
                    sig=sigH
                    NH = NH + 1
                else:
                    sig=sigL
...
def mixtureID(PopN, DATA, Time, Conc, R, model = "expo", 
    ...
    cell line is caracterised by 4 parameters (alpha, b: minimal drug response 
    ...
    design personalised treatment strategies. 
    ...
    for p in range(1,PopN+1):
        ...
        if p>1:
            bnds = [(1e-05, 0.5) for i in range(p-1)]
        else:
            bnds = []
        if model=="expo":
            for i in range(p):
                bnds.append(bounds_expo['alpha'])
                bnds.append(bounds_expo['b'])
                bnds.append(bounds_expo['E'])
                bnds.append(bounds_expo['n'])
        ...
        for n in range(num_optim) :
            x0=[random.random()*(bnds[i][1]-bnds[i][0]) + bnds[i][0] for i in range(len(bnds))]
    ...
    print("Mixture parameter(s): ", 1 if pfinal==1 else x_final[0])
    if model=="expo":
    ...
    for i in range(pfinal):
        ...
        plt.plot(range(len(Conc)),[rateexpo(param,x) for x in Conc], '-*', linewidth=3, 
