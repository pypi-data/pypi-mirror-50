...
LOOP_PERIOD = 0.25  # seconds
MAX_BUFFER_SIZE = 1000  # if the buffer reaches this size, we flush it without waiting for the next iteration
...
class AsyncRecorder(SpanRecorder):
    ...
    def run(self):
        ...
        while running:
            ...
            # * The buffer size is exceeding MAX_BUFFER_SIZE
            if loop_duration >= self.period or not running or len(buffer) >= MAX_BUFFER_SIZE:
                loop_duration = 0
                try:
                    self.flush(buffer)
                    buffer = []
