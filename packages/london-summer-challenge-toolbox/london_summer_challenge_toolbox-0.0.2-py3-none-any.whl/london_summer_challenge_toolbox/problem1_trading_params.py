...
from london-summer-challenge-toolbox.problem1_data_source import Problem1DataSource
...
import pandas as pd
import numpy as np
from london-summer-challenge-toolbox.problem1_time_rule import DayTimeRule
import sys
...
class MyTradingParams(TradingSystemParameters):
    ...
    def getTimeRuleForUpdates(self):
        return DayTimeRule(startDate=self.__startDate, endDate=self.__endDate, frequency='Mo', sample='1', startTime='00:00', endTime='17:00')
    ...
    def trainModel(self, df=None, y=None):
        ...
        if df is None:
            ...
            self.setScore(np.mean(corr_array))
        if y is None:
...
class ScoreCalculator(Feature):
    ...
    def computeForInstrument(cls, updateNum, time, featureParams, featureKey, instrumentManager):
        ...
        score = (previousValue*(updateNum-1-burn)+currentScore)/(updateNum-burn)#sm.accuracy_score(predictionData, trueValue)
