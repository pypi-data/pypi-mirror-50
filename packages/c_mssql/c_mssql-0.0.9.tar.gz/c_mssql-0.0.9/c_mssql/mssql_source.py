...
class Mssql_Source(Sql_Mssql):
    ...
    def get_sql_title(self,sql_str):
        conn = _mssql.connect(server=self.db_config.server, user=self.db_config.user, password=self.db_config.password, database=self.db_config.database,charset='utf8')
        try:
