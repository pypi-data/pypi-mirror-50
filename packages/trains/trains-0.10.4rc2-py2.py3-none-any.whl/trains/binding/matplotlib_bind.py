...
from ..utilities.io_manager import IOCallsManager
...
class PatchedMatplotlib:
    ...
    def patch_matplotlib():
        PatchedMatplotlib._io_calls_manager = IOCallsManager()
        # only once
    ...
    def patched_show(*args, **kw):
        ...
        if PatchedMatplotlib._current_task and running_remotely():
            ...
            try:
                ...
                PatchedMatplotlib._io_calls_manager.remove_thread(tid)
        return ret

    @staticmethod
    def _report_figure(force_save_as_image=False, stored_figure=None, set_active=True, specific_fig=None):
        tid = threading._get_ident() if six.PY2 else threading.get_ident()
        if not PatchedMatplotlib._current_task or PatchedMatplotlib._io_calls_manager.is_plot_called(tid,
                                                                                                     stored_figure or specific_fig):
            return
        ...
        try:
            ...
            PatchedMatplotlib._io_calls_manager.add_io_to_thread(tid, mpl_fig)
