...
from aioredis import ConnectionsPool,connection
...
class RedisLock(object):
    ...
    """
    def __init__(self,redis_c:Union[ConnectionsPool,connection],key,try_times=5,await_times=3):
    ...
    async def __aenter__(self):
        ...
        for _ in range(self._try_times):
            status = await StringCommandsMixin.msetnx(self._redis_c,self._redis_c,"Redis_lock_add")
            if status==1:
                break
            else:
