...
class Calendar(sortedcontainers.SortedSet):
    def __init__(self, dates=None, key=None):
        ...
        if dates is None: 
            dates = []
        elif not all([internals.isdatelike(item) for item in dates]):
    ...
    def lb(self, date, default=RAISE):
        ...
        if date < self[0]: 
            ...
            return default
        return self[self.bisect_key_left(date) - (1 if date in self else 0)]
    ...
    def generate(cls, *args, **kwargs):
        """
        Creates a new calendar using dateutil.rrule
...
class Grouper: 
    ...
    def apply(self, func):
        ...
        return Calendar().union(*[func(cdr) for cdr in self.calendars])
