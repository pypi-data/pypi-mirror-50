...
@ns_data.route(*['/basedirs'])
class Basedir_ls(Resource):
    @api.marshal_with(models["BaseDirCollection"],
                      skip_none=False)
    def get(ns, *args, **kwargs):
        """Retrieve a list of the currently configured base directories of the *live* server instance. This will be different from the originally configured base directories when the container was first started.
        ...
        mod, call = import_fqn("datacustodian.ipfs.ls_basedirs")
        results = call(*args, **kwargs)
        return results
    
    @api.expect(_get_expectant("BaseDir"),
                validate=False)
    @api.marshal_with(models["BaseDirCollection"],
                      skip_none=False)
    def post(ns, *args, **kwargs):
        """Add an additional base directory to the list of configured ones. This sets the base directory up in the `overrides` of the application so that the changes persist after restart. The original application spec files are not altered.

        """
        if "BaseDir" in parsers: #pragma: no cover
            kwargs = parsers["BaseDir"].parse_args(request)
        kwargs["_data"] = request.json
        kwargs["_request"] = request

        mod, call = import_fqn("datacustodian.ipfs.add_basedir")
        results = call(*args, **kwargs)
        return results
    

@ns_data.route(*['/basedirs/<string:key>'])
class Basedirs(Resource):
    @api.marshal_with(models["BaseDirCollection"],
                      skip_none=False)
    def delete(ns, *args, **kwargs):
        """Remove the base directory configured under `key` from the *live* server instance. This removes the base directory in the `overrides` of the application so that the changes persist after restart. The original application spec files are not altered.
        ...
        mod, call = import_fqn("datacustodian.ipfs.rm_basedir")
        results = call(*args, **kwargs)
        return results
...
class Browse(Resource):
    ...
    def post(ns, *args, **kwargs):
        ...
        """
        if "Pagination" in parsers: #pragma: no cover
...
class Files(Resource):
    ...
    @api.param('recursive', """""", __in="query",
               required=False, default='False')
    @api.param('layout', """""", __in="query",
               required=False, default='')
    @api.param('chunker', """Chunker algorithm to use.""", __in="query",
               required=False, default='size-262144')
    @api.param('raw_leaves', """""", __in="query",
               required=False, default='False')
    @api.param('hidden', """""", __in="query",
               required=False, default='False')
    @api.param('wrap', """""", __in="query",
               required=False, default='False')
    @api.param('shard', """""", __in="query",
               required=False, default='False')
    @api.param('progress', """""", __in="query",
               required=False, default='False')
    @api.param('cid_version', """""", __in="query",
               required=False, default='0')
    @api.param('hashfun', """Hashing function to use.""", __in="query",
               required=False, default='sha2-256')
    @api.param('stream_channels', """""", __in="query",
               required=False, default='True')
    @api.param('nocopy', """""", __in="query",
               required=False, default='False')
    @api.param('replication_factor_min', """Minimum replication factor in the cluster.""", __in="query",
               required=False, default='0')
    @api.param('replication_factor_max', """Maximum replicatior factor across the cluster.""", __in="query",
               required=False, default='0')
    @api.param('shard_size', """Not well documented in reference IPFS cluster implementation...""", __in="query",
               required=False, default='104857600')
    @api.param('name', """Name of the file that was added, or `bytes` if the file name is unknown.
""", __in="query",
               required=False, default='')
...
""", __in="query",
    ...
    @api.marshal_with(models["IPFSPinResult"],
                      skip_none=False)
...
@ns_data.route(*['/pins'])
class Pins_ls(Resource):
    @api.marshal_with(models["IPFSPinList"],
                      skip_none=False)
    def get(ns, *args, **kwargs):
        ...
        mod, call = import_fqn("datacustodian.ipfs.ls_pins")
        results = call(*args, **kwargs)
        return results
