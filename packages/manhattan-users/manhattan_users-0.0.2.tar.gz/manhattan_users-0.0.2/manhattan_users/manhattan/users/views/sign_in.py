...
def log_failed_attempts(state):
    ...
    cache = flask.current_app.config['USER_FAILED_SIGN_IN_CACHE']
    ...
    cache.set(
        attempt_key,
        attempts + 1,
        flask.current_app.config['USER_FAILED_SIGN_IN_LOCKOUT']
    )
    ...
    if user_cls.is_locked_out(state.form.email.data):
        ...
        lockout = flask.current_app.config['USER_FAILED_SIGN_IN_LOCKOUT']
...
def sign_in(state):
    ...
    user_session, new_session = state.user.sign_in()
    ...
    if state.new_session:
        ...
        flask.current_app.manage.send_email(
            ...
            global_vars={
                ...
                'user_session': state.user_session.to_json_type()
            },
            template_map={
...
def redirect(state):
    ...
    if not next_url:
        ...
        next_url = flask.url_for(state.manage_config.get_endpoint('dashboard'))

    return flask.redirect(next_url or dashboard_url)
