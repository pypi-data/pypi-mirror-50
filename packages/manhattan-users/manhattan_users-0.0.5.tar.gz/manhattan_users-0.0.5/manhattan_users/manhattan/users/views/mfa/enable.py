...
# Define the links
enable_chains.set_link(factories.config(form_cls=EnableForm))
...
def generate_qr_code(state):
    ...
    mfa_provisision_uri = mfa_client.provisioning_uri(
        user.email,
        issuer_name=flask.current_app.config['USER_MFA_ISSUER']
    )
...
def enable_mfa(state):
    ...
    recovery_codes = []
    for i in range(0, flask.current_app.config['USER_MFA_RECOVERY_CODES']):
        recovery_codes.append(
            random_code(
                flask.current_app.config['USER_MFA_RECOVERY_CODE_LENGTH']
            )
        )
