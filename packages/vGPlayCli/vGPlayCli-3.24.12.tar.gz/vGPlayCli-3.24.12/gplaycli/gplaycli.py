...
from gpapi.googleplay import AuthError
...
try:
    __version__ = '%s [Python%s] ' % (get_distribution('vgplaycli').version, sys.version.split()[0])
...
class GPlaycli:
    ...
    def home(self):
        try:
            ...
            logger.info("Token has expired while request browse. Retrieving a new one.")
            self.refresh_token()
            homeapps = self.api.getHomeApps()
    ...
    def list(self, cat, ctr=None, nb_results=None, offset=None):
        try:
            ...
            logger.info("Token has expired while request list. Retrieving a new one.")
            self.refresh_token()
            listapps = self.api.list(cat, ctr, nb_results, offset)
    ...
    def searchSuggest(self, query):
        try:
            ...
            logger.info("Token has expired while request suggest. Retrieving a new one.")
            self.refresh_token()
            suggestapps = self.api.searchSuggest(query)
    ...
    def detail(self, pkg_todetail):
        ...
        for pkg in pkg_todetail:
            try:
                ...
                logger.info("Token has expired while request detail. Retrieving a new one.")
                self.refresh_token()
                detail = self.api.details(pkg[0])
                details.append(detail)
            except RequestError as request_error:
    ...
    def search(self, search_string, nb_results, free_only=True, include_headers=True):
        ...
        try:
            ...
            logger.info("Token has expired while request search. Retrieving a new one.")
            self.refresh_token()
            results = self.api.search(search_string, nb_result=nb_results)
        except IndexError:
            results = []
