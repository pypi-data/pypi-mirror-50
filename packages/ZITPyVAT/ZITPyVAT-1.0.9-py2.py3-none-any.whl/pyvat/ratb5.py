...
def signal_gen():
    ...
    while True:
        ...
        if sel == 0:
            ...
            k0 = raw_input("Input K0(0x{0:04X}):0x".format(ret))
            k0 = int(k0, base=16)
            ue.set_tx_k0(k0)
        elif sel == 2:
            datt = raw_input("Input Datt(dB):")
            datt = float(datt)
            ue.set_datt_n3(datt)
        elif sel == 3:
            ...
            freq = raw_input("Input Freq({0:.3f}Mhz -> {1}):".format(float(disp_freq(ue.pll_cfg_freq)) / 1000,
                                                                     disp_freq(ue.pll_cfg_freq)))
            freq = int(freq)
            ue.pll_chip_en(0x01)
            ue.pll_chip_cfg(freq, ue.pll_cfg_flag)
        elif sel == 4:
            option = raw_input("Input Operation, 0, OFF; 1, ON:")
            option = int(option)
            ue.lna_chip_en(option)
            ue.pll_chip_en(option)
        elif sel == 5:
            freq = raw_input("Input Freq(922.265Mhz -> 922625):")
            freq = int(freq)
            power = raw_input("Input the Power(-8~15):")
            power = float(power)
...
def eeprom_test():
    ...
    while True:
        ...
        if sel == 0:
            ...
            offset = raw_input("Input The offset:0x")
            offset = int(offset, base=16)
            length = raw_input("Input the Length:")
            length = int(length)
            ue.get_eeprom(offset, length)
        elif sel == 2:
            offset = raw_input("Input The offset:0x")
            offset = int(offset, base=16)
            data_str = raw_input("Input the data str(00 01 02 FF 0E):")
        ...
        if sel == 0:
            ...
            while True:
                ...
                if option == 0:
                    ...
                    power_in = raw_input("Input the Power:")
                    power_in = float(power_in)
                    k0_in = raw_input("Input the K0 Value:0x")
                    k0_in = int(k0_in, base=16)
                    datt_in = raw_input("Input the Datt Value:")
