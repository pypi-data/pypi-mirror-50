...
from pydantic import BaseModel, Schema, StrictStr
...
class BaseError(Exception):
    ...
    def build_resp_model(cls):
        ...
        class _ErrorResponseModel(BaseModel):
            jsonrpc: str = Schema('2.0', const=True, example='2.0')
...
class MethodRoute(APIRoute):
    def __init__(
        ...
        **kwargs,
        ...
        class _Request(BaseModel):
            jsonrpc: str = Schema('2.0', const=True, example='2.0')
            id: Union[StrictStr, int] = Schema(None, example=0)
            method: str = Schema(name, const=True, example=name)
    ...
    async def jsonrpc_app_response(self, http_request: Request):
        ...
        raw_response = self.response_class(status_code=200, content=content, background=background_tasks)
        return raw_response
    ...
    async def jsonrpc_app_content(
        ...
        try:
            content = await self.jsonrpc_app(http_request, background_tasks, dependency_cache, req)
        except BaseError as error:
            content = self.get_error_resp(error)
        return content
    ...
    async def jsonrpc_app(self, http_request: Request, background_tasks: BackgroundTasks, dependency_cache, req):
        try:
            ...
            if errors:
                raise self.request_validation_error(RequestValidationError(errors), req=req)
...
class EntrypointRoute(APIRoute):
    def __init__(
        ...
        **kwargs,
        ...
        def endpoint(request: JsonRpcRequest):
            del request
        ...
        for req in req_list:
            ...
            try:
                ...
                if 'method' not in req:
                    raise InvalidRequest({'errors': [
                        {'loc': ('method', ), 'type': 'value_error.missing', 'msg': 'field required'},
                    ]})

                if not isinstance(req['method'], str):
                    raise InvalidRequest({'errors': [
                        {'loc': ('method', ), 'type': 'type_error.str', 'msg': 'str type expected'},
                    ]})
                ...
                for route in self.entrypoint.routes:
                    ...
                    if match == Match.FULL:
                        ...
                        resp = await route.jsonrpc_app_content(http_request, background_tasks, dependency_cache, req)
                        break
                else:
