...
class DockerNode(BaseNode):
    def formatNodeInfo(self, nodeInfo):
        ...
        node = lcc.classify(
            ...
            outputs=lambda p: p.get("defInPortUuid", "").startswith("out"),
            params=lambda p: "defInPortUuid" not in p,
        )
        inputs = {
            i["defOutPortUuid"]: Port(
                uuid=i["defOutPortUuid"],
                name=i["name"],
                type=ports[i["defOutPortUuid"]]["type"],
                subtype=ports[i["defOutPortUuid"]]["subtype"],
            )
            for i in node["inputs"]
        }
        outputs = {
            o["defOutPortUuid"]: Port(
