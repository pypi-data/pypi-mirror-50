...
from techela import COURSE, COURSEDIR, __version__


with open(os.path.join(COURSEDIR, 'course-data.json')) as f:
...
def get_course_data():
    ...
    lectures = glob.glob(f'{COURSEDIR}/lectures/*.ipynb')
...
def hello():
    ...
    myassignments = [os.path.join(COURSEDIR, 'my-assignments', af)
...
The roster.csv file is just the file downloaded from s3, renamed to roster.csv.
    """
    import csv
    roster = f'{COURSEDATA["local-box-path"]}/roster.csv'
