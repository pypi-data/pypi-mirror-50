...
class DiffMetric:
    ...
    def validate_extended_properties_type(self, key, value):
        ...
        if key in ["name", "value"]:
            raise ValueError("ep key {} is reserved".format(key))
...
def get_categorical_column_metrics(cdp):
    ...
    for i in range(len(cdp.value_counts)):
        metrics[constants.DIMENSION_CATEGORY_NAME].append(cdp.value_counts[i].value)
...
class ClassifierDriftDetector:
    ...
    def get_classifier_drift_detector_results(self, x1, x2, learner=None):
        ...
        for train, test in sss.split(data, target):
            ...
            all_mcc = self.sklearn.metrics.matthews_corrcoef(target, all_preds)

            test_preds = clf1.predict(data.iloc[test])
            test_mcc = self.sklearn.metrics.matthews_corrcoef(
                target[test],
                test_preds)

            train_preds = clf1.predict(data.iloc[train])
            train_mcc = self.sklearn.metrics.matthews_corrcoef(
                target[train],
                train_preds)
...
class DatasetDiff:
    ...
    def _get_numerical_distance_metric(p1, p2, m_name, ep):
        ...
        results = [DiffMetric(name=m_name,
                              value=dm_methods[m_name](x1, x2, x1_weight, x2_weight),
                              extended_properties=ep)]
