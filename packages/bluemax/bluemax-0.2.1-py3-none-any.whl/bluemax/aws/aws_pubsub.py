...
class AwsCache:
    ...
    async def apublish(cls, topic_name, message):
        ...
        return await AwsCache.client('sns').publish(
...
def publish(topic_name, message, loop=None):
    ...
    return asyncio.ensure_future(
        AwsCache.apublish(topic_name, message), loop=loop
    )
...
class aws_publish:
    ...
    async def _publish_(self, *args, **kwargs):
        ...
        if self.topic_arn is None:
            self.topic_arn = await AwsCache.get_topic_arn(
                stage_awair(self.topic_name)
            )
    ...
    def __call__(self, *args, **kwargs):
        ...
        return asyncio.ensure_future(
            self._publish_(*args, **kwargs), loop=self.loop
        )
...
class aws_subscribe:
    ...
    def __call__(self, *args, **kwargs):
        ...
        return asyncio.ensure_future(
            self._subscribe_(*args, **kwargs), loop=self.loop
        )
