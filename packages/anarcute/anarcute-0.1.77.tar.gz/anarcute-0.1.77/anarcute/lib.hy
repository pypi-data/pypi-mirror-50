(do 
      ...
      (defn condense[obj &optional [key thru][value thru][operator +]]
        (let [res {}]
          (for [(, k v) (.items obj)]
            (setv k (key k)
                  v (value v))
            (if (in k res) (assoc res k (operator (get res k) v)) (assoc res k v)))
          res))
      (setv group condense)
  ...
  `(do "tf*idf"
       ...
       (defn tf-idf[context text]
         (setv context-v (if (= str (type context)) (vectorize context) context)
               text-v (if (= str (type text)) (vectorize text) text))
