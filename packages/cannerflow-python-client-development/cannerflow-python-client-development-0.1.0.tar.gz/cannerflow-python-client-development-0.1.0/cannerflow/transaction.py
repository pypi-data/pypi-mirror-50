...
class Transaction(object):
    ...
    def begin(self):
        ...
        if not response.ok:
            raise cannerflow.exceptions.DatabaseError(
    ...
    def commit(self):
        query = cannerflow.execution.CannerflowQuery(self._request, COMMIT)
        try:
            list(query.execute())
    ...
    def rollback(self):
        query = cannerflow.execution.CannerflowQuery(self._request, ROLLBACK)
        try:
            list(query.execute())
        except Exception as err:
            raise cannerflow.exceptions.DatabaseError(
