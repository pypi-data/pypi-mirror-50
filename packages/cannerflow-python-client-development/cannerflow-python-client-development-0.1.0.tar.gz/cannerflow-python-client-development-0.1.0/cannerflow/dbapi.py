...
from cannerflow import constants
import cannerflow.exceptions
import cannerflow.execution
...
class Connection(object):
    """Cannerflow supports transactions and the ability to either commit or rollback
    ...
    def __init__(
        ...
        self._http_session = cannerflow.execution.CannerflowRequest.http.Session()
    ...
    def close(self):
        """Cannerflow does not have anything to close"""
    ...
    def _create_request(self):
        return cannerflow.execution.CannerflowRequest(
            self.host,
            self.port,
            self.user,
            self.source,
...
class Cursor(object):
    ...
    def rowcount(self):
        ...
        Cannerflow cannot reliablity determine the number of rows returned by an
    ...
    def setinputsizes(self, sizes):
        raise cannerflow.exceptions.NotSupportedError

    def setoutputsize(self, size, column):
        raise cannerflow.exceptions.NotSupportedError

    def execute(self, operation, params=None):
        self._query = cannerflow.execution.CannerflowQuery(self._request, sql=operation)
    ...
    def cancel(self):
        if self._query is None:
            raise cannerflow.exceptions.OperationalError(
...
ROWID = DBAPITypeObject()  # nothing indicates row id in Cannerflow
