...
import cannerflow.redirect
import requests
from cannerflow import constants, exceptions
from cannerflow.transaction import NO_TRANSACTION


__all__ = ["CannerflowQuery", "CannerflowRequest"]
...
class CannerflowRequest(object):
    """
    Manage the HTTP requests of a Cannerflow query.
    ...
    :param catalog: to query. The *catalog* is associated with a Cannerflow
    ...
    :param session_properties: set specific Cannerflow behavior for the current
    ...
    Please refer to :class:`CannerflowStatus` to access the status returned by
    :meth:`CannerflowRequest.process`.
    ...
    def _process_error(self, error, query_id):
        ...
        if error_type == "EXTERNAL":
            raise exceptions.CannerflowExternalError(error, query_id)
        elif error_type == "USER_ERROR":
            return exceptions.CannerflowUserError(error, query_id)

        return exceptions.CannerflowQueryError(error, query_id)
    ...
    def process(self, http_response):
        # type: (requests.Response) -> CannerflowStatus
...
class CannerflowResult(object):
    """
    Represent the result of a Cannerflow query as an iterator on rows.
...
class CannerflowQuery(object):
    """Represent the execution of a SQL statement by Cannerflow."""

    def __init__(
        self,
        request,  # type: CannerflowRequest
        sql,  # type: Text
    ):
        # type: (...) -> None
        ...
        self._sql = sql
        self._result = CannerflowResult(self)
    ...
    def execute(self):
        # type: () -> CannerflowResult
        """Initiate a Cannerflow query by sending the SQL statement
        ...
        """
        if self._cancelled:
            raise exceptions.CannerflowUserError("Query has been cancelled", self.query_id)
        ...
        if status.next_uri is None:
            self._finished = True
        self._result = CannerflowResult(self, status.rows)
        return self._result
