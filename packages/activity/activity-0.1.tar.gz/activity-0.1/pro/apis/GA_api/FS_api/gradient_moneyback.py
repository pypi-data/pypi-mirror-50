    ...
    for index in range(0, len(unify_buygift_list)):
        ...
        for bean in unify_buygift_list:
            if len(bean.product_list) < 1:
                continue
            # 返回每个方案计算后的商品集合
            list1 = gradient_moenyback(list1, bean, userInfo)
...
# 具体运算
def gradient_moenyback(productList, bean, userInfo):
    ...
    if len(productListA) > 0:
        ...
        is_can_meet_condition = False  # 记录是否可以执行此促销
        for promotion in reversed(bean.operation_set):
            ...
            # 满减值
            bean.moenyback = promotion.moenyback

            # response = choice(bean, userInfo, productListA, promotion_qtty_sum, promotion_amt_list_sum,
            #                   promotion_amt_retail_sum, promotion_amt_receivable_sum)
            # if type(response) == str:
            #     return productList
            response = unify_gradient_cal(bean, userInfo, productListA, promotion_qtty_sum,
            ...
            if response == 1:
                # 说明该促销满足条件，但不是最优的
                is_can_meet_condition = True
            if type(response) == str:
                return productList

        if is_can_meet_condition:
            # 说明该促销满足条件，但不是最优的
