    ...
    for index in range(0, len(unify_buygift_list)):
        ...
        for bean in unify_buygift_list:
            if len(bean.product_list) < 1:
                continue
            # 返回每个方案计算后的商品集合
            list1 = unify_special_pirce(list1, bean, userInfo)
...
# 具体运算
def unify_special_pirce(productList, bean, userInfo):
    ...
    if len(productListA) > 0:
        ...
        condition_qtty(bean, userInfo, productListA, promotion_qtty_sum,org_promotion_qtty_sum)
