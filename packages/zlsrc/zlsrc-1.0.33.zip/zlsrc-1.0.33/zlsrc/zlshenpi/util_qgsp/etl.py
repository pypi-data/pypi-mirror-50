...
def create_gg(conp):
    sql1 = """
    drop table if exists %s.gg_src;
    create table if not exists %s.gg_src
    (
    name text,
    ggstart_time text,
    href text,
...
def insert_tb(tbname, diqu, conp):
    ...
    sql2 = """
    insert into %s.gg_src select  * from %s.%s
...
def gg_distinc(conp, **arg):
    sql = """
    delete from %s.gg_src
    where ctid in (select ctid from (select row_number() over(partition by name,href,ggstart_time order by ctid) as rn , ctid from %s.gg_src) t 
    where t.rn <>1);
    """ % (conp[4], conp[4])
...
def gg(conp, diqu, i=-1):
    create_gg(conp)
    sql = """
    select table_name from information_schema.tables where table_schema='%s' and table_name ~'_gg_src$' order by table_name
    """ % conp[4]
...
def gg_cdc(conp, diqu, i=-1):
    # create_gg(conp)
    sql = """
    select table_name from information_schema.tables where table_schema='%s' and table_name ~'_gg_src_cdc$' order by table_name
    """ % conp[4]
...
def gg_meta(conp, data, diqu, i=-1, headless=True):
    ...
    if "gg_src" in arr or 'quanguoxiangmu' in arr[0] or 'quanguoxiangmu' in arr[1]:
...
def html_work(conp, f, size=None, headless=True):
    m = page()
    if size is not None:
        sql = "select distinct href from %s.gg_src where not coalesce(info,'{}')::jsonb?'hreftype' or coalesce(info,'{}')::jsonb->>'hreftype'='可抓网页' limit %d" % (
            conp[4], size)
    else:
        sql = "select distinct href from %s.gg_src where not coalesce(info,'{}')::jsonb?'hreftype' or coalesce(info,'{}')::jsonb->>'hreftype'='可抓网页' " % (
            conp[4])
...
"""
update  "weihai"."gg_src"

set info=coalesce(info,'{}')::jsonb ||'{"hreftype":"不可抓网页"}'
 where  href in (select distinct href from weihai.gg_src where href not in(select href from weihai.gg_html ) 
...
def html_cdc(conp, f, headless=True):
    m = page()
    sql = "select distinct href from %s.gg_src where href not in(select href from %s.gg_html ) and (not coalesce(info,'{}')::jsonb?'hreftype' or coalesce(info,'{}')::jsonb->>'hreftype'='可抓网页')" % (
        conp[4], conp[4])
...
def est_tbs(conp, data, **args):
    ...
    sql2 = """
    select table_name from information_schema.tables where table_schema='%s' and table_name ~'^[^t].*gg_src$' order by table_name desc limit 1
    """ % conp[4]
    ...
    if df2['table_name'].tolist() != []:
        ...
        latest_gg_table_name_g = 'quanguoxiangmu_2002_01_01_2003_01_01_gg_src'

    old_year, old_month = re.findall('quanguoxiangmu\_(\d+)\_(\d+)\_\d+\_\d+\_\d+\_\d+\_gg_src', latest_gg_table_name_g)[0]
    ...
    for w in data[-count:]:
        ...
        old_year, old_month = re.findall('quanguoxiangmu\_(\d+)\_(\d+)\_\d+\_\d+\_\d+\_\d+\_gg_src', w[0])[0]
        now_year, now_month = datetime.strftime(datetime.now(), '%Y'), datetime.strftime(datetime.now(), '%m')
        if int(old_year) >= 2016:
            count = (int(now_year) - int(old_year)) * 12 + (int(now_month) - int(old_month))
        else:
...
def est_gg(conp, **arg):
    ...
    for tbname in data:
        insert_tb(tbname, diqu=diqu, conp=conp)
        print('%s 写入gg_src , 成功。' % tbname)

    print('所有_gg_src表写入gg_src成功。')
...
def est_html_work(conp, f, **args):
    ...
    for i in range(turn):
        ...
        sql = "select href from %s.gg_src where href not in(select href from %s.gg_html ) limit %s " % (conp[4], conp[4], limit)
...
def merge_gg(conp):
    ...
    if 'gg' not in tbs1:
        ...
        sql="""
    SELECT  name,ggstart_time ,href ,'审批' as ggtype ,'审批' as jytype,diqu ,
    json_build_object('xiangmu_code' ,xiangmu_code,'shenpishixiang',shenpishixiang,
    'shenpibumen',shenpibumen,'statu',statu) as info 
    ...
    insert into %s.gg
    SELECT distinct on(name,href,ggstart_time) name,ggstart_time ,href ,'审批' as ggtype ,'审批' as jytype,diqu ,
        json_build_object('xiangmu_code' ,xiangmu_code,'shenpishixiang',shenpishixiang,
        'shenpibumen',shenpibumen,'statu',statu) as info
    
    from %s.gg_src  as b where not exists(select 1 from %s.gg as a where a.href=b.href)
