...
def default( value, klass: Type[BaseException] = TypeError ):
    if value is NOT_PROVIDED:
        raise klass( "The value could not be obtained and no default has been provided." )
...
def instance_message( name: str, value: object, type_: TType ) -> str:
    ...
    :return:            The message
    """
    return "Type mismatch on «'{}'» - expected «{}», got «{}», value «{}».".format( name, format_type( type_ ), format_type( type( value ) ), value )


def type_error( name: str, value: object, type_: TType, err_class = TypeError ) -> TypeError:
    """
...
def get_traceback_ex( ex: BaseException = None ) -> TracebackCollection:
