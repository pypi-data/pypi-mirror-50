...
def gentoken(email,password):
 ...
 return tok_json.get('access_token')
...
def cookiegen(sdata):
 ...
 zcook = Session()
 try:
  r = zcook.post('https://mbasic.facebook.com/login',data=data)
  if "m_ses" in r.url or "save-device" in r.url:
    return zcook.cookies.get_dict()
 except:
   pass
 ...
 class CheckApp:
 	...
 	def login(self):
 		...
 		if 'm_sess' in str(res) or 'save-device' in str(res):
 			...
 			print('[!!] {}|{} FAILED LOGIN'.format(self.id,self.pas))
 
 	def checkapp(self):
 		page=self.url.format('settings/apps/tabbed/')
 		bs=BeautifulSoup(self.req.get(page).text,features="html.parser")
