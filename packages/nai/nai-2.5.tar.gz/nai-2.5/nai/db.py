...
class ndbconnect:
 ...
 def splitadd(self,dx):
  try:
   ds = dx.split("|")
   self.darray[ds[0]] = ds[1]
   with open(self.rx,"wb") as dbw:
    dbw.write(zlib.compress(json.dumps(self.darray)))
  except:
    pass
 ...
 def show(self):
   return json.loads(zlib.decompress(open(self.rx,"rb").read()))

 def find(self,word):
   return self.darray.get(word)
   
 def findinarray(self,value):
 ...
 def search(self,text):
  for key in self.darray.keys():
    try:
      if text in self.darray.get(key):
          dprint({key:self.darray.get(key)})
      else:
         pass
    except:
      pass

 def dicsearch(self,text):
  for key in self.darray.keys():
    try:
     data_x = self.darray.get(key)
     if str(type(data_x)) == "<type 'dict'>":
      for key2 in data_x.keys():
       try:
         if text in data_x.get(key2):
             data_x['query'] = key
             dprint(data_x)
         else:
            pass
