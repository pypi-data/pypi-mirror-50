...
#    La fonction utilise la librairie ip2geotools (site de géolocalisation des IP)
#    Exemple:
...
#        try:
#            detail = DbIpCity.get(ip, api_key='free')
#            lon,lat,note = detail.longitude,detail.latitude,'...'
...
#def trace2df(filename,token=0):
#    """
#    permet de transformer un tracert en dataframe contenant hop-RTT1-RTT2-RTT3-IP-lon-lat
#    filename: fichier tracert
#    La fonction utilise la librairie ipinfo (site de géolocalisation des IP), pour fonctionner, il faut un
#    access_token (clé API) en s'inscrivant sur ipinfo.
#    La fonction gère plusieurs clé API qu'il faut rentrer un peu plus bas
#    Exemple:
#        on fait un tracert de www.google.com comme ceci:
#        tracert google.com >trace.txt
#        (sur jupyter on utilise !)
#        Ensuite 
#        df = trace2df("trace.txt","mon access_token")
#    """
#    with open(filename, "r") as inpt:
#        with open("__"+filename, "w") as output:
#            output.write("hop     RTT1 ms1     RTT2 ms2     RTT3 ms3  IP\n")
...
#                    output.write(line)
#    filename = '__'+filename
#    access_tokens = ['ae0bf455a65764'] #mettre ici vos clés API
#    if access_tokens == []:
#        print("Il faut rentrer votre clé API dans la fonction trace2df\nfin ...")
#        return None
...
#    tr['RTT3'] = pd.to_numeric(tr['RTT3'])
#    tr['IP'] = tr['IP'].astype(str)
#    print("\nMofification/suppression des colonnes inutiles")
#    print("recherche IP public...",end='')
#    try:
#        ip = get('https://api.ipify.org').text
#        print(ip)
#        tr.at[0,'IP'] = ip
...
#    details = [handler.getDetails(ip) for ip in tr['IP']]
#    tr['Latitude'] = [float(detail.latitude) for detail in details]
#    tr['Longitude'] = [float(detail.longitude) for detail in details]
#    print("Géolocalisation via Ipinfo\n",tr)
#    tr.drop_duplicates(subset=['Latitude','Longitude'], keep='last',inplace=True)
#    print("Elimination des doublons\n",tr)
#    os.remove(filename)
#    return tr
