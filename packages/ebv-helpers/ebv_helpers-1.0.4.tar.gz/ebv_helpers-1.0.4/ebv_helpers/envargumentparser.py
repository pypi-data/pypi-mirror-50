...
class EnvArgumentParser(argparse.ArgumentParser):
    ...
    def _add_action(self, action):
        ...
        if value is not None:
            ...
            if value == 'None':
                action.default = None
            elif action.type is dict:
                action.default = json.loads(value)
            elif action.nargs in ('+', '*') or isinstance(action.nargs, (int, long)) and action.nargs >= 1:
