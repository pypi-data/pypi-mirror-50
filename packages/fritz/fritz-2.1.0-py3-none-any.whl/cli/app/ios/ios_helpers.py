...
def get_bundle_id(xcodeproj_path: pathlib.Path) -> Optional[str]:
    ...
    """
    cmd = "xcodebuild -project {xcodeproj} -showBuildSettings | grep PRODUCT_BUNDLE_IDENTIFIER"
    cmd = cmd.format(xcodeproj=str(xcodeproj_path))
...
def remove_file_in_xcodeproj(
    project: XcodeProject, filename: str
) -> XcodeProject:
    ...
    for file in files:
        ...
        for build_file in project.objects.get_objects_in_section(
            u"PBXBuildFile"
        ):
