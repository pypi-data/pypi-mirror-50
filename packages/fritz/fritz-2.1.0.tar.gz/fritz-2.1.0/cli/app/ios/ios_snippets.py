...
class FritzInfoAPIKeyUpdater(FritzInfoSnippet):
    ...
    def is_updated(self):
        ...
        existing_path = next(
            self.app_config.root.rglob("Fritz-Info.plist"), None
        )
        if not existing_path:
            return True

        return self.app.api_key == ios_helpers.find_fritz_api_key(
            existing_path
        )
...
class FritzInfoXcodeProject(FritzInfoSnippet):
    ...
    def add(self):
        relative_path = self.path.relative_to(self.app_config.root)
        project = ios_helpers.add_path_to_xcodeproj(
    ...
    def update(self):
        relative_path = self.path.relative_to(self.app_config.root)
        project = ios_helpers.remove_file_in_xcodeproj(
            self.project, relative_path.name
        )
...
class AddFritzToPodfile(PodfileSnippet):
    ...
    def add(self):
        ...
        pod_target_line = "target '{target}'".format(
            target=self.app_config.app_name
        )
