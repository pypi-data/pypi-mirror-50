...
def plot_against(df, name, col, *args):
    plot_agg(df, name, col, "mean", args)

def plot_agg(df, name, col, agg="mean", size=(20, 10)):
    df.groupby(name).agg({col: [agg]}).plot.bar(stacked=True, figsize=size)

def plot_and_save(df, name, col, path, *args):
    plot_against(df, name, col, args)
    plt.savefig(path + f"/{col}-{name}")

def qplot_against(df, name, col, bins, *args):
    qname = f"q{name}"
    df[qname] = pd.qcut(df[name], bins)
    plot_against(df, qname, col, args)
