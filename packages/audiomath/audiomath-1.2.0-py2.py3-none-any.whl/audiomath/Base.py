...
class Sound( object ):
	...
	Concatenation of sound data in time:
		...
		concatenated in time. This can be performed even if there
		is a mismatch in the number of channels.
	...
	def __init__(self, source=None, fs=None, nchan=2, dtype=None, bits=None ):
		"""
	...
	def NumberOfSamples( self ):
		"""
		`s.NumberOfSamples()` returns the length of the sound
		in samples.
		"""
	...
	def Duration(self):
		"""
		`s.Duration()` returns the duration of the sound in
		seconds.
		"""
	...
	def Concatenate( self, *args ):
		...
		(in seconds) of silent intervals. Replace the sample
		array `self.y` with the result. 
		"""
	...
	def Play( self, *pargs, **kwargs ):
		...
		uses it to play the sound, waits for it to finish, then
		destroys the `Player` again.
