...
class Player( GenericInterface.GenericPlayer ):
	...
	def __init__( self, sound, resample=False, device=None, stream=None, verbose=None ):
		...
		if SINGLE_OUTPUT_STREAM:
			...
			if stream is None: stream = DEFAULT_OUTPUT_DEVICE
			self.__stream = stream if isinstance( stream, Stream ) else Stream( device=stream, mode=mode )
