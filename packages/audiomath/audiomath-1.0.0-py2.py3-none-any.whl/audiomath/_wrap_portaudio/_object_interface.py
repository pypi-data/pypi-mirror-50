...
def FindDevice( id=None, mode=None, apiPreferences=None, returnAll=False ):
	...
	if returnAll: return reordered
	if not reordered: raise ValueError( 'could not find a device that matched the criteria' )
	return reordered[ 0 ]

class Stream( object ):
	def __init__( self, device=None, mode=None, apiPreferences=None, outputCallbacks=None, sound=None, sampleRate=None, sampleFormat=None, verbose=None ):
		...
		soundCh = sound.NumberOfChannels() if sound else 0
		soundFs = sound.fs if sound else 0
		if not apiPreferences:
		...
		if device is not None:
			...
			if soundCh:
				nInputChannels  = min( nInputChannels,  soundCh )
				nOutputChannels = min( nOutputChannels, soundCh )
			if not mode:
				if nOutputChannels: mode = 'o' * nOutputChannels
				else: mode = 'i' * nInputChannels
		if not mode and soundCh > 0: mode = 'o' * soundCh
		if not sampleRate and soundFs > 0: sampleRate = soundFs
