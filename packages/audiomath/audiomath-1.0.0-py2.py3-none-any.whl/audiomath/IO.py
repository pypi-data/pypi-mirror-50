...
def Read_AVBin( self, filename, duration=None, verbose=False ):
	...
	self.SetBitDepth( sourceHandle.audio_format.sample_size )
...
def Read( self, source, raw_dtype=None ):
	...
	Args:
		source:
			...
			For filenames are decoded according to their file extension, unless
			the `raw_dtype` argument is explicitly specified, in which case files
		...
		raw_dtype (str):
			...
			overrides the current value of `self.NumberOfChannels()` and interpret the
	...
	for i in range( 32 ):
		...
		isFileName = True
	if isFileName: source = os.path.realpath( source )
