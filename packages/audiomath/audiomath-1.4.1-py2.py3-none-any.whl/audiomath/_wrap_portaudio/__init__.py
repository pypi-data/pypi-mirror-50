...
def GetHostApiInfo():
	"""
	Return a list of records corresponding to PortAudio's
	`Pa_GetHostApiInfo()` output for every available host
	API.   `print()` the result, or otherwise convert it
	to `str()`, to see a pretty tabulated summary.
	"""
...
def GetDeviceInfo():
	...
	combination of host API and device.   `print()` the
	result, or otherwise convert it to `str()`, to see a
	pretty tabulated summary.
	"""
...
def FindDevices( id=None, mode=None, apiPreferences=None ):
	"""
	Call `GetDeviceInfo()`, filtering and reordering the
	outputs according to the specified criteria. `print()`
	the result, or otherwise convert it to `str()`, to see
	a pretty tabulated summary.
	
	Use `FindDevice()` (singular) to return just the top
	ranking result (and assert its existence).
	...
	Args:
		id (int, dict, str, None):
			...
			- `int`: matches only the device with whose
			  `index` field matches `id`
			- `dict`: matches only the device with whose
			  `index` field matches `id['index']`
...
def FindDevice( id=None, mode=None, apiPreferences=None ):
	"""
	Return the first device matched by `FindDevices()`
	(plural) according to the specified criteria. Raise an
...
def Tabulate( records, *fields ):
	"""
	Return a pretty-printed table of the specified `fields`
