...
class Player( GenericInterface.GenericPlayer ):
	...
	def __init__( self, sound, resample=False, device=None, stream=None, verbose=None, **kwargs ):
		"""DOC-TODO (PortAudioInterface-specific)"""
		...
		device = stream.outputDevice if isinstance( stream, Stream ) else FindDevice( id=stream, mode=mode )
		if not SINGLE_OUTPUT_STREAM and any( word.startswith( 'ASIO' ) for word in device[ 'name' ].upper().split() ): SINGLE_OUTPUT_STREAM = True
		if SINGLE_OUTPUT_STREAM:
			...
			if not isinstance( stream, Stream ):
				...
				else: stream = Stream( device=stream, mode=mode )
			...
			self.__stream = stream if isinstance( stream, Stream ) else Stream( device=device[ 'index' ], mode=mode )
...
class Recorder( GenericInterface.GenericRecorder ):
	__stream = __verbose = None
	def __init__( self, seconds, stream=None, device=None, start=True, verbose=None ):
		"""DOC-TODO"""
		if stream is None and device is not None: stream = device
		if device is None and stream is not None: device = stream
		if device != stream: raise ValueError( 'the `stream` and `device` arguments are synonymous---they cannot take different values unless one is left as `None`' )
