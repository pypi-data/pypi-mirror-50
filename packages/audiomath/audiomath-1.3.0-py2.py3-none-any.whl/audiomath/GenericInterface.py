...
from . import Playlist;       from .Playlist import Queue, QueueError
...
class GenericPlayer( object ):
	...
	def _OutputCallback( self, t, outputFrameCount, numberOfChannels, sampleFormat ):
		...
		if speed == 1.0:
			...
			self._nextSample = sampleSub[ -1 ] + 1
		else:
			if self._sampleSubParams != sampleSubParams:
				self._sampleSubParams = sampleSubParams
			...
			self._nextSample = self._nextSample + outputFrameCount * speed
		...
		# The logic is already implemented in `snd.dat2str( data=source, dtype=sampleFormat )` 
	...
	def ResetDynamics( self, global_t0=None ):
		"""
		DOC-TODO
		"""
		if self.__synchronizeDynamics:
			...
			self.__t0_unsynch = self.__t0_global[ 0 ] if global_t0 else None
	...
	def synchronizeDynamics( self ):
		"""
		Many properties, such as `.volume`, `.speed`, `.pan`,
		`.levels` and even `.playing`, can be dynamic, i.e.
		they can be set to functions of time `t` in seconds.
		...
		will only play once::
			...
			p.Play(0, loop=True, synchronizeDynamics=True, playing=lambda t:(t/p.sound.duration)%2<1 )
		...
		repeatedly, with pauses::
			...
			p.Play(0, loop=True, synchronizeDynamics=False, playing=lambda t:(t/p.sound.duration)%2<1 )
	...
	def dynamics( self ):
		"""
		DOC-TODO - explain general dynamics concept here, refer to `.synchronizeDynamics`
		"""
		return tuple( ( name, func ) for order, name, func, nArgs in sorted( self._dynamics.values() ) )
	...
	def queue( self, value ):
		...
		self.__sound = self.__queue.currentSound
	...
	def autoAdvance( self ):
		...
		- `False`:  when the current `.sound` finishes playing,
		  rewind the `.head` to the start of the current sound
		  but do not advance through the `.queue`.
		
		- `True` or `'play'`: when the current `.sound` finishes
		  playing, continue to the next `Sound` instance in the
		  `.queue`, if any.
		  
		- `'pause'`: when the current `.sound` finishes playing,
		  advance to the next `Sound` instance in the `.queue`,
...
class GenericRecorder( object ):
	...
	def WaitFor( self, condition, finalRecordingStatus=None ):
		...
		argument may be:
			...
			a string:
				in which case the method will use the
				string as a prompt and wait for the user
				to enter any string (even the empty string)
				on the console;
		...
		to end an ongoing recording as follows::
			...
			r.WaitFor('Press return to stop recording: ', finalRecordingStatus=False)
		"""
	...
	def Cut( self, position=None ):
		"""
		Stop recording, shorten `self.sound` by cutting it off at
		the current `.head` position (or the explicitly specified
		`position` in seconds), and return the `Sound` instance.
		"""
		if position is None: position = self.head
		if position: self.sound = self.sound[ :float( position ) ] # there was a reason for not cutting when position is 0
		return self.sound

	@classmethod
	def MakeRecording( cls, space=60, prompt='Recording for {duration:g} seconds - press return to finish early: ', verbose=None, cut=True, **kwargs ):
		"""
		Record and return a `Sound`.
		...
		Args:
			...
			prompt (str):
				If supplied, this provides console interaction:
				the prompt is printed, and you can press return
				to end the recording early. If empty or `None`,
				you can still stop the recording early with ctrl-C.
...
class Fader( object ):
	...
	the `Player` property will automatically be replaced
	with the final static value.
	
	Example::
	
		import time, audiomath as am
		p = am.Player(am.TestSound('12'))
		p.Play(loop=1, volume=am.Fader(3.0, start=0, end=1)) # fade in
		time.sleep(6.0)           # wait for fade-in, and stay constant for a few seconds
