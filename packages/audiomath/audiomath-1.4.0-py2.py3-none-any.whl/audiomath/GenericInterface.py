...
class GenericRecorder( object ):
	...
	`Record`, which returns a `Sound` instance, than to
	create or interact directly with `Recorder` instances.
	"""
	...
	def Record( self, position=None, wait=True ):
		...
		move the recording `.head` to that position, in
		seconds from the beginning, before starting.
		If `wait` is true, `Wait()` until the end, or
