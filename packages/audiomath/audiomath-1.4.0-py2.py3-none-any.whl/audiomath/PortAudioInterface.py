...
class Player( GenericInterface.GenericPlayer ):
	...
	def __init__( self, sound, device=None, stream=None, resample=False, verbose=None, **kwargs ):
		"""
		Args:
			sound (Sound, Queue, None):
				`Sound` instance (or list of `Sound` instances in
				a `Queue`) to play.
			...
			resample (bool):
				how to handle potential mismatch between the sampling
				frequency of the sound data `self.sound.fs` and the
				sampling frequency of the output stream
				`self.stream.fs`.  If true, replace `self.sound` with
				a copy resampled to the stream's preferred rate.
				If false, simply adjust playback speed accordingly.
				
			verbose (bool, None):
				verbosity for debugging. If `None`, inherit
				from the setting specified by
				`SetDefaultVerbosity()`, if any.
...
class Recorder( GenericInterface.GenericRecorder ):
	__stream = __verbose = None
	def __init__( self, seconds, device=None, stream=None, start=True, verbose=None ):
		"""
		Args:
			seconds (float, Sound):
