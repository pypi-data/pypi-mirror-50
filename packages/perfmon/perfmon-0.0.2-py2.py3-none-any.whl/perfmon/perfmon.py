...
class PerfMon(object):
    ...
    def get_adb_su(self):
        ...
        sub_result_adbroot = subprocess_cmd_v2(self.adb_shell_pre +  ' service call SurfaceFlinger 1013')
        if len(sub_result_adbroot) >0:
            if 'Error' not in sub_result_adbroot :
                self.root=1
                self.adb_shell_root_pre = self.adb_shell_pre
                return
        sub_result_suc = subprocess_cmd_v2(self.adb_shell_pre +  ' su -c service call SurfaceFlinger 1013')
        if len(sub_result_suc) >0:
            if ( 'Error' not in sub_result_suc) and ('Parcel' in  sub_result_suc) :
                ...
                return
        sub_result_suroot = subprocess_cmd_v2(self.adb_shell_pre +  ' su root service call SurfaceFlinger 1013')
    ...
    def get_sdk_version(self):
        logging_info(sys._getframe().f_code.co_name)
        sub_result = subprocess_cmd_v2(self.adb_shell_pre + ' getprop ro.build.version.sdk')
        if len(sub_result) > 0:
    ...
    def get_pid_uid(self):
        ...
        for i in range(1,50):
            if self.sdk_version < 26:
                str_cpu = subprocess_cmd_v2(self.adb_shell_pre  + ' ps | '+ self.grep + self.process)
            else:
                str_cpu = subprocess_cmd_v2(self.adb_shell_pre  + ' ps -A | '+ self.grep + self.process)
    ...
    def cacl_pid_cpu_proc(self):# adb -s HT73A0203875 shell cat /proc/10099/stat
        logging_info(sys._getframe().f_code.co_name)
        str_pid_cpu = subprocess_cmd_v2(self.adb_shell_pre + ' cat /proc/' + self.pid + '/stat ')
    ...
    def cacl_total_cpu_proc(self):
        logging_info(sys._getframe().f_code.co_name)
        str_total_cpu = subprocess_cmd_v2(self.adb_shell_pre + ' cat /proc/stat ')
    ...
    def cacl_ram(self):
        ...
        if self.model=='perfmon_native':
            str_ram = subprocess_cmd_v2(self.adb_shell_pre + ' dumpsys meminfo '+ self.process)
            ...
            if self.root !=0:
                str_ram = subprocess_cmd_v2(self.adb_shell_root_pre + ' cat /proc/' + self.pid + '/smaps | '+ self.grep + ' Pss')
                for str_line in str_ram.split('\n'):
                    if len(str_line) > 0:
                        str_line_list = filter(None, str_line.replace('\n','').replace('\t','').split(' '))
                ...
                str_ram = subprocess_cmd_v2(self.adb_shell_pre + ' top -d 1 -n 1 | '+ self.grep +' ' + self.pid)
                for str_line in str_ram.split('\n'):
                    if len(str_line) > 0:
                        str_line_list = filter(None, str_line.replace('\n','').replace('\t','').split(' '))
    ...
    def cacl_fps(self):
        ...
        if self.fps_count == 0:
            self.fps_last_frame_num = self.get_fps_frame_num(subprocess_cmd_v2(self.get_str_fps()))
        self.fps_current_frame_num = self.get_fps_frame_num(subprocess_cmd_v2(self.get_str_fps()))
    ...
    def cacl_traffic_sed(self):
        logging_info(sys._getframe().f_code.co_name)
        str_sed_traffic = subprocess_cmd_v2(self.adb_shell_root_pre + ' cat /proc/uid_stat/' + str(self.uid) + '/tcp_snd')
    ...
    def cacl_traffic_rcv(self):
        logging_info(sys._getframe().f_code.co_name)
        str_rcv_traffic = subprocess_cmd_v2(self.adb_shell_root_pre + ' cat /proc/uid_stat/' + str(self.uid) + '/tcp_rcv')
    ...
    def cacl_battery(self):
        ...
        if self.sdk_version >= 21:
            if self.batteryFirst == 1 :
                str_battery = subprocess_cmd_v2(self.adb_shell_pre + ' dumpsys batterystats –enable full-wake-history ')
                str_battery = subprocess_cmd_v2(self.adb_shell_pre + ' shell dumpsys batterystats --reset')
                str_battery = subprocess_cmd_v2(self.adb_shell_pre + ' shell  logcat -c')
                self.batteryFirst = 0
            str_battery = subprocess_cmd_v2(self.adb_shell_pre + ' bugreport >  bugreport.txt')
            str_battery = subprocess_cmd_v2(self.adb_shell_pre + ' dumpsys battery')
...
if __name__ == '__main__':
    test = PerfMon()
    test.set_param('HT7AV1A00994','com.autonavi.auto','perfmon_cpu')  #nexus 6p
