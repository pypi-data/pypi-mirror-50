...
class Pax:
    ...
    def __init__(self):
        ...
        try:
            data = self.__read_local_config()
            self.api_endpoint = data["api_endpoint"]
            log.info("Local data loaded", data)

        except Exception as e:
            log.info("Local config not found. Please pax configure.", 
            {
                "error": str(e)
            })
    ...
    def initialize(self):
        ...
        link = self.get_remote_git_branch()
    ...
    def get_remote_git_branch():
        ...
        process.close()
        remote_url = remote_origin.replace(".git", "")  # Remove the .git extension.
        return remote_url
