...
data_path = af.path_util.make_and_return_path_from_path_and_folder_names(
    path=workspace_path, folder_names=["data", data_type]
)
...
image_post_cti = image_pre_cti.add_cti_to_image(
...
image_post_cti = cti_image.ImageFrame(
    frame_geometry=frame_geometry, array=image_post_cti
)

# Now, output every image to the data folder.
image_pre_cti.output_as_fits(
    file_path=data_path + "/image_pre_cti.fits", overwrite=True
)
image_post_cti.output_as_fits(
    file_path=data_path + "/image_post_cti.fits", overwrite=True
)
