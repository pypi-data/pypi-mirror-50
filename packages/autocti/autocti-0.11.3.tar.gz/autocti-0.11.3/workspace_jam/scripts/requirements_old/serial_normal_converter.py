...
most_likely, most_probable, lower_limits, upper_limits, lower_error, upper_error, error_pecision = req.setup_multinest_results(
...
delta_ellipticity_model_ml = convert.convert_to_ellipticity(
    rho=model_ml_rho, tau=model_ml_tau
)
...
print(
    np.abs(np.sum(delta_ellipticity_model_ml) - np.sum(delta_ellipticity_true))
    / (1.1e-4)
)
print(
    np.abs(np.sum(delta_ellipticity_model) - np.sum(delta_ellipticity_true)) / (1.1e-4)
)
print(
    np.abs(np.sum(delta_ellipticity_upper) - np.sum(delta_ellipticity_true)) / (1.1e-4)
)
print(
    np.abs(np.sum(delta_ellipticity_lower) - np.sum(delta_ellipticity_true)) / (1.1e-4)
)
