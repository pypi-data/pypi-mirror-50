...
from autoaf.core import non_linear as nl
...
def setup_multinest_results(
    model_mapper, name, input_model, sigma_limit=2.0, requirements=None
):

    multinest = af.MultiNest(
        model_mapper=model_mapper, name=name, sigma_limit=sigma_limit
    )
    ...
    error_precision = list(
        map(lambda upper, lower: upper - lower, upper_limits, lower_limits)
    )

    most_probable_difference = list(
        map(lambda input, mp: mp - input, input_model, most_probable)
    )
    upper_limit_difference = list(
        map(lambda input, upper: upper - input, input_model, upper_limits)
    )
    lower_limit_difference = list(
        map(lambda input, lower: lower - input, input_model, lower_limits)
    )
