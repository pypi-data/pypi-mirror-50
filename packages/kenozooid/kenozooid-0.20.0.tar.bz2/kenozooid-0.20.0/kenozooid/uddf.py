    ...
    doc = xml.uddf(
        xml.generator(
            ...
            xml.manufacturer(
                ...
                xml.contact(xml.homepage('https://wrobell.dcmod.org/kenozooid/')),
                id='kenozooid'),
...
def create_dives(dives, equipment=None):
    ...
    """
    f = partial(create_dive, equipment=equipment)
    yield from (f(dive) for dive in dives)
...
def create_dive(dive, equipment=None):
    ...
    return xml.dive(
        ...
        None if not dive.profile else xml.samples(create_dive_samples(dive.profile, dive.mode)),
