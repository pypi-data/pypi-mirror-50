...
class SensusUltraUDDFTestCase(unittest.TestCase):
    ...
    def test_profile_conversion(self):
        ...
        # 43 dives
        self.assertEqual(43, len(dives))

        dive = dives[-1] # su stores data from last to first
        self.assertEqual(datetime(2009, 9, 19, 13, 14, 40), dive.datetime)

        self.assertEqual(12.0, dive.depth)
        self.assertEqual(170, dive.duration)
        self.assertEqual(288.6, dive.temp)
        ...
        self.assertEqual(12.0, round(max(s.depth for s in samples), 1))
        self.assertEqual(288.6, min(s.temp for s in samples
            if s.temp is not None))

        # check first sample, UDDF obligatory one
        self.assertEqual(0, samples[0].time)
        self.assertEqual(0.0, samples[0].depth)

        # check if second sample does not start with 0 time
        self.assertEqual(SU_DATA_INTERVAL, samples[1].time)

        # check last UDDF required sample
        self.assertEqual(170, samples[-1].time) # dive.duration above should
                                                 # equal to this value, too
        self.assertEqual(0.0, samples[-1].depth)

        # check time of the one sample before last
        self.assertEqual(160, samples[-2].time)

        # check the one before last and last samples times
        t1 = samples[-2].time
        t2 = samples[-1].time
        self.assertEqual(SU_DATA_INTERVAL, t2 - t1) 
...
class ParserTestCase(unittest.TestCase):
    ...
    def test_handshake(self):
        ...
        v = _handshake(data)
        self.assertEqual(3, v.ver2)
        self.assertEqual(2, v.ver1)
        self.assertEqual(7524, v.serial)
        self.assertEqual(65257038, v.time)
    ...
    def test_dive_header(self):
        ...
        v = _dive_header(data)
        self.assertEqual(40249564, v.time)
        self.assertEqual(10, v.interval)
        self.assertEqual(1111, v.threshold)
        self.assertEqual(15, v.endcount)
        self.assertEqual(1, v.averaging)
...
class DataParserTestCase(unittest.TestCase):
    ...
    def test_version(self):
        ...
        self.assertEqual('Sensus Ultra 3.2', ver)
    ...
    def test_gases(self):
        ...
        self.assertEqual((), tuple(drv.gases(data)))
