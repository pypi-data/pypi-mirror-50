...
class FindDataTestCase(unittest.TestCase):
    ...
    def _qt(self, xml, query, expected, **data):
        ...
        node = nodes[0]
        self.assertEqual(expected, node.get('id'), nodes)
    ...
    def test_dive_data(self):
        ...
        self.assertEqual(301, dive.number)
        self.assertEqual(datetime(2009, 9, 19, 13, 10, 23), dive.datetime)
        self.assertEqual(20, dive.duration)
        self.assertEqual(30.2, dive.depth)
        self.assertEqual(251.4, dive.temp)
        self.assertEqual(10.1, dive.avg_depth)
        self.assertEqual('opencircuit', dive.mode)
        self.assertEqual(3, len(list(dive.profile)))
    ...
    def test_profile_data(self):
        ...
        self.assertEqual(kd.Sample(depth=2.61, time=0, temp=296.73), profile[0])
        self.assertEqual(kd.Sample(depth=4.18, time=10), profile[1])
        self.assertEqual(kd.Sample(depth=6.25, time=20), profile[2])
        self.assertEqual(kd.Sample(depth=8.32, time=30, temp=297.26), profile[3])
    ...
    def test_dump_data_decode(self):
        ...
        self.assertEqual(b'01234567890abcdef', s)
...
"""
        self.assertEqual((3, 0), ku.get_version(BytesIO(s1)))
        self.assertEqual((3, 1), ku.get_version(BytesIO(s2)))
...
class CreateDataTestCase(unittest.TestCase):
    ...
    def test_create_basic(self):
        ...
        self.assertEqual('3.2.0', doc.get('version'))

        q = '//uddf:generator/uddf:datetime/text()'
        dt = ku.xp_first(doc, q)
        self.assertEqual(ku.FMT_DT(now), dt)
    ...
    def test_set_data(self):
        ...
        self.assertEqual(1, len(divers), sd)
        self.assertTrue(divers[0].text is None, sd)

        fnames = doc.xpath('//firstname/text()')
        self.assertEqual(1, len(fnames), sd)
        self.assertEqual('A', fnames[0], sd)

        lnames = doc.xpath('//lastname/text()')
        self.assertEqual(1, len(lnames), sd)
        self.assertEqual('B', lnames[0], sd)
        ...
        fnames = doc.xpath('//firstname/text()')
        self.assertEqual(2, len(fnames), sd)
        self.assertEqual(['A', 'X'], fnames, sd)
    ...
    def test_create_data_list(self):
        ...
        self.assertEqual(1, len(streets), sd)
        self.assertEqual('X', streets[0], sd)

        cities = doc.xpath('//city/text()')
        self.assertEqual(1, len(cities), sd)
        self.assertEqual('Y', cities[0], sd)
        ...
        fnames = doc.xpath('//firstname/text()')
        self.assertEqual(2, len(fnames), sd)
        self.assertEqual(['A', 'A1'], fnames, sd)
    ...
    def test_create_attr_data(self):
        ...
        divers = doc.xpath('//diver')
        self.assertEqual(2, len(divers), sd)
        self.assertTrue(divers[0].text is None, sd)
        self.assertTrue(divers[1].text is None, sd)

        fnames = doc.xpath('//diver/@fn')
        self.assertEqual(1, len(fnames), sd)
        self.assertEqual('A1', fnames[0], sd)

        lnames = doc.xpath('//diver/@ln')
        self.assertEqual(1, len(lnames), sd)
        self.assertEqual('B1', lnames[0], sd)

        ku.set_data(doc, fq, fname='A2', lname='B2')
        sd = et.tostring(doc)

        divers = doc.xpath('//diver')
        self.assertEqual(3, len(divers), sd)

        fnames = doc.xpath('//diver/@fn')
        self.assertEqual(2, len(fnames), sd)
        self.assertEqual(['A1', 'A2'], fnames, sd)

        lnames = doc.xpath('//diver/@ln')
        self.assertEqual(2, len(lnames), sd)
        self.assertEqual(['B1', 'B2'], lnames, sd)
    ...
    def test_create_attr_list_data(self):
        ...
        self.assertEqual(2, len(links), sd)
        self.assertEqual('A1', links[0].get('ref'), sd)
        self.assertEqual('A2', links[1].get('ref'), sd)
    ...
    def test_create_attr_list_data_reuse(self):
        ...
        t = doc.xpath('//t')
        self.assertEqual(3, len(t), sd)
        self.assertEqual('1', t[0].get('a'), sd)
        self.assertEqual('2', t[0].get('b'), sd)
        self.assertEqual('A1', t[1].get('a'), sd)
        self.assertEqual('A2', t[1].get('b'), sd)
        self.assertEqual('A3', t[2].get('a'), sd)
        self.assertEqual('A4', t[2].get('b'), sd)
    ...
    def test_create_node(self):
        ...
        self.assertEqual('diver', d.tag)
        self.assertEqual('test', t.tag)

        *_, = ku.create_node('diver/test', parent=doc)
        sd = et.tostring(doc, pretty_print=True)
        self.assertEqual(1, len(dq(doc)), sd)
        self.assertEqual(1, len(tq(doc)), sd)

        *_, = ku.create_node('diver/test', parent=doc)
        sd = et.tostring(doc, pretty_print=True)
        self.assertEqual(1, len(dq(doc)), sd)
        self.assertEqual(2, len(tq(doc)), sd)

        # verify the parent order
        self.assertEqual(['a', 'diver', 'b'],
    ...
    def test_create_node_prepend(self):
        ...
        nodes = tq(doc)
        self.assertEqual(2, len(nodes), sd)
        # test order
        self.assertEqual('test1', nodes[0].tag)
        self.assertEqual('test2', nodes[1].tag)
    ...
    def test_create_dive_data_with_dive_site(self):
        ...
        self.assertEqual('markgraf', site_id, s)
    ...
    def test_create_dive_data_with_buddy(self):
        ...
        self.assertEqual('b1', b1, s)
        self.assertEqual('b2', b2, s)
    ...
    def test_create_dive_samples(self):
        ...
        self.assertEqual('19', ku.xp_first(n, '//divetime/text()'), sd)
        self.assertEqual('3.1', ku.xp_first(n, '//depth/text()'), sd)
        self.assertEqual('20.0', ku.xp_first(n, '//temperature/text()'), sd)
        self.assertEqual(None, ku.xp_first(n, '//divemode/@type'), sd)
    ...
    def test_create_dive_samples_deco_alarm(self):
        ...
        samples = ku.create_dive_samples(s)
        n, sd = xml2et(xml.dive(samples))
        self.assertEqual('deco', ku.xp_first(n, '//alarm/text()'), sd)
    ...
    def test_create_dive_samples_setpoint(self):
        ...
        samples = ku.create_dive_samples(s)
        n, sd = xml2et(xml.dive(samples))
        self.assertEqual('1.17', ku.xp_first(n, '//setpo2/text()'), sd)
        self.assertEqual('user', ku.xp_first(n, '//setpo2/@setby'), sd)
    ...
    def test_create_dive_samples_dive_mode(self):
        ...
        self.assertEqual(['opencircuit'], modes)
        ...
        n, sd = xml2et(xml.dive(samples))
        modes = list(ku.xp(n, '//divemode/@type'))
        self.assertEqual(['closedcircuit'], modes)
    ...
    def test_dump_data_encode(self):
        ...
        self.assertEqual(b'QlpoOTFBWSZTWZdWXlwAAAAJAH/gPwAgACKMmAAUwAE0xwH5Gis6xNXmi7kinChIS6svLgA=', s)
    ...
    def test_create_site(self):
        ...
        self.assertEqual('markgraf', id, s)
        ...
        self.assertEqual(1, len(d), s)
        self.assertEqual('Scapa Flow', d[0], s)
        ...
        self.assertEqual(['markgraf', 'konig'], ids, s)
    ...
    def test_create_site_with_pos(self):
        ...
        self.assertEqual(6.1, float(x))
        y = ku.xp_first(f, '//uddf:site//uddf:latitude/text()')
        self.assertEqual(6.2, float(y))
    ...
    def test_create_buddy(self):
        ...
        self.assertEqual('CFT', d[0], s)

        d = list(ku.xp(f,
            '//uddf:buddy/uddf:personal/uddf:membership/@memberid'))
        self.assertEqual('123', d[0], s)
        ...
        self.assertEqual(['tcora', 'tcora2'], ids, s)
...
"""))
        ...
        self.assertEqual(['2009-03-02T23:02:00', '2009-04-02T23:02:00'], times)
...
class RangeTestCase(unittest.TestCase):
    ...
    def test_simple(self):
        ...
        """
        self.assertEqual('1 <= n and n <= 3', ku.parse_range('1-3'))
        self.assertEqual('n == 2 or n == 4', ku.parse_range('2, 4'))
        self.assertEqual('n == 1 or n == 3 or 4 <= n and n <= 7',
            ku.parse_range('1,3,4-7'))
        self.assertEqual('1 <= n', ku.parse_range('1-'))
        self.assertEqual('n <= 10', ku.parse_range('-10'))
    ...
    def test_in_range_query_all(self):
        ...
        self.assertEqual(['d01', 'd02', 'd03'], ids)
    ...
    def test_in_range_query(self):
        ...
        self.assertEqual(['d02'], dt)

        q = '//uddf:dive[in-range(position(), "2-3")]/@id'
        dt = list(ku.find(f, ku.XPath(q)))
        self.assertEqual(['d02', 'd03'], dt)
...
"""
        ...
        with ku.NodeCopier(t) as nc:
            r = nc.copy(a, c)
        self.assertEqual('{http://www.streit.cc/uddf/3.2/}a1', r.tag)
...
"""
        ...
        self.assertEqual(2, len(n), sd)
        self.assertEqual('id-a1', n[0].get('id'))
        self.assertEqual('id-a2', n[1].get('id'))
...
"""
        ...
        self.assertTrue(n is not None, sd)
        self.assertEqual(2, len(n), sd)
        self.assertEqual('id-a21', n[0].get('id'), sd)
        self.assertEqual('id-a24', n[1].get('id'), sd)
...
"""
        ...
        n = ku.xp_first(t, '//uddf:a1')
        self.assertTrue(n is not None, sd)
        self.assertEqual(1, len(n), sd)
        self.assertEqual('id-a4', n[0].get('id'), sd)
...
"""
    ...
    def test_save_filename(self):
        ...
        self.assertEqual('<?xml vers', data)
...
def test_create_dive():
    ...
    assert '19' == ku.xp_first(n, '//divetime/text()')
    assert '3.1' == ku.xp_first(n, '//depth/text()')
    assert '20.0' == ku.xp_first(n, '//temperature/text()')
    assert ku.xp_first(n, '//divemode/@type') is None


def test_create_dive_no_profile():
    """
    Test UDDF dive creation without dive profile.
    """
    d = kd.Dive(
        datetime=datetime(2012, 2, 25),
        depth=32.5,
    ...
    assert '2012-02-25T00:00:00' == ku.xp_first(n, '//datetime/text()')
    assert '32.5' == ku.xp_first(n, '//greatestdepth/text()')
    assert '45' == ku.xp_first(n, '//diveduration/text()')
