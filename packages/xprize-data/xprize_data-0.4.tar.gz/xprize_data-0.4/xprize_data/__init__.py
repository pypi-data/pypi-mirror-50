...
def getDatasets(apiKey, container="public", topic=""):
    ...
    container_name = container  #"public"
    folder_name = topic #file name starts with

    #get SAS token
    azure_storage_sas_token = __getSASToken(apiKey,container)

    #url = 'https://functions-pop.azurewebsites.net/api/blob-sas-token-generator?code=' + apiKey
    #payload = {'container':container_name, 'blobname':'', 'permissions':'Read,List'}
...
def __getSASToken(apiKey, container_name):
     #get SAS token
    url = 'https://functions-pop.azurewebsites.net/api/blob-sas-token-generator?code=' + apiKey
    payload = {'container':container_name, 'blobname':'', 'permissions':'Read,List'}
    headers = {'content-type': 'application/json'}
...
def downloadFile(apiKey,blobName,container_name="public"):
    clear_output()
    azure_storage_account_name = "datacommons"
    targetBlobName = blobName
    azure_storage_sas_token = __getSASToken(apiKey,container_name)

    blob_service = BlockBlobService(account_name = azure_storage_account_name, sas_token = azure_storage_sas_token,)

    #Download file
    clear_output()
    if targetBlobName is not None:
        print('Downloading file ' + targetBlobName)
        _, filename = os.path.split(targetBlobName)
        downloaded_file=blob_service.get_blob_to_path(container_name, targetBlobName, filename,progress_callback=__progress_callback)
   

#Download Callback (Private)
def __progress_callback(current, total):
    print ('Progress...', end=" ")
    print (current, end =" / ")
    print (total)
