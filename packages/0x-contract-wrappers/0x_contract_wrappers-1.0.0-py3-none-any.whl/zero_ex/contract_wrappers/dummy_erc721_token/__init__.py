...
class ApproveMethod(ContractMethod):
    ...
    def call(
        ...
        `msg.sender` is the current NFT owner, or an authorizedoperator of the
        current owner.
...
class TransferFromMethod(ContractMethod):
    ...
    def call(
        ...
        Throws unless `msg.sender` is the current owner, an authorizedoperator,
        or the approved address for this NFT. Throws if `_from` isnot the
        current owner. Throws if `_to` is the zero address. Throws if`_tokenId`
        is not a valid NFT.
...
class MintMethod(ContractMethod):
    ...
    def call(
        ...
        Function to mint a new tokenReverts if the given token ID already
        exists
...
class OwnerOfMethod(ContractMethod):
    ...
    def call(
        ...
        NFTs assigned to zero address are considered invalid, and queriesabout
        them do throw.
...
class BalanceOfMethod(ContractMethod):
    ...
    def call(self, _owner: str, tx_params: Optional[TxParams] = None) -> int:
        ...
        NFTs assigned to the zero address are considered invalid, and
        thisfunction throws for queries about the zero address.
...
class BurnMethod(ContractMethod):
    ...
    def call(
        ...
        Function to burn a tokenReverts if the given token ID doesn't exist or
...
class SetApprovalForAllMethod(ContractMethod):
    ...
    def call(
        ...
        Emits the ApprovalForAll event. The contract MUST allowmultiple
        operators per owner.
...
class SafeTransferFrom2Method(ContractMethod):
    ...
    def call(
        ...
        or the approved address for this NFT. Throws if `_from` isnot the
        current owner. Throws if `_to` is the zero address. Throws if`_tokenId`
        is not a valid NFT. When transfer is complete, this functionchecks if
        `_to` is a smart contract (code size > 0). If so, it
        calls`onERC721Received` on `_to` and throws if the return value is not-
