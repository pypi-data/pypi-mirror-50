...
__COLLAPSE_KEYS = {"USE", "ACCEPT_LICENSE", "ACCEPT_KEYWORDS"}
...
def create_config_placeholder():
    ...
    with open(env.PORTMOD_CONFIG, "w") as file:
        config_string = f"""# This is a placeholder config file for Portmod {get_version()}
...
def read_config(path: str, old_config: Dict) -> Dict:
    """
    ...
    if isinstance(new_config.get("USE", ""), str):
        new_config["USE"] = set(new_config.get("USE", "").split())
    new_config["ARCH"] = new_config.get("ARCH")
    new_config["TEXTURE_SIZE"] = new_config.get("TEXTURE_SIZE")

    if isinstance(new_config.get("ACCEPT_KEYWORDS", ""), str):
        new_config["ACCEPT_KEYWORDS"] = new_config.get("ACCEPT_KEYWORDS", "").split()
    if isinstance(new_config.get("ACCEPT_LICENSE", ""), str):
        new_config["ACCEPT_LICENSE"] = new_config.get("ACCEPT_LICENSE", "").split()
    if isinstance(new_config.get("PORTMOD_MIRRORS", ""), str):
        new_config["PORTMOD_MIRRORS"] = new_config.get("PORTMOD_MIRRORS", "").split()
...
def get_config() -> Dict:
    ...
    if os.path.exists(env.PORTMOD_CONFIG):
        total_config = read_config(env.PORTMOD_CONFIG, total_config)
    else:
