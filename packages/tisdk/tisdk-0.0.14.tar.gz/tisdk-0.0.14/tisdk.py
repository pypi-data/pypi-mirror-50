...
class Ti:
    ...
    def request(self, method, url, data=None):
        return ti_request(self.username, self.secret_key, method, url, data)
...
def ti_request(username, secret_key, method, url, data=None):
    if not url.startswith('/'):
        url = '/' + url
    if 'http' not in url:
        url = 'http://taiqiyun.wowfintech.cn' + url
...
def main():
    ...
    len_args = len(args)
    if len_args == 1 and args[1] == 'demo':
        # demo request
        username = 'to_9123456789_grs-open-match'
        secret_key = '4fbe6e7084ec4d05'
        method = 'post'
        url = 'http://taiqiyun.wowfintech.cn/api/grs-open-match/v1/'
        data = 'name:oppo'
    elif len_args > 4:
        username = args[1]
        secret_key = args[2]
