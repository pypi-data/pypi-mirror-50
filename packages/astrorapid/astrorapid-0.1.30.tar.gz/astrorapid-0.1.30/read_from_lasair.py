...
def read_lasair_json(object_name='ZTF18acsovsw'):
    ...
    if photoZ is None:  # TODO: Get correct redshift
        try:
            if "z=" in tns_info:
                ...
                redshift = 0.036#0.1  # return  0.019 #
    ...
    for cand in data['candidates']:
        ...
        if 'sigmapsf' in cand:
            ...
            magerr.append(0.01 * cand['magpsf'])  #magerr.append(None)  #magerr.append(0.1 * cand['magpsf'])  #
            photflag.append(0)
...
def classify_lasair_light_curves(object_names=('ZTF18acsovsw',), plot=True, figdir='.'):
    ...
    import sys
    np.savez('save_real_ZTF_unprocessed_flux_snia_osc', peakflux_g=peakfluxes_g, peakflux_r=peakfluxes_r, redshifts=redshifts)
    import pickle
    with open('save_real_mags_snia_osc.pickle', 'wb') as f:
        pickle.dump([object_names, mjds, passbands, mags, magerrs, zeropoints, photflags], f)

    # np.savetxt('/Users/danmuth/PycharmProjects/astrorapid/paper_plots/NT_past_year.csv', np.array([obj_names, ras, decs]).T, fmt="%s")
    ...
    # real_tde_probs = []
    # names = []
    # real_TDEs = ('ZTF19aapreis', 'ZTF18abxftqm', 'ZTF19aabbnzo', 'ZTF19aarioci')
    # radec = []
    # peakmags = []
    # for objidx in range(len(predictions)):
    #     for i, name in enumerate(classification.class_names):
    #         if name == 'TDE':
    #             print(predictions[objidx][:,i].shape)
    #             print(predictions[objidx][:,i])
    #             prob = max(predictions[objidx][:,i])
    #             object_name = obj_names[objidx]
    #             if isinstance(object_name, tuple):
    #                 object_name, z_in = object_name
    #             if object_name in real_TDEs:
    #                 real_tde_probs.append(prob)
    #                 names.append(object_name)
    #             probabilities.append(prob)
    #             radec.append((ras[objidx], decs[objidx]))
    #             peakmags.append((peakmags_g[objidx], peakmags_r[objidx]))
    # print(radec)
    # np.savez('plot_nuclear_transients2_new', real_tde_probs=np.array(real_tde_probs), probabilities=np.array(probabilities), names=np.array(names), radec=np.array(radec), peakmags=np.array(peakmags))
    #
    # import matplotlib.pyplot as plt
    # plt.figure()
    # for i in range(len(real_tde_probs)):
    #     plt.text(real_tde_probs[i], 5., names[i], rotation=90, color='red')
    #
    # import seaborn as sns
    # sns.set_style("white")
    # print(probabilities)
    # sns.distplot(probabilities, color='dodgerblue', label='Nuclear Transients in past year')
    # plt.xlim(0, 1)
    #
    ...
    # plt.legend()
    # plt.tight_layout()
    # plt.savefig('astrorapid/prob2_new.pdf')
...
if __name__ == '__main__':
    ...
    # classify_lasair_light_curves(object_names=['ZTF18abuxjrd'])
    ...
    nuc_tran_plot = np.load('plot_nuclear_transients2_new.npz')
    ...
    np.savetxt('/Users/danmuth/PycharmProjects/astrorapid/paper_plots/TDE_candidates_new.csv', np.hstack([radecs, peakmags])[probabilities > 0.4486])
    ...
    sns.set_style("white")
    print(probabilities)
    sns.distplot(probabilities, color='dodgerblue', label='Nuclear Transients in past year', kde=False, norm_hist=False)
    plt.xlim(0, 1.)
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig('astrorapid/prob2_unnormalised_new.pdf')
    ...
    plt.tight_layout()
    plt.savefig('astrorapid/cumulativeTDE_new.pdf')

    # Type Ia SNe in ZTF from OSC
    # snia_names = []
    # import json
    # with open('ZTF_SNIa.json') as json_file:
    #     data = json.load(json_file)[:3]
    ...
    #
    # classify_lasair_light_curves(object_names=snia_names, figdir='astrorapid/real_snia_from_osc/nodc_wredshift')
    ...
    classify_lasair_light_curves(object_names=nuclear_transients, figdir='astrorapid/nuclear_transients')
