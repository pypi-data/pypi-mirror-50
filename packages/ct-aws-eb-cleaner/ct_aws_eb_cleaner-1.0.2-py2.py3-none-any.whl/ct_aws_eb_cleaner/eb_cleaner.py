...
def execute(command: str) -> Any:
    ...
    if stderr is not None:
        ...
        print('[info] Command response:', stdout)
        return stdout
...
def eb_delete_application_version(application_name: str, version_label: str):
    command = f"aws elasticbeanstalk delete-application-version --application-name \"{application_name}\" --version-label \"{version_label}\""
    execute(command)
...
def run():
    ...
    for app_version in applications_versions['ApplicationVersions']:
        if app_version['VersionLabel'] not in active_application_version:
            print('[info] Deleting in application environment \'{}\' the application version \'{}\'.'.format(app_version['ApplicationName'], app_version['VersionLabel']))
            eb_delete_application_version(
