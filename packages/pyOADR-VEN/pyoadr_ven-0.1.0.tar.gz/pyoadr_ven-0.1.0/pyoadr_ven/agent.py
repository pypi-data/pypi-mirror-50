...
class OpenADRVenAgent:
    """
        ...
        RPC calls (see method documentation): **HA** commented as not needed for now
    ...
    def initialize_config(self, config):
        """
            ...
            Configuration parameters (see openadrven.config for a sample config file):
                ...
                client_pem_bundle:  The pem bundle for the user that has been generated on the vtn. Pem bundle (private key + certificate concatenated)
                vtn_ca_cert:  The path of the certificate from the CA that the client certs have been signed with.
        """
    ...
    def onstart_method(self):
        ...
        for event in self._get_events():
            _LOGGER.debug("Re-caching event with ID {}".format(event.event_id))
            self._active_events[event.event_id] = event
        for report in self._get_reports():
            _LOGGER.debug(
                "Re-caching report with ID {}".format(report.report_request_id)
            )
    ...
    def process_event(self, evt):
        ...
        """
        # if evt.is_active_or_pending():
        #     self._complete_event_if_over(evt)
        #     if evt.status == evt.STATUS_ACTIVE:
        #         # It's an active event. Which is fine; nothing special needs to be done here.
        #         return
        #     self._activate_event_if_starts_now(evt)

        now = get_aware_utc_now()
        if evt.is_active_or_pending():
            _LOGGER.debug("event is active")

            _LOGGER.info("timenow: %s", now)
            _LOGGER.info("endtime: %s", evt.end_time)
            _LOGGER.info("boolean: %s", now > evt.end_time)
            if evt.end_time is not None and now > evt.end_time:
                _LOGGER.debug(
                    "Setting event {} to status {}".format(
                        evt.event_id, evt.STATUS_COMPLETED
                    )
                )
    ...
    def _complete_event_if_over(self, event):
        ...
        if now > event.end_time:
            _LOGGER.debug(
                "Setting event %s to status %s", event.event_id, event.STATUS_COMPLETED
            )
            self.set_event_status(event, event.STATUS_COMPLETED)

    def _activate_event_if_starts_now(self, event):
        now = get_aware_utc_now()
        if now < event.start_time:
            return
        if event.opt_type is not event.OPT_TYPE_OPT_IN:
            return
        _LOGGER.debug(
            "Setting event {} to status {}".format(event.event_id, event.STATUS_ACTIVE)
        )
        self.set_event_status(event, event.STATUS_ACTIVE)
    ...
    def force_opt_type_decision(self, event_id):
        ...
        if (
            ...
            and event.opt_type
            not in [models.EiEvent.OPT_TYPE_OPT_IN, models.EiEvent.OPT_TYPE_OPT_OUT]
        ):
    ...
    def handle_oadr_event(self, oadr_event):
        ...
        def create_temp_event(received_ei_event):
            ...
            if event_id is None:
                raise exceptions.OpenADRInterfaceException(
                    "Missing eiEvent.eventDescriptor.eventID",
                    response_codes.OADR_BAD_DATA,
                )
            _LOGGER.debug("Processing received event, ID = {}".format(event_id))
            tmp_event = models.EiEvent(self.oadr_current_request_id, event_id)
            extractor = extractors.OadrEventExtractor(
                event=tmp_event, ei_event=received_ei_event
            )
        ...
        def update_event(temp_event, event):
            ...
            _LOGGER.debug(
                "Modification number has changed: {}".format(
                    temp_event.modification_number
                )
            )
            # OADR rule 57: If modificationNumber increments, replace the event with the modified version.
            if event.opt_type == models.EiEvent.OPT_TYPE_OPT_OUT:
                ...
                if temp_event.status == models.EiEvent.STATUS_CANCELED:
                    if event.status != models.EiEvent.STATUS_CANCELED:
                    ...
                    if (
                        event.status == models.EiEvent.STATUS_CANCELED
                        and temp_event.status != models.EiEvent.STATUS_CANCELED
                    ):
        ...
        def create_event(event):
            ...
            if event.status == models.EiEvent.STATUS_CANCELED:
                ...
                _LOGGER.debug(
                    "Event arrived from VTN, What shall we do with it OpenDSR??? We should automatically opt-in. Be aware 'self.core.schedule()' commented"
                )
                # **HA** ToDo: Opt in
        ...
        if (
            ...
            if event:
                if temp_event.modification_number < event.modification_number:
                    _LOGGER.debug(
                        "Out-of-order modification number: {}".format(
                            temp_event.modification_number
                        )
                    )
                ...
                _LOGGER.debug("Creating event for ID {}".format(temp_event.event_id))
                event = temp_event
                create_event(event)
    ...
    # ***************** Event Cache Requests ********************
    def unresponded_events(self) -> list:
        events = self.EiEvents
        return [
            event
            for event in events
            if event.status == models.EiEvent.STATUS_UNRESPONDED
        ]
    def near_events(self) -> list:
        events = self.EiEvents
        return [event for event in events if event.status == models.EiEvent.STATUS_NEAR]

    def far_events(self) -> list:
        events = self.EiEvents
        return [event for event in events if event.status == models.EiEvent.STATUS_FAR]

    def active_events(self) -> list:
        """Return a list of events that are currently in progress - status is active.
        """
        events = self.EiEvents
        return [event for event in events if event.status == models.EiEvent.STATUS_ACTIVE]
    ...
    def is_event_in_progress(self) -> bool:
        ...
        @returns bool: True if an event in progress
        """
    ...
    def _get_events(
        ...
        if event_id is not None:
            events = filter(lambda x: x.event_id == event_id, events)
        if in_progress_only:
            events = filter(
                lambda x: x.status
                not in [
                    models.EiEvent.STATUS_COMPLETED,
                    models.EiEvent.STATUS_CANCELED,
                ],
                events,
            )
        if started_after:
            events = filter(lambda x: x.start_time > started_after, events)
        if end_time_before:
            ...
            events = filter(
                lambda x: x.end_time < end_time_before if x.end_time else True, events
            )
        return list(events)

    def add_event(self, event):
        """A new event has been created. Add it to the event cache"""
        self._active_events[event.event_id] = event
        self.EiEvents.append(event)
        self.commit()

    def set_event_status(self, event, status):
        _LOGGER.debug(
            "Transitioning status to {} for event ID {}".format(status, event.event_id)
        )
        event.status = status
        self.commit()

    def expire_event(self, event):
        """Remove the event from the event cache."""
    ...
    def _get_reports(
        ...
        if active_only:
            reports = filter(
                lambda x: x.status
                not in [
                    models.EiReport.STATUS_COMPLETED,
                    models.EiReport.STATUS_CANCELED,
                ],
                reports,
            )
        if started_after:
