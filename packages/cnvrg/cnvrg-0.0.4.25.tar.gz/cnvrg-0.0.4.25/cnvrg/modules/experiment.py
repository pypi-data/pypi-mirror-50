...
class Experiment(CnvrgJob):
    ...
    def __run_callable(function_command, title=None, project=None, cwd=None, library=None, **kwargs):
        ...
        exit_status = control_stdout_helper.run_callable(function_command, callback=lambda x: experiment.log(x), err_callback=lambda x: experiment.log(x, log_type=LOGS_TYPE_ERROR))
        experiment.finish(exit_status=exit_status)
        return experiment

    @staticmethod
    def __run_local(cmd, **kwargs):
        function_command = lambda: spawn_helper.run_sync(cmd, print_output=True, cwd=kwargs["cwd"])
        return Experiment.__run_callable(function_command, cmd=cmd, **kwargs)

    @staticmethod
    def run(command=None, title=None, project=None, compute=None, computes=None, datasets=None, dataset=None, local=True, library=None, working_directory=None, sync_before=True, sync_after=False, **kwargs):
        ...
        if "local" in computes: local = True
        if len(computes) == 0: local = True
