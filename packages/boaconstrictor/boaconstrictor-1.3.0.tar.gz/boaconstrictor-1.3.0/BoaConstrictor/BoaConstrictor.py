	...
	for hello in lines:
		...
		if skipping_line == False:
			...
			try:
				...
				comment_index = 0
			try:
				switch_index = line.index(' switch ')
			except:
				switch_index = -1
			if ((' switch ' not in line) or (switch_index < comment_index)):
				to_append = []
				line_ended = False
				asdf = 0
				x=0
				...
				current_switch_value = switch_index + 1
				skipping_line = True
				next_line_loc = (lines[hello + 1].index('case')) + 1
				to_append.append(f"if {current_switch_value} == {lines[hello + 1][next_line_loc]}:")
				the_tabs += 1
...
def main():
	...
	cli_parser.add_argument("filename", help="Path to the *.bc file you want compiled, replacing <<\\>> characters with <</>>")
