...
def is_ascii(char: str) -> bool:
    """
    Returns whether or not a single character is an ASCII character.
    :param char: A single character
    :return: If that character is an ASCII character.
    """

    if len(char) == 0:
        raise ValueError("Cannot determine if an empty string is ascii or not.")

    if len(char) > 1:
        raise ValueError("The value {} is not a single character!".format(char))
...
def strip_non_ascii(string: str) -> str:
    ...
    :return: A string with no non-ascii characters in it.
    """
    good_characters = []

    for char in string:
        if is_ascii(char):
            good_characters.append(char)
