...
class RabbitMQ:
    ...
    def rabbitmq_publisher(self, message, queue, routing_key, exchange="", delivery_mode=2, mode="single"):
        ...
        if mode == "multi":
            thread = 20
            try:
                for batch in tqdm(range(0, len(message), thread * 10)):
    ...
    def rabbitmq_consumer(self, queue, size=1):
        try:
            results = []
            queue = [queue] * size
            thread = min(int(size / 5) + 1, 100)
            for batch in tqdm(range(0, size, thread * 5)):
                queue = queue[batch:batch + (thread * 5)]
                pool = ThreadPool(thread)
                pool_results = pool.map(self.get_from_rabbitmq, queue)
                pool.close()
                pool.join()
