...
class IndegoAPI():
    ...
    def getModel(self):
        ...
        elif value == '3600HB0100':
            self._state = 'Indego 350 Connect'
        elif value == '3600HB0101':
            self._state = 'Indego 400 Connect'
        else:
    ...
    def getBattery(self):
        """ Get OperatingData """
        _LOGGER.debug("getOperatingData")
        complete_url = 'alms/' + self.serial + '/operatingData'
        _LOGGER.debug("Complete URL: " + complete_url)
        temp = self.get(complete_url)
        #value = temp['state']
        value = temp
        self._state = value
        _LOGGER.debug(f"OperatingData RAW = {value}")
        #return self._state
...
###
    ...
    def getNextCut(self):
        """ Get NextCut """
        _LOGGER.debug("getNextCut")
        #https://api.indego.iot.bosch-si.com/api/v1/alms/{{alm_sn}}/predictive/nextcutting?withReason=true]
        complete_url = 'alms/' + self.serial + '/predictive/nextcutting?withReason=true'
        _LOGGER.debug("Complete URL: " + complete_url)
        temp = self.get(complete_url)
        #value = temp['state']
        value = temp
        self._state = value
        _LOGGER.debug(f"NextCut = {value}")
        return self._state
