...
def pvpolyfit(train_df, test_df, Y_tag, xs, I_tag, ghi_tag, cs_tag, highest_num_clusters, highest_degree, Y_high_filter, min_count_per_day, plot_graph = True, graph_type = 'regression', print_info = False):
    pvpoly = PVPolyfit(train_df, test_df, Y_tag, xs, I_tag, ghi_tag, cs_tag)

    # TODO: raise Exception for common error of rmse failure due to clustering number
    # TODO: add loop so num_clusters does not have to be input
            #TODO: alternative: just have user loop PVPolyfit 
    ...
    rmse_list = []
    std_rmse_list = []
    pvpoly_objects = [] #[copy.deepcopy(pvpoly)]*len(range(1,highest_num_clusters))
    for i in range(1, highest_num_clusters):
        pvpoly_iter = copy.deepcopy(pvpoly)
        try:
            pvpoly_iter.run(num_clusters = i, num_iterations = 1, degrees = list(range(1,highest_degree)))
    ...
    if len(rmse_list) == 0:
        raise Exception("Error 100. Check documentation for common errors.")
    ...
    days_rmses = pvpoly_objects[min_idx].plot(graph_type = graph_type, print_info = print_info, plot_graph = plot_graph)

    # TODO: delete print statements
    # TODO: finish methods
    # TODO: play with clustering inputs - want to make obvious ones ft
    # TODO: clean out unused code

    return rmse_list[min_idx], std_rmse_list[min_idx], days_rmses
...
class PVPolyfit:
    ...
    Additionally, the datetime index must be of the form '%m/%d/%Y %I:%M:%S %p'
        Refer here if change is necessary: https://stackoverflow.com/questions/49168546/panda-changing-dataframe-date-index-format
    '''

    def __init__(self, train_df, test_df, Y_tag, xs, I_tag, ghi_tag, cs_tag):
    ...
    def prepare(self, Y_high_filter, min_count_per_day):
        ...
        self.train_df = preprocess.data_preprocessing(self.train_df, self.xs, self.Y_tag, self.I_tag, Y_high_filter)
        ...
        index_list, day_hour_list, self.cut_results, self.train_df = utilities.find_and_break_days_or_hours(self.train_df, True, min_count_per_day = min_count_per_day, frequency = 'days')
        ...
        self.test_df = preprocess.data_preprocessing(self.test_df, self.xs, self.Y_tag, self.I_tag, Y_high_filter)
        ...
        test_index_list, test_day_hour_list, self.test_cut_results, self.test_df = utilities.find_and_break_days_or_hours(self.test_df, True, min_count_per_day = min_count_per_day, frequency = 'days')
    ...
    def run(self, num_clusters = 6, num_iterations = 1, degrees = list(range(1,10))):
        ''' 
        ...
        for degree in self.degrees:
            ...
            _,_,dfg,_ = utilities.find_and_break_days_or_hours(best_df, False, min_count_per_day = 0, frequency = 'days')
    ...
    def plot(self, graph_type = 'regression', print_info = True, plot_graph = False):
        ...
        if graph_type == 'regression':
            ...
            colors = ['red', 'blue', 'green', 'orange', 'purple', 'brown', 'yellow', 'pink', 'gray', 'cyan', 'darkgreen', 'burleywood', 'lawngreen', 'cornflowerblue']*5
            markers = ['x', 'o', '*', 'v', 'D', '+']*5
            for i in range(len(self.all_best_dfs[0])):
                model_number = self.test_km_labels[i]
                color = colors[model_number]
                marker = markers[model_number]
