...
#            if d.cell_parameter_bounds != [(d.cell_parameters_optimized[i]*(1.0-percentage/100.0),self.cell_parameters_optimized[i]*(1.0+percentage/100.0)) for i in range(6)]
    ...
    def DoOptimization(self,algorithm="all nonlinear",RIR_implementation="area of highest peak only",chemistry_weight=None,chemistry_constraint_a=None,chemistry_constraint_b=None,print_start_and_finish=True):
        """possible_algorithms:
            "2-step",
            "all nonlinear",
        """
        ...
        sol = self.RunFlexibleOptimization(algorithm,chemistry_weight,chemistry_constraint_a,chemistry_constraint_b)
    ...
    def RunFlexibleOptimization(self, algorithm="2-step", chemistry_weight=None, chemistry_constraint_a=None, chemistry_constraint_b=None):
        ...
        if algorithm == "2-step":
            if len(x) > 0:
                sol = minimize( self.TwoStep, x, bounds = bounds, method='L-BFGS-B', args=(chemistry_weight) )
                self.algorithm_number_of_iterations = sol.nit
                sol_is_defined = True
            else:
                self.algorithm_number_of_iterations = 0
                sol_is_defined = False
        if algorithm == "all nonlinear":
...
#        self.algorithm_number_of_iterations = sol.nit
        ...
        if algorithm == "2-step":
            ...
            list_of_profiles = [np.array(backgroundYs)]
            if len(profileYs) > 0:
                list_of_profiles.append(np.array(profileYs))
            if len(difYs) > 0:
                list_of_profiles.append(np.array(difYs))
            A = np.vstack(list_of_profiles).T

            if (chemistry_weight != 0.0) and (chemistry_weight is not None):
                # construct constraint equations, one oxide at a time
                self.CalculateChemistry()
                for oxide in self.all_oxide_names:
                    #print oxide
                    row = []
                    for b in self.background_optimized:
                        row.append(0.0)
                    for p in self.profiles:
                        if p.RIR < 0.001:
                            entry = 0.0
                        else:
                            entry = (p.chemistry.get(oxide,0.0) - self.input_profile.chemistry.get(oxide,0.0)) / p.RIR
                            # entry *= p.area_with_scaling_1
                            #FIXME: should I implement some kind of scaling as above?
                        row.append(entry)
                    for d in self.difs:
                        if d.RIR < 0.001:
                            entry = 0.0
                        else:
                            entry = (d.chemistry.get(oxide,0.0) - self.input_profile.chemistry.get(oxide,0.0)) / d.RIR
                            # entry *= d.GetArea(scaling=1.0, start_2theta=self.start_2_theta, end_2theta=self.end_2_theta)
                            #FIXME: should I implement some kind of scaling as above?
                            row.append(entry)
                    A = np.vstack([A,np.array(row)])
                    #print "A", A
                
                v = np.append(np.ones(len(inputXY[0])),np.full(len(self.all_oxide_names),chemistry_weight))
                weight_matrix = np.diag(v)
        
                A = np.dot(weight_matrix,A)
                
                b = np.append(inputXY[1],[0.0]*len(self.all_oxide_names))
                
                solution = np.linalg.lstsq(A, b, rcond=None)
            
            else:
                solution = np.linalg.lstsq(A, inputXY[1], rcond=None)
...
#                 d.Find2ThetasFromCellParameters()
    ...
    def TwoStep(self, x, chemistry_weight):
        inputXY = np.array(self.input_profile.xy_data)
