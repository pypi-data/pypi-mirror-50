...
class Disclosure(Mongo):
    ...
    def addMissingPeople(self, missing):
        ...
        for u in sorted(missing, key=lambda x: x['full_name']):
            # _id = u['object_id']
            pid = u['id']
            u['pid'] = str(pid)
            ...
            if not u['end_date']:
                u['end_date'] = '2999-12-31'
            self.peopledb.insert_one(u)
        print
    ...
    def updatePerson(self, person):
        ...
        for key in person:
            current[key] = person[key]

        if 'ccure_person_id' in current:
            del current['ccure_person_id']

        del current['_id']

        self.peopledb.replace_one({'pid': str(pid)}, current)
