...
class Disclosure(Mongo):
    ...
    def addMissingPeople(self, missing):
        ...
        for p in sorted(missing, key=lambda x: x['full_name']):
            ...
            # email = str(u['email'])
            # print('   Adding person '+full_name+' <'+email+'> ['+str(pid)+']')
            # # convert ints to strings before saving them.
            # for k in u:
            #     if isinstance(u[k], int):
            #         u[k] = str(u[k])
            # if not p['end_date']:
            #     p['end_date'] = '2999-12-31'
    ...
    def preparePerson(self, p):
        ...
        if 'id' in p:
            ...
            print('Person has no "id" set: %s' % (p['full_name']))
            sys.exit()
    ...
    def prepareUser(self, g):
        """Prepare a user record for the database."""
        # picture = 'https://lh3.googleusercontent.com/-XdUIqdMkCWA/AAAAAAAAAAI/AAAAAAAAAAA/4252rscbv5M/photo.jpg'
        # if 'profile' in g and 'image' in g['profile']:
        #     picture = g['profile']['image']['url'].replace('?sz=50', '')

        # pid = 0
        # if 'person' in g and 'pid' in g['person']:
        #     pid = g['person']['pid']
    ...
    def comparePerson(self, d, p):
        ...
        for k in sorted(p):
            ...
            if d[k] != p[k]:
                update = True
                if k not in ['adp_sync_date', 'last_updated']:
    ...
    def renameUser(self, d, g):
        """Rename a user."""
        did = d['object_id']
        gid = g['object_id']

        # get current user
        current = self.usersdb.find_one({'_id': did})
        if not current:
            print('User not found! '+str(did))
            return

        # delete current user
        self.usersdb.delete_one({'_id': did})

        # re-add current user with new id
        current['_id'] = gid
        self.usersdb.insert_one(current)
    ...
    def updatePerson(self, person):
        ...
        if 'ccure_person_id' in new:
            del new['ccure_person_id']

        if current_id != person_id:
            self.peopledb.delete_one({'_id': current_id})
            self.peopledb.insert_one(new)
        else:
    ...
    def updateUser(self, user):
        ...
        gid = user['object_id']
        email = user['email']
        current = self.usersdb.find_one({'_id': gid})
        if not current:
        ...
        self.usersdb.replace_one({'_id': gid}, current)

    def updatePeople(self, disclosure_people, people):
        """Update people in the database."""
        count = 0
        for oid in sorted(disclosure_people, key=lambda x: disclosure_people[x]['full_name']):
            d = disclosure_people[oid]
            pid = d['pid']
            if int(pid) not in people:
    ...
    def updateUsers(self, disclosure_users, google_users, people):
        ...
        for email in sorted(disclosure_users):
            ...
            uid = d['object_id']
            if uid not in google_users:
                # print('Google user not found: '+uid)
                continue
            g = self.prepareUser(google_users[uid])
            update = self.compareUser(d, g)

            # replace = False
            if update:
                count += 1
                if d['object_id'] != g['object_id']:
                    self.renameUser(d, g)
                self.updateUser(g)
        return count
