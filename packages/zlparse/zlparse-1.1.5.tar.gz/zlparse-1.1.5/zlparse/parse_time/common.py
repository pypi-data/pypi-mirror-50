...
def extimefbsj(ggstart_time, page):
    soup = BeautifulSoup(page, 'lxml')
    txt = re.sub('[^\u4E00-\u9Fa5a-zA-Z0-9:：\-\\/]', '', soup.text.strip())
    p = "(?:发布时间|信息时间)[：:](20[0-2][0-9])[\-\.年\\/]([1-9]|[0][1-9]|[1][0-2])[\-\.\\月/]([0-9]{,2})"
...
def extime(ggstart_time, page):
    soup = BeautifulSoup(page, 'lxml')
    txt = re.sub('[^\u4E00-\u9Fa5a-zA-Z0-9:：\-\\/]', '', soup.text.strip())
    p = "(?:更新日期|变更日期|更新时间|发文日期|发布日期|公示日期|发布于|公告时间|公示时间)[：:](20[0-2][0-9])[\-\.年\\/]([1-9]|[0][1-9]|[1][0-2])[\-\.\\月/]([0-9]{,2})"
    a = re.findall(p, txt.replace('documentwrite', ''))
    # print(a)
    if a != []:
        return '-'.join(a[0])
...
def strptime_transfromgg_guangdong_guangdongsheng(ggstart_time, page):
    list1 = []
    soup = BeautifulSoup(page, 'lxml')
...
def strptime_transfrom_yunan(ggstart_time, page):
    soup = BeautifulSoup(page, 'lxml')
    txt = re.sub('[^\u4E00-\u9Fa5a-zA-Z0-9:：\-\\/]', '', soup.text.strip())
...
def extimefbsj_v2(ggstart_time, page):
    if ggstart_time is not None:
        res = ggstart_time
    else:
        res = extimefbsj(ggstart_time, page)
    return res
...
def extime_sichuan_luzhou_ggzy1(ggstart_time, page):
    soup = BeautifulSoup(page, 'lxml')
...
def extime_jiangxi_dexing_ggzy1(ggstart_time, page):
    ...
    if txt!=[]:
        ...
        txt=soup.find_all('p',attrs={'class':'infotime'})
        if txt!=[]:
