...
def extime_all(page, ggstart_time, quyu, conp=False):
    ...
    if type(page_time) == type(curent_time) :
        ...
        page_time = pd.to_datetime(res.split()[0][:-1], format='%Y-%m-%d', errors='ignore')
        if page_time <= curent_time:
            return ext_from_ggtime(res.split()[0][:-1])
        else:
            return None
...
if __name__ == '__main__':
    ...
    print(extime_all('20', '2017-04-48', 'xxxxx'))