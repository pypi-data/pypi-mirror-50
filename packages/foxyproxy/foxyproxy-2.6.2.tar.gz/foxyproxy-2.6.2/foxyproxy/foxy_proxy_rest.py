    ...
    def init_rest(self):
        ...
        def terminal(name, limit=1):
            ...
            if (terminal_name is not None) and len(terminal_name) > 0:
                new_list = self.signer.prioritize(terminal_name, limit)
        ...
        def terminal2():
            ...
            if ("limit" in request_in) and isinstance(request_in['limit'], int):
                ...
                limit = 1
            if ("name" in request_in) and (len(request_in['name']) > 0):
                new_list = self.signer.prioritize(request_in['name'], limit)
