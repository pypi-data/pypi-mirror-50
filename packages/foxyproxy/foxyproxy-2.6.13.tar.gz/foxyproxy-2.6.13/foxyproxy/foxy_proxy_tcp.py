        ...
        while (not stop_event.is_set()) and (not _stopping):
            ...
            try:
                ...
                while available_threads < 1:
                    ...
                    if result.process_id in processes:
                        processes[result.process_id].join()
                        # processes[result.process_id].terminate()
                        del processes[result.process_id]
                    else:
                        logging.error("Inconsistency in processes map - %d" % result.process_id)

                    if available_threads == 50:
                        for _key, _value in processes.items():
                            logging.debug("Remaining processes %d, %s" % (_key, ""))

                    del result
                    available_threads += 1
                    logging.debug('Client thread released budget - case 0 (budget: %d)' % available_threads)
                    ...
                    try:
                        ...
                        try:
                            while True:
                                ...
                                available_threads += 1
                                logging.debug('Client thread released budget - case 1 (budget: %d)' % available_threads)
                                if available_threads == 50:
                                    for _key, _value in processes.items():
                                        logging.debug("Remaining processes %d, %s" % (_key, ""))
                        except Empty:
                ...
                new_client = ClientThread("tcp", conn, signer, request_results, process_counter)
                processes[process_counter] = new_client
                process_counter += 1
                new_client.name = "tcp connection"
                new_client.daemon = False
