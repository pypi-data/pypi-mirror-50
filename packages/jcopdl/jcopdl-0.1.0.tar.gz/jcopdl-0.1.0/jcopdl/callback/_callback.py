...
class Callback:
    ...
    def __init__(self, model, config=None, init_epoch=1, save_every=50, early_stop_patience=10,
                 plot_every=25, outdir="model"):
        ...
        self.ckpt = Checkpoint(model, config, init_epoch)
    ...
    def log(self, train_cost=None, test_cost=None, train_score=None, test_score=None):
        report = f'Epoch {self.ckpt.epoch:4}\n'
        if train_cost is not None:
            ...
            report += f'Train_cost  = {train_cost:.3f} | '
        if test_cost is not None:
            ...
            report += f'Test_cost  = {test_cost:.3f} |\n'
    ...
    def _runtime_plotting(self, scale, figsize, mode):
        ...
        if self.ckpt.epoch % self.plot_every == 0:
            ...
            plt.xlabel("Epoch")
            if mode == "Cost":
                plot_func(range(self.init_epoch, self.ckpt.epoch + 1), self.ckpt.train_cost, 'r-', label="Train")
                if self.ckpt.test_cost != []:
                    plot_func(range(self.init_epoch, self.ckpt.epoch + 1), self.ckpt.test_cost, 'b-', label="Test")
            elif mode == "Score":
...
class Checkpoint:
    def __init__(self, model, config, init_epoch):
        self.train_cost = []
        self.test_cost = []
        self.train_score = []
        self.test_score = []
