...
class BaseExperiment(object):
    ...
    def _get_experiment_key(self):
        ...
        if experiment_key is not None:
            if experiment_key.isalnum() and 32 <= len(experiment_key) <= 50:
    ...
    def get_callback(self, framework, *args, **kwargs):
        ...
        Note:
            This callback is already added to your Keras `model.fit()`
            callbacks list automatically, to report model training metrics
            to Comet.ml so you do not need to call this manually.
        """
        if framework == "keras":
