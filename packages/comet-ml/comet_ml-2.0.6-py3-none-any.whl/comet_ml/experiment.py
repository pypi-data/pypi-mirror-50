...
class BaseExperiment(object):
    ...
    def set_step(self, step):
        """
        Sets the current step in the training process. In Deep Learning
        each step is after feeding a single batch into the
        network. This is used to generate correct plots on
        Comet.ml. You can also pass the step directly when reporting
        [log_metric](#experimentlog_metric), and
    ...
    def _log_asset_data(
        self, data, file_name=None, overwrite=False, asset_type="asset", step=None
    ):
        self.set_step(step)
        try:
            fp = data_to_fp(data)
    ...
    def log_histogram(self, values, name=None, overwrite=False, step=None, **kwargs):
        """
        Logs a 2D histogram of values as an asset for this experiment.
        ...
        Args:
            ...
            name: str (optional), name of summary
            step: Optional. Used as the X axis when plotting on Comet.ml
            overwrite: bool (optional), overwrite previously-logged summary
            kwargs: Optional. Additional keyword arguments for histogram.
        """
        if isinstance(values, Histogram):
            histogram = values
