...
def add_arguments(parser):
    ...
    createwallet.add_argument("assetid", type=str, help="The ID of the asset to create a wallet for")
...
def _create_wallet(config):
    wallet = config.get_user_api().wallets.create(config.args.assetid, config.args.password)
    print(wallet.id, wallet.address)


def _list_wallets(config):
    for wallet in config.get_user_api().wallets.all():
        print(wallet.id, wallet.address, wallet.balances)
