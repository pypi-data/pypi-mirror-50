...
def pull(items, start=None, end=None, grid='15min', header='__auto__', capsules_as='auto', tz_convert=None,
    ...
    if not (isinstance(items, (pd.DataFrame, pd.Series))):
        raise RuntimeError('"items" must be a pandas.DataFrame or pandas.Series')
        ...
        try:
            ...
            for index, row in query_df.iterrows():
                ...
                if phase == 'signals' and 'Signal' in item_type:
                    ...
                    total_capsules = _pull_condition(capsules_as, formulas_api, formula, parameters, item_id,
                                                     item_name, output_df, pd_start, pd_end, tz_convert,
                                                     total_capsules, column_names, index, status_df,
...
def _pull_condition(capsules_as, formulas_api, formula, parameters, item_id, item_name, output_df, pd_start, pd_end,
                    tz, total_capsules, column_names, status_index, status_df, _drain_status_updates):
    try:
        ...
        while True:
            ...
            if capsules_as == 'signals':
                ...
                for capsule in capsules_output.capsules:
                    ...
                    output_df.at[total_capsules, 'Capsule Start'] = _common.convert_to_timestamp(capsule.start, tz)
                    output_df.at[total_capsules, 'Capsule End'] = _common.convert_to_timestamp(capsule.end, tz)
                    for prop in capsule.properties:  # type: ScalarPropertyV1
