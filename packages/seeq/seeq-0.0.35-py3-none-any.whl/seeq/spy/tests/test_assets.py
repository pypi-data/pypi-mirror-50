...
def test_instantiate():
    ...
    # It won't like the "Instance Template" column since we're specifying the HVAC class directly
    with pytest.raises(RuntimeError):
        spy.assets.instantiate(HVAC, hvac_metadata_df)

    hvac_metadata_df = hvac_metadata_df.drop(columns=['Instance Template'])
    ...
    push_results_df = spy.push(metadata=instances_with_calcs_df, errors='catalog')
...
@pytest.mark.system
def test_instantiate_with_module():
    hvac_metadata_df = spy.search({
        'Name': 'Area ?_*',
        'Datasource Class': 'Time Series CSV Files'
    })

    hvac_metadata_df['Instance Path'] = 'My HVAC Units >> Facility #2'
    ...
    hvac_metadata_df['Instance Template'] = hvac_metadata_df['Name'].apply(_template_chooser)

    hvac_metadata_df['Area'] = hvac_metadata_df['Name'].str.extract('(Area .)_.*')
    hvac_metadata_df['Instance Asset'] = hvac_metadata_df['Area'] + ' ' + hvac_metadata_df['Instance Template']

    instances_df = spy.assets.instantiate(sys.modules[__name__], hvac_metadata_df)
