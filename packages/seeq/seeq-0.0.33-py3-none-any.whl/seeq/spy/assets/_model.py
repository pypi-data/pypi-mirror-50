            ...
            def component_wrapper(self, metadata):
                ...
                for func_result in func_results:
                    if isinstance(func_result, Mixin):
                        component_instance = func_result  # type: Mixin
                        component_definitions.extend(component_instance.build(metadata))
                    elif isinstance(func_result, dict):
                        component_instance = func_result  # type: dict
                        Asset._add_asset_metadata(self, component_instance, None)
                        component_definitions.append(component_instance)
                    else:
