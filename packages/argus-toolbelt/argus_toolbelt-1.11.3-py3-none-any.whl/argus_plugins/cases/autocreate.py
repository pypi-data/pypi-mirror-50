...
def autocreate(
    ...
    def _create_case(customer: dict, group: str, description: str, data: dict):
        ...
        case = create_case(
            ...
            watchers=_get_case_watchers(event=data,
    ...
    def _get_case_watchers(event: dict = None,
                           case_watcher_from_field: str = None,
                           case_watcher: str = None,
                           watcher_type: ("email", "sms") = "email",
                           verbose: bool = False):
        watchers = [case_watcher] if case_watcher is not None else []
        watchers.append(value_from_nested_key(case_watcher_from_field,
                                                 event))
        # Had a problem with None being in the list.

        return _create_case_watcher_structure(watchers, watcher_type,
                                              verbose=verbose)

    def _create_case_watcher_structure(watchers: list,
                                       type: ("sms", "email"),
                                       verbose: bool = False):
        structured_watchers = []
