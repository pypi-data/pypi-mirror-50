...
class MultirotorClient(object):
    ...
    def disableApiControl(self, vehicle_name = ''):
        ...
        Returns:
            bool: Success
        """
        return self.client.call('disableApiControl', False, vehicle_name)
...
# -----------------------------------  Multirotor APIs ---------------------------------------------
    ...
    def moveByRollPitchYawZAsync(self, roll, pitch, yaw, z, duration, vehicle_name = ''):
        ...
        Roll (rate) / pitch (rate) / yaw (rate) set soints are given in **radians**, in the body frame. 
    ...
    def moveByVelocityAsync(self, vx, vy, vz, duration, drivetrain = DrivetrainType.MaxDegreeOfFreedom, yaw_mode = YawMode(), vehicle_name = ''):
        ...
        Args:
            ...
            vy (float): Description
            vz (float): Description
    ...
    def setTrajectoryTrackerGains(self, gains, vehicle_name = ''):
        ...
        Must be called once before either moveOnSpline* API is called
        
        Args:
            gains (list[float]): Pass TrajectoryTrackerGains().to_list() to set default values. Look at TrajectoryTrackerGains to set custom gains
            vehicle_name (str, optional): Name of the multirotor to send this command to 
        """
        self.client.call('setTrajectoryTrackerGains', gains, vehicle_name)

    def moveOnSplineAsync(self, waypoints, vel_max=15.0, acc_max=7.5, add_curr_odom_position_constraint=True, add_curr_odom_velocity_constraint=True, viz_traj=True, vehicle_name = ''):
        """
        ...
        Args:
            waypoints (list[Vector3r]): List of 3D waypoints, defined in local NED frame of the vehicle to track.
            vel_max (float, optional): Maximum magnitude of velocity along trajectory
            acc_max (float, optional): Maximum magnitude of acceleration along trajectory
            add_curr_odom_position_constraint (bool, optional): Add a start constraint of current linear velocity vector, so that the planned trajectory is smooth if the drone is already moving. 
            add_curr_odom_velocity_constraint (bool, optional): Should only be set to true if add_curr_odom_position_constraint is True. 
                If this is set to true, the trajectory planning backend will add a velocity vector constraint equivalent to the current odometry / linear velocity, so that the planned trajectory is smooth if the drone is already moving. 
            viz_traj (bool, optional): set this to True. visualize trajectory in unreal window. Note that visualization would appear in the FPV image, so this should only be used for debugging. 
            vehicle_name (str, optional): Name of the multirotor to send this command to 
        
        Returns:
            bool: Success
        """
        return self.client.call_async('moveOnSpline', waypoints, add_curr_odom_position_constraint, add_curr_odom_velocity_constraint, vel_max, acc_max, viz_traj, vehicle_name)

    def moveOnSplineVelConstraintsAsync(self, waypoints, velocity_constraints, vel_max=15.0, acc_max=7.5, add_curr_odom_position_constraint=True, add_curr_odom_velocity_constraint=True, viz_traj=True, vehicle_name = ''):
        """
        - Fits a minimum jerk trajectory to the list of given 3D waypoints (specified by the waypoints parameter).
        ...
        Args:
            waypoints (list[Vector3r]): List of 3D waypoints, defined in local NED frame of the vehicle to track.
            velocity_constraints (list[Vector3r]): List of 3D velocity vector constraints, defined in local NED frame of the vehicle to track.
            vel_max (float, optional): Maximum magnitude of velocity along trajectory
            acc_max (float, optional): Maximum magnitude of acceleration along trajectory
            add_curr_odom_position_constraint (bool, optional): Add a start constraint of current linear velocity vector, so that the planned trajectory is smooth if the drone is already moving. 
            add_curr_odom_velocity_constraint (bool, optional): Should only be set to true if add_curr_odom_position_constraint is True. 
        ...
        Returns:
            bool: Success
        """
        return self.client.call_async('moveOnSplineVelConstraints', waypoints, velocity_constraints, add_curr_odom_position_constraint, add_curr_odom_velocity_constraint, vel_max, acc_max, viz_traj, vehicle_name)
