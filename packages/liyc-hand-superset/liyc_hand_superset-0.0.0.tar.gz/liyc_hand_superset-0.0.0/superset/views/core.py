...
from superset.hand import base as handBase
from superset.hand import filters as handFilters
import io
...
class SliceModelView(SupersetModelView, DeleteMixin):  # noqa
    ...
    base_filters = [["id", handFilters.HandSliceFilter , lambda: []]]
    label_columns = {
...
class DashboardModelView(SupersetModelView, DeleteMixin):  # noqa
    ...
    base_filters = [["slice", handFilters.HandDashboardFilter, lambda: []]]
    label_columns = {
...
class Superset(BaseSupersetView):
    ...
    def copy_dash(self, dashboard_id):
        ...
        """Added By Hand Start"""
        all_perm = handBase.get_all_permissions()
        if ('can_add', 'DashboardModelView') not in all_perm:
            return json_error_response(msg="你没有添加看板权限", status=500)
        """Added By Hand End"""
    ...
    def excel(self, client_id):
        ...
        rejected_tables = self.rejected_datasources(
            query.sql, query.database, query.schema)
        if rejected_tables:
            flash(security_manager.get_datasource_access_error_msg('{}'.format(rejected_tables)))
            return redirect('/')
        blob = None
        if results_backend and query.results_key:
            logging.info(
