...
def config_configuration_parser(parser_config):
    ...
    config_required_group.add_argument(
        "--pretrained_weights",
        default="cryolo_model.h5",
        help="Path to h5 file that is used for initialization. Until you want to use weights from a previous dataset as initialization, the filename specified here should be same as saved_weights_name.",
        widget="FileSaver",
        gooey_options={
            "validator": {
                "test": 'user_input.endswith("h5")',
    ...
    config_model_group = parser_config.add_argument_group(
        "Model/Denoising", "Model configuration and denoising options"
    )
    ...
    config_model_group.add_argument(
        "--janni_model", help="Path to JANNI model", widget="FileChooser"
    )
    ...
    if "config" in sys.argv[1]:
        ...
        if args.filter == "LOWPASS":
            ...
            if args.janni_model is None:
                ...
                sys.exit(1)
            filter = [args.janni_model, args.filtered_output]
